  ...
  const stageHtml = days.length ? days.map(function(day, di) {
    ...
    const rows = stages.map(function(e) {
      ...
      const typeIcon = e.type === "opplevelse"
        ...
        : { start:"🏁", stop:"📍", lunch:"🍽️", middag:"🍷", hotel:"🏨", end:"🏁", fuel:"⛽" }[e.type] || "📍";
...
function serveDepartmentPage(dept, req, res) {
  ...
  historyEvents.sort(function(a, b) { return new Date(b.date) - new Date(a.date); }); // newest first
  ...
  function evCard(ev) {
    ...
    if (ev.eventType === "tur" && ev.route && ev.route.days && ev.route.days.length) {
      ...
      var STOP_ICONS = { start:"🚀", stop:"🅿️", lunch:"🍽️", middag:"🍷", hotel:"🏨", fuel:"⛽", opplevelse:"🎯", end:"🏁" };
      ...
      var dayRows = days.map(function(d, di) {
        ...
        var stops = (d.stages||[]).filter(function(e){ return ["lunch","middag","hotel","opplevelse","stop","fuel"].includes(e.type); });
  ...
  var divBord2    = _pc("borderLight",   "borderDark",   "#e0e0e0", "#333333");
  ...
  var typeStandBg  = _pc("boothBgLight", "boothBgDark",  "#e8f5e9", "#1a4a1a");
  var typeStandTxt = _pc("boothTxtLight","boothTxtDark", "#2e7d32", "#4caf50");
  var typeKursBg   = _pc("courseBgLight",  "courseBgDark",   "#e3f2fd", "#1a2a4a");
  var typeKursTxt  = _pc("courseTxtLight", "courseTxtDark",  "#1565c0", "#64b5f6");
  var typeTurBg    = _pc("tripBgLight",   "tripBgDark",    "#f3e5f5", "#3a1a4a");
  var typeTurTxt   = _pc("tripTxtLight",  "tripTxtDark",   "#6a1b9a", "#c084fc");
  var typeMoteBg   = _pc("meetingBgLight",  "meetingBgDark",   "#fffde7", "#3a3a1a");
  var typeMoteTxt  = _pc("meetingTxtLight", "meetingTxtDark",  "#f57f17", "#ffd700");
  var colCount = Math.max(1, columns.length);
  var css = '*{box-sizing:border-box;margin:0;padding:0}'
    + 'body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:'+bodyBg+';color:'+bodyClr+';min-height:100vh}'
  ...
  var html = '<!DOCTYPE html><html lang="no"><head>'
    ...
    + '<button onclick="showDeptQR()" title="Del lenke via QR" style="background:none;border:1px solid '+btnBorder+';color:'+btnTxt+';border-radius:8px;padding:6px 12px;font-size:.85rem;cursor:pointer;margin-right:.5rem;white-space:nowrap">📱 Del QR</button>'
    + '</div>'   // close header
  ...
  if (filtered.length === 0) {
    ...
    // Mobil tab-nav
    if (columns.length > 1) {
      html += '<div class="mob-tabs" id="mobTabs">';
    ...
    columns.forEach(function(kol, ki) {
      ...
      if (pagaende.length > 0) {
        html += '<div style="font-size:.68rem;text-transform:uppercase;letter-spacing:.8px;color:'+divLbl2+';margin:.75rem 0 .4rem;border-bottom:1px solid '+divBord2+';padding-bottom:.3rem">Pågående nå</div>';
    ...
    // ── History panel (events older than 2 weeks) ─────────────────
  ...
  // Build history toggle button (shown in footer area, only if there's history)
  var histBtnHtml = historyEvents.length > 0
    ? '<button id="historyToggleBtn" onclick="toggleHistory()" style="'
      + 'background:transparent;border:1px solid '+divBorder+';color:'+txt2+';'
      + 'padding:.5rem 1.1rem;border-radius:20px;font-size:.78rem;font-weight:700;'
      + 'cursor:pointer;display:flex;align-items:center;gap:.4rem;margin:1.5rem auto 0;'
      + 'transition:border-color .15s,color .15s">'
      + '🕐 Vis historikk (' + historyEvents.length + ')</button>'
    : '';
  ...
  html += '</div><footer>' + escHtml(siteName) + ' · ' + escHtml(dept.name) + '</footer>'
    ...
    + '  btn.innerHTML=open?"🕐 Vis historikk (' + historyEvents.length + ')":"✕ Skjul historikk";\n'
      ...
      (d.stages || []).forEach(function(e, eIdx) {
        [[e.from, eIdx===0&&dIdx===0?"start":e.type],
         [e.to, e.type==="end"?"end":e.type]
          ...
          var sym = tp==="start"||eIdx===0&&dIdx===0?"Flag, Green":tp==="end"||tp==="end"?"Flag, Red":tp==="hotel"||tp==="hotel"?"Lodging":tp==="lunch"||tp==="middag"?"Restaurant":tp==="bensinstopp"?"Gas Station":tp==="opplevelse"?"Scenic Area":"Waypoint";
...
app.put("/api/events/:id/route", auth, managerOrAdmin, function(req, res) {
  ...
  const clean = days.map(d => ({
    ...
    stages: Array.isArray(d.stages) ? d.stages.slice(0, 50).map(e => ({
      type:    ["start","stop","lunch","middag","hotel","end","fuel","opplevelse"].includes(e.type) ? e.type : "stop",
...
app.post("/api/ai/generate-route", auth, managerOrAdmin, rateLimit(10, 60000), async function(req, res) {
  ...
  let prompt;
  const validTypes = ["start","stop","opplevelse","lunch","middag","hotel","end","fuel"];
