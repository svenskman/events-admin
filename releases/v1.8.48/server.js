...
app.get("/api/tracker-position", auth, async function(req, res) {
...
app.put("/api/volunteers/:id/clothing", auth, managerOrAdmin, function(req, res) {
  ...
  var i = all.findIndex(function(v){ return v.id === req.params.id; });
  if (i < 0) return res.status(404).json({ error: "Not found" });
  var allowed = ["jacket","vest","cap","tshirt","hoodie","softshell","other"];
  ...
  allowed.forEach(function(item) {
    if (req.body[item] !== undefined) {
      clothing[item] = {
        ...
        note:     (req.body[item].note     || "").slice(0, 100)
      };
    }
  });
...
// Navigate to current live tracker position (catch-up)
async function catchUpToTracker() {
  var btn = document.getElementById('catchUpBtn');
  if (btn) { btn.disabled = true; btn.textContent = '⏳ Henter posisjon…'; }

  try {
    // Try to read position from server proxy
    var r = await fetch('/api/tracker-position?url=' + encodeURIComponent(LIVE_TRACKING_URL));
    var d = await r.json();
    if (d.ok && d.lat && d.lon) {
      var url = 'https://www.google.com/maps/dir/?api=1&destination=' + d.lat + ',' + d.lon + '&travelmode=driving';
      window.location.href = url;
      return;
    }
  } catch(e) {}

  // Fallback: open the tracker URL so user can get position manually
  if (btn) { btn.disabled = false; btn.textContent = '🏍 Kjør inn i kjøretøyet'; }
  // Show manual input
  var wrap = document.getElementById('catchUpManual');
  ...
  if (trackDiv) {
    ...
    if (!IS_PREVIEW && ev.eventType === 'tur' && ev.liveTrackingUrl && going) {
      ...
      trackDiv.innerHTML = '<div style="position:relative;width:100%;height:100%;overflow:hidden;border-radius:1vw">'
        ...
        + '<button id="catchUpBtn" onclick="catchUpToTracker()" style="position:absolute;bottom:.5rem;left:50%;transform:translateX(-50%);background:rgba(0,0,0,.75);color:#fff;border:1px solid rgba(255,255,255,.3);border-radius:6px;padding:5px 12px;font-size:.75rem;font-weight:700;cursor:pointer;white-space:nowrap;z-index:3">🏍 Kjør inn i kjøretøyet</button>'
        + '</div>'
        + '<div id="catchUpManual" style="display:none;margin-top:.5rem;padding:.6rem;background:rgba(0,0,0,.6);border-radius:8px">'
        + '<p style="font-size:.75rem;color:#ccc;margin:0 0 .4rem">Lim inn koordinater eller Google Maps-lenke:</p>'
        + '<div style="display:flex;gap:.4rem">'
        + '<input id="catchUpCoords" type="text" placeholder="59.123, 10.456" style="flex:1;padding:5px 8px;border-radius:5px;border:1px solid #555;background:#1a1a1a;color:#fff;font-size:.8rem"/>'
        + '<button onclick="catchUpManualNav()" style="background:var(--ev-accent,#e00);color:#fff;border:none;border-radius:5px;padding:5px 10px;font-size:.78rem;font-weight:700;cursor:pointer">Naviger</button>'
        + '</div></div>';
