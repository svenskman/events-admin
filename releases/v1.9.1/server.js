  ...
  <div class="ev-panel" id="tab-gb">
    ...
    <div class="ev-card" style="margin-top:1rem">
      ...
      <div id="gbPhotoArea" style="display:none;margin-top:.75rem">
        <button id="gbPhotoBtn" data-gbid="" onclick="gbPickFile()" style="display:flex;align-items:center;justify-content:center;gap:.5rem;width:100%;background:var(--y);color:#111;font-weight:700;padding:.75rem;border-radius:8px;font-size:.95rem;cursor:pointer;margin-bottom:.5rem;border:none">📷 ${T('gb_upload', lang)}</button>
...
async function registerStand(){
  ...
  if(r.ok){
    ...
    if(photoArea){
      ...
      // Auto-hide after 15 seconds if no upload started
      clearTimeout(window._gbAutoHideTimer);
      window._gbAutoHideTimer = setTimeout(function(){
        var pa=document.getElementById('gbPhotoArea');
        var lbl=document.getElementById('gbPhotoBtn');
        if(pa&&pa.style.display!=='none'&&lbl&&!lbl.dataset.gbid){pa.style.display='none';}
        else if(pa&&pa.style.display!=='none'){pa.style.display='none';}
      },15000);
    }
...
function gbPickFile() {
  // Create a fresh input element on every tap - most reliable on Android/Samsung
  var inp = document.createElement('input');
  inp.type = 'file';
  inp.accept = 'image/*,video/*';
  inp.multiple = true;
  // Must be in DOM and visible for Samsung Internet to fire onchange
  ...
  inp.addEventListener('change', function() {
    ...
    // Clean up after a delay (removing too early breaks some browsers)
    setTimeout(function() {
...
async function gbDirectUpload(input){
  ...
  // Use label as the visual button (btn was undefined — fixed)
