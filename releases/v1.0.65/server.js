...
app.post("/api/tv/:deptId/screenshot", auth, managerOrAdmin, function(req, res) {
  ...
  // Serve cached version if exists and <24h old
  ...
  execFile("wkhtmltoimage", [
    ...
    // wkhtmltoimage exits with code 1 on some sites even if screenshot succeeded
    if (fs.existsSync(dest) && fs.statSync(dest).size > 5000) {
      return res.json({ ok: true, screenshotUrl: "/uploads/" + path.basename(dest) });
    }
    res.status(500).json({ error: "Klarte ikke ta skjermbilde av siden" });
  });
});
...
#evDetailImg{width:100%;max-height:30vh;object-fit:cover;border-radius:12px;border:2px solid rgba(255,255,255,.1);display:none;margin-bottom:0}
...
function showEventCards(upcoming, activeIdx) {
  ...
  cards.innerHTML = upcoming.slice(0, count).map(function(ev, i) {
    ...
    var thumbWrapped = ev.image
      ...
      : '<div class="ev-thumb-placeholder" style="background:' + style.bg + ';' + ringStyle + '">'
          + (ev.eventType === 'stand' ? '\U0001f7e2' : ev.eventType === 'kurs' ? '\U0001f535' : ev.eventType === 'tur' ? '\U0001f7e3' : '\U0001f7e1')
        + '</div>';
