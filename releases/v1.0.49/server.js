...
// ── Avsendernavn-kaskade: undergruppe → avdeling → global ────────
function getEvSenderInfo(ev) {
  ...
  // Cascade name: subgroup → department → global siteName
...
async function sendEmail({ to, subject, html, text, replyTo, fromName: fromNameOverride }) {
  const s = getSettings();
  if (!s.emailEnabled) {
...
app.put("/api/departments/:deptId/subgroups/:sgId/appearance", auth, function(req, res) {
  ...
  // Also save displayName and contactEmail on the subgroup itself (not in appearance)
  if (app.displayName !== undefined)
...
app.post("/api/events/:id/register", rateLimit(10, 60000), function(req, res) {
  ...
  if (!req.body.silent) {
    ...
    sendEmail({ to: email, subject: mail.subject, html: mail.html, text: mail.text, fromName: _si.name }).catch(function(e) { console.error('[email] catch:', e && e.message); });
  }
