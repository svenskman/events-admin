...
function _broadcastTvUpcoming(department) {
  ...
  deptIds.forEach(function(deptId) {
    ...
    var upcoming = events
      .filter(function(e) {
        return e.department === deptId && e.date && new Date(e.date).getTime() > now - 3600000 && !e.hideFromList;
      })
...
app.post("/api/tv/:deptId/screenshot", auth, managerOrAdmin, function(req, res) {
  ...
  // A10: SSRF protection — block internal network URLs
  var urlObj;
...
app.get("/api/tv/:deptId/stream", function(req, res) {
  ...
  const upcoming = events
    .filter(function(e) { return e.department === deptId && e.date && new Date(e.date).getTime() > now - 3600000 && !e.hideFromList; })
    .sort(function(a, b) { return new Date(a.date) - new Date(b.date); })
    .slice(0, state.eventCount || 5)
    .map(function(e) {
        try {
...
app.get("/api/tv/:deptId/state", function(req, res) {
  ...
  const now = Date.now();
  const upcoming = events
    .filter(function(e) { return e.department === req.params.deptId && e.date && new Date(e.date).getTime() > now - 3600000 && !e.hideFromList; })
    .sort(function(a, b) { return new Date(a.date) - new Date(b.date); })
    .slice(0, state.eventCount || 5)
    .map(function(e) {
        try {
  ...
  <div id="eventLayer">
    ...
    <div id="evDetail">
      ...
      <div id="evDetailContent">
        ...
        <div id="tvImgDebug" style="display:none;font-size:.6rem;color:#f59e0b;background:rgba(0,0,0,.7);padding:2px 6px;border-radius:4px;font-family:monospace;position:absolute;top:4px;left:4px;z-index:10"></div>
...
function _showEvDetail(upcoming, idx, count) {
  ...
  // Debug: show paths if both exist and are identical
  var dbg = document.getElementById('tvImgDebug');
  if (dbg) {
    if (ev.image && ev.tvImageTop) {
      dbg.textContent = 'main: ' + ev.image.split('/').pop() + ' | top: ' + ev.tvImageTop.split('/').pop();
