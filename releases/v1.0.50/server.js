...
app.post("/api/events/:id/shifts", auth, managerOrAdmin, function(req, res) {
  ...
  const ev = events.find(function(e) { return e.id === req.params.id; });
  if (!ev) return res.status(404).json({ error: "Not found" });
  // Filter to only valid staff IDs that exist on this event
  ...
  if (!validIds.length) return res.status(400).json({ error: "No valid staff IDs provided" });
  if (!ev.shifts) ev.shifts = [];
  ev.shifts.push({ id: uuid(), staffIds: validIds, date, startTime, endTime });
  writeJSON(EVENTS_FILE, events);
  res.json({ ok: true });
});
