...
// ── Watchdog-integrasjon (NAF → styrer ddos-lab via watchdog) ───
const WATCHDOG_URL   = process.env.WATCHDOG_URL   || "";  // http://naf-watchdog:3001
...
const LAB_CONTAINER = process.env.LAB_CONTAINER || "ddos-lab";
const LAB_DATA_DIR  = process.env.LAB_DATA_DIR  || "";  // /lab-data montert fra NAF
const LAB_APP_DIR   = process.env.LAB_APP_DIR   || "";  // /lab-app montert fra NAF
...
app.post("/api/lab/remote/control", auth, adminOnly, rateLimit(10, 60000), async function(req, res) {
  ...
  if (!["restart", "restore", "copy-from-naf"].includes(action)) return res.status(400).json({ err: "Ugyldig handling" });

  // Kopier NAF sine egne filer til lab og restart
  if (action === "copy-from-naf") {
    ...
    try {
      ...
      if (!copied.includes("server.js")) return res.status(500).json({ err: "server.js ikke funnet i NAF /app" });
      await dockerRestart(LAB_CONTAINER);
      return res.json({ ok: true, files: copied, message: "NAF sine filer kopiert til lab – restartes nå" });
    } catch(e) {
  ...
  for (var i = 0; i < missing.length; i++) {
    ...
    try {
      const coords = await new Promise(function(resolve) {
        ...
        require("https").get(url, { headers: { "User-Agent": "naf-events/1.0" } }, function(r) {
          let data = "";
    ...
    try {
      ...
      async function reverseGeocode(lat, lon) {
        try {
          ...
          const r = await fetch(url, { headers: { "User-Agent": "naf-events/1.0" } });
          const d = await r.json();
          if (d && d.address) {
            const a = d.address;
