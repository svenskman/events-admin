...
function serveDepartmentPage(dept, req, res) {
  ...
  if (filtered.length === 0) {
    ...
    if (acctLink) {
      html += '<div style="display:flex;justify-content:flex-end;padding:.35rem 0 .1rem">'
        + '<a href="' + escHtml(acctLink) + '" target="_blank" rel="noopener" style="'
        + 'font-size:.72rem;font-weight:600;color:'+txt2+';text-decoration:none;'
        + 'display:flex;align-items:center;gap:.3rem;opacity:.75;transition:opacity .15s'
    ...
    if (showFilterBar) {
      ...
      columns.forEach(function(kol, ki) {
        html += '<button class="mob-tab' + (ki===0?' active':'') + '" onclick="switchMobTab(' + ki + ')">' + escHtml(kol.label) + '</button>';
      });
      if (columns.length > 1) {
        html += '<button class="mob-tab" id="mobTabAll" onclick="switchMobTabAll()" style="white-space:nowrap">Alle</button>';
      }
      html += '</div>';
      if (allHistCount > 0) {
