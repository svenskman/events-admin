    ...
    ? gb.map(function(g){
        ...
        const photoHtml = photos.length
          ? '<div style="display:flex;flex-wrap:wrap;gap:.4rem;margin:.6rem 0">'
            + photos.map(function(p){ return '<a href="'+p+'" target="_blank" rel="noopener"><img src="'+p+'" style="width:80px;height:80px;object-fit:cover;border-radius:6px;border:1px solid #333"/></a>'; }).join('')
            + '</div>'
          : '';
  ...
  sse.onmessage=function(e){
    ...
    if(data.ok){
      ...
      if(grid&&data.photoUrl){
        ...
        img.style.cssText='width:64px;height:64px;object-fit:cover;border-radius:6px;border:2px solid #4ade80';
    ...
    <div id="gbEntries">
      ${(ev.guestbook||[]).filter(g=>g.approved).length
        ? (ev.guestbook||[]).filter(g=>g.approved).map(function(g){
            const ph=(g.photos||[]).length?'<div style="display:flex;flex-wrap:wrap;gap:.4rem;margin:.6rem 0">'+(g.photos||[]).map(function(p){return'<a href="'+p+'" target="_blank" rel="noopener"><img src="'+p+'" style="width:80px;height:80px;object-fit:cover;border-radius:6px;border:1px solid #333"/></a>';}).join('')+'</div>':'';return `<div class="gb-entry"><div class="gb-name">${escHtml(g.name)}</div><div class="gb-msg">${escHtml(g.message).replace(/\n/g,'<br>')}</div>${ph}<div class="gb-date">${new Date(g.createdAt).toLocaleDateString('nb-NO',{day:'numeric',month:'long',year:'numeric'})}</div></div>`;
          }).join('')
