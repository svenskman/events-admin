    ...
    ? days.map(function(day, di){
        ...
        return `<div style="margin-bottom:1.5rem">
          ...
          ${etapper.map(function(e, ei){
            const icon = e.type === 'opplevelse'
              ? (opplevelseIcons[e.opplevelseSubtype] || '⭐')
              : (typeIcons[e.type] || '📍');
            const navUrl = e.til
              ? 'https://www.google.com/maps/dir/?api=1&destination=' + encodeURIComponent(e.til)
              : '';
            return `<div class="etappe-item">
              <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:.5rem">
                <div style="flex:1">
                  <div class="etappe-num">${icon} ${escHtml(e.type || 'stopp').toUpperCase()}</div>
                  <div class="etappe-name">${escHtml(e.fra || '')}${e.fra && e.til ? ' → ' : ''}${escHtml(e.til || '')}</div>
                  <div class="etappe-meta">
                    ${e.km ? `<span>📏 ${e.km} km</span>` : ''}
                    ${e.notat ? `<span style="color:var(--text2)">${escHtml(e.notat)}</span>` : ''}
                  </div>
                </div>
                ${navUrl ? `<a href="${escHtml(navUrl)}" target="_blank" rel="noopener" style="flex-shrink:0;background:var(--y);color:#111;font-weight:700;font-size:.75rem;padding:6px 10px;border-radius:6px;text-decoration:none;white-space:nowrap">🗺 Naviger</a>` : ''}
              </div>
            </div>`;
          }).join('')}
        </div>`;
...
${tabletCSS(accentColor, theme)}
<style>
.leaflet-container{background:${isLt?'#e8e0d8':'#1a1a1a'}}
.nav-etappe-active{border-left-color:${accentColor}!important;background:${isLt?'rgba(0,0,0,.05)':'rgba(255,255,255,.07)'}!important;transform:scale(1.01)}
.nav-etappe-done{opacity:.4}
.nav-pulse{animation:nav-pulse 1.8s ease-in-out infinite}
@keyframes nav-pulse{0%,100%{box-shadow:0 0 0 0 ${accentColor}99}60%{box-shadow:0 0 0 14px ${accentColor}00}}
</style>
...
<div class="ev-content">
  ...
  <div class="ev-panel" id="tab-rute">
    ${days.length ? `
    <!-- Kart -->
    <div id="ruteMapWrap" style="border-radius:10px;overflow:hidden;margin-bottom:1rem;border:1px solid var(--g3);position:relative;background:var(--g2)">
      <div id="ruteMap" style="height:280px;width:100%"></div>
      <div id="ruteMapStatus" style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);font-size:.82rem;color:var(--text2);pointer-events:none">Laster kart…</div>
    </div>
    ` : ''}
    <div class="ev-card">
  ...
  <!-- GPS Nav FAB – vises kun på etappeplan-fanen -->
  ${days.length ? `
  <div id="navFab" style="display:none;position:fixed;bottom:1.5rem;left:50%;transform:translateX(-50%);z-index:160;flex-direction:column;align-items:center;gap:.4rem">
    <div id="navFabStatus" style="background:rgba(0,0,0,.75);color:#fff;font-size:.72rem;padding:4px 12px;border-radius:20px;white-space:nowrap;max-width:90vw;overflow:hidden;text-overflow:ellipsis;text-align:center"></div>
    <button id="navFabBtn" onclick="toggleGpsNav()" style="background:var(--y);color:#111;border:none;border-radius:50px;padding:14px 28px;font-size:1rem;font-weight:900;cursor:pointer;display:flex;align-items:center;gap:.6rem;box-shadow:0 4px 24px rgba(0,0,0,.6);min-height:56px;white-space:nowrap">
      📍 Start GPS-navigering
    </button>
  </div>` : ''}
...
// ── GPS-navigering ─────────────────────────────────────────────
var ETAPPER=${JSON.stringify(days.map(function(day){return (day.etapper||[]).map(function(e){return {fra:e.fra||'',til:e.til||'',km:e.km||null,type:e.type||'stopp',notat:e.notat||''};});}))}; 
var ALL_ETAPPER=ETAPPER.flat();
var _map=null,_gpsWatch=null,_gpsActive=false,_posMarker=null,_accCircle=null,_curIdx=0,_routeLines=[];
var ACCENT='${accentColor}';

function initMap(){
  if(_map||!window.L)return;
  var wrap=document.getElementById('ruteMap');
  var status=document.getElementById('ruteMapStatus');
  if(!wrap)return;
  _map=L.map('ruteMap',{zoomControl:true,attributionControl:false});
...
async function geocodePlace(name){
  try{
    var r=await fetch('https://nominatim.openstreetmap.org/search?format=json&limit=1&q='+encodeURIComponent(name+', Skandinavia'),{headers:{'Accept-Language':'nb'}});
    var d=await r.json();
...
async function geocodeAndDraw(){
  if(!_map||!ALL_ETAPPER.length)return;
  var pts=[];
  var names=[];
  // Collect unique place names
  ALL_ETAPPER.forEach(function(e,i){
    if(i===0&&e.fra)names.push(e.fra);
    if(e.til)names.push(e.til);
  });
  // Geocode all
  for(var i=0;i<names.length;i++){
    var ll=await geocodePlace(names[i]);
    if(ll)pts.push({name:names[i],ll:ll});
    await new Promise(function(r){setTimeout(r,200);}); // Nominatim rate limit
  }
  if(pts.length<2){
    var s=document.getElementById('ruteMapStatus');
    if(s){s.textContent='Kunne ikke geocode ruten';s.style.display='block';}
    return;
  }
  // Draw route line
  var lls=pts.map(function(p){return p.ll;});
  var line=L.polyline(lls,{color:ACCENT,weight:4,opacity:.85}).addTo(_map);
  _routeLines.push(line);
  _map.fitBounds(line.getBounds(),{padding:[24,24]});
  // Draw markers
  pts.forEach(function(p,i){
    var isFirst=i===0,isLast=i===pts.length-1;
    var icon=L.divIcon({className:'',html:'<div style="background:'+(isFirst||isLast?ACCENT:'#fff')
      +';width:'+(isFirst||isLast?'16px':'11px')+';height:'+(isFirst||isLast?'16px':'11px')
      +';border-radius:50%;border:2px solid '+(isFirst||isLast?'#111':ACCENT)
      +';box-shadow:0 2px 6px rgba(0,0,0,.5)"></div>',
      iconSize:[16,16],iconAnchor:[8,8]});
    L.marker(p.ll,{icon:icon}).bindPopup('<strong>'+p.name+'</strong>').addTo(_map);
  });
  // Store geocoded points on etapper for GPS matching
  window._etappeGeoPoints=pts;
}

// Override showTab to init map and toggle GPS FAB
(function(){
...
function toggleGpsNav(){
  if(_gpsActive) stopGpsNav(); else startGpsNav();
}

function startGpsNav(){
  if(!navigator.geolocation){
    alert('GPS er ikke tilgjengelig i denne nettleseren.');return;
  }
  _gpsActive=true;
  _curIdx=0;
  var btn=document.getElementById('navFabBtn');
  if(btn){btn.innerHTML='⏹ Stopp navigering';btn.classList.add('nav-pulse');}
  highlightEtappe(0);
  _gpsWatch=navigator.geolocation.watchPosition(onGpsPos,onGpsErr,
...
function stopGpsNav(){
  ...
  var btn=document.getElementById('navFabBtn');
  if(btn){btn.innerHTML='📍 Start GPS-navigering';btn.classList.remove('nav-pulse');}
  var status=document.getElementById('navFabStatus');
  if(status)status.textContent='';
  clearHighlight();
  if(_posMarker){_posMarker.remove();_posMarker=null;}
...
function onGpsPos(pos){
  ...
  if(!_posMarker){
    var icon=L.divIcon({className:'',html:'<div style="width:18px;height:18px;border-radius:50%;background:'+ACCENT
      +';border:3px solid #fff;box-shadow:0 0 0 3px '+ACCENT+'55"></div>',iconSize:[18,18],iconAnchor:[9,9]});
    _posMarker=L.marker([lat,lon],{icon:icon,zIndexOffset:1000}).addTo(_map);
  } else { _posMarker.setLatLng([lat,lon]); }
  if(!_accCircle){
    _accCircle=L.circle([lat,lon],{radius:acc,color:ACCENT,fillColor:ACCENT,fillOpacity:.1,weight:1}).addTo(_map);
  } else { _accCircle.setLatLng([lat,lon]);_accCircle.setRadius(acc); }
  _map.panTo([lat,lon],{animate:true,duration:0.5});
  // Check proximity to next etappe destination
  var pts=window._etappeGeoPoints||[];
  if(_curIdx<ALL_ETAPPER.length&&pts.length>_curIdx+1){
    var nextPt=pts[_curIdx+1];
    var dist=haversine(lat,lon,nextPt.ll[0],nextPt.ll[1]);
    var status=document.getElementById('navFabStatus');
    var eta=ALL_ETAPPER[_curIdx];
    if(status)status.textContent=(eta?eta.til:'')+(dist<50000?' – '+fmtDist(dist)+' gjenstår':'');
    if(dist<500){
      _curIdx++;
      highlightEtappe(_curIdx);
      if(_curIdx>=ALL_ETAPPER.length){
        stopGpsNav();
        var s=document.getElementById('navFabStatus');
        if(s)s.textContent='🏁 Destinasjon nådd!';
        alert('🏁 Du har nådd destinasjonen!');
      }
    }
  }
}
...
function onGpsErr(e){
  var status=document.getElementById('navFabStatus');
  if(status)status.textContent='GPS-feil: '+e.message;
}

function highlightEtappe(idx){
  document.querySelectorAll('.etappe-item').forEach(function(el,i){
    el.classList.remove('nav-etappe-active','nav-etappe-done');
    if(i<idx)el.classList.add('nav-etappe-done');
    if(i===idx){
      el.classList.add('nav-etappe-active');
...
function clearHighlight(){
  document.querySelectorAll('.etappe-item').forEach(function(el){
    el.classList.remove('nav-etappe-active','nav-etappe-done');
  });
}
...
// Load Leaflet JS dynamically (CSS already in head)
(function(){
  var s=document.createElement('script');
  s.src='https://unpkg.com/leaflet@1.9.4/dist/leaflet.js';
  s.onload=function(){
    // Auto-init map if rute tab is active on load
    if(document.getElementById('tab-rute')&&document.getElementById('tab-rute').classList.contains('active'))initMap();
  };
