...
function buildPermissions(user, settings) {
  ...
  var editableDepartments = isAdmin
    ...
    : accessList.map(function(t) {
        ...
        // Fallback: if ID doesn't match, try slug or name (handles data migration issues)
        if (!dept) {
          dept = departments.find(function(a) {
            return (a.slug && a.slug === t.department) ||
                   (a.name && a.name.toLowerCase() === (t.department || '').toLowerCase());
          });
          if (dept) console.warn('[DEPT] accessList dept ID "' + t.department + '" not found by ID, matched by slug/name to "' + dept.name + '" (' + dept.id + ')');
          else console.warn('[DEPT] accessList dept ID "' + t.department + '" has no match in settings.departments — entry will be skipped');
        }
  ...
  try {
    ...
    const mailResult = await sendEmail({
      ...
      html: [
        ...
        "<p style='color:#888;font-size:.9em'>Konfigurer e-post (SMTP) under Innstillinger for å sende bekreftelses-mail til deltakere.</p>",
      ].join(""),
...
// ── Demo: force password re-sync (useful if container was restarted with new env) ──
app.post("/api/demo-reset-password", function(req, res) {
  if (!IS_DEMO) return res.status(404).json({ err: "Not a demo instance" });
  try {
    var users = readJSON(USERS_FILE);
    var demoUser = users.find(function(u) { return u.id === "demo-admin" || u.email === DEMO_EMAIL; });
    if (!demoUser) {
      // Re-seed entirely
      _seedDemoData(true);
      return res.json({ ok: true, action: "reseeded" });
    }
    demoUser.hash  = bcrypt.hashSync(DEMO_PASSWORD, 10);
    demoUser.email = DEMO_EMAIL;
    writeJSON(USERS_FILE, users);
    res.json({ ok: true, action: "hash_updated", email: DEMO_EMAIL });
  } catch(e) {
    res.status(500).json({ err: e.message });
  }
});
...
// ── Demo: auto-update from GitHub before each reset ──────────────
async function _demoAutoUpdate() {
  if (!IS_DEMO || !GITHUB_TOKEN || !GITHUB_REPO) {
    console.log("[DEMO] Auto-update skipped (no GITHUB_TOKEN/GITHUB_REPO configured)");
    return false;
  }
  try {
    // 1. Check manifest for latest version
    const UPDATE_BASE_URL = "https://raw.githubusercontent.com/" + GITHUB_REPO + "/main";
    const manifestR = await httpGet(UPDATE_BASE_URL + "/manifest.json");
    if (manifestR.status !== 200) { console.warn("[DEMO] Could not fetch manifest:", manifestR.status); return false; }
    const manifest = JSON.parse(manifestR.body.toString());
    const latest = manifest.version || "0.0.0";
    if (compareVersions(latest, APP_VERSION) <= 0) {
      console.log("[DEMO] Already on latest version " + APP_VERSION + ", skipping update");
      return false;
    }
    console.log("[DEMO] Update available: " + APP_VERSION + " → " + latest + ", downloading...");

    // 2. Download and verify files
    const crypto = require("crypto");
    const tmpDir = path.join(DATA, "_update_tmp_demo");
    try { fs.mkdirSync(tmpDir, { recursive: true }); } catch(e) {}
    const files = (manifest.files || []).filter(function(f) {
      return ["server.js","index.html","package.json"].includes(f.name);
    });
    if (!files.length) { console.warn("[DEMO] No files in manifest"); return false; }

    for (var fi = 0; fi < files.length; fi++) {
      var f = files[fi];
      var r = await httpGet(f.url);
      if (r.status !== 200) throw new Error("Download failed for " + f.name + " (HTTP " + r.status + ")");
      if (f.sha256) {
        var hash = crypto.createHash("sha256").update(r.body).digest("hex");
        if (hash !== f.sha256) throw new Error("Checksum mismatch for " + f.name);
      }
      fs.writeFileSync(path.join(tmpDir, f.name), r.body);
    }

    // 3. Patch version string in server.js
    var serverPath = path.join(tmpDir, "server.js");
    if (fs.existsSync(serverPath)) {
      var sc = fs.readFileSync(serverPath, "utf8");
      sc = sc.replace(/const APP_VERSION\s*=\s*["'][^"']*["']/, 'const APP_VERSION = "' + latest + '"');
      fs.writeFileSync(serverPath, sc);
    }

    // 4. Back up current files
    var backupDir = path.join(DATA, "_update_backup_" + APP_VERSION);
    try { fs.mkdirSync(backupDir, { recursive: true }); } catch(e) {}
    for (var fi2 = 0; fi2 < files.length; fi2++) {
      var src = path.join(process.cwd(), files[fi2].name);
      if (fs.existsSync(src)) try { fs.copyFileSync(src, path.join(backupDir, files[fi2].name)); } catch(e) {}
    }

    // 5. Place new files
    for (var fi3 = 0; fi3 < files.length; fi3++) {
      var dst = path.join(process.cwd(), files[fi3].name);
      fs.copyFileSync(path.join(tmpDir, files[fi3].name), dst);
    }
    try { fs.rmSync(tmpDir, { recursive: true, force: true }); } catch(e) {}

    console.log("[DEMO] Updated to v" + latest + " — restarting in 3s");
    setTimeout(function() { process.exit(0); }, 3000); // restart picks up new files
    return true;
  } catch(e) {
    console.error("[DEMO] Auto-update failed:", e.message);
    return false;
  }
}
...
if (IS_DEMO) {
  _seedDemoData(false);
  // Always ensure demo user hash matches current DEMO_PASSWORD env var
  // (covers case where data volume persists but password env var changed)
  (function _syncDemoPassword() {
    try {
      var users = readJSON(USERS_FILE);
      var demoUser = users.find(function(u) { return u.id === 'demo-admin' || u.email === DEMO_EMAIL; });
      if (demoUser) {
        ...
        demoUser.email = DEMO_EMAIL;
        writeJSON(USERS_FILE, users);
        console.log('[DEMO] Password hash synced to current DEMO_PASSWORD env var.');
      }
    } catch(e) { console.error('[DEMO] Failed to sync password hash:', e.message); }
  })();
  setInterval(async function() {
    console.log('[DEMO] Reset interval fired — checking for updates first...');
    var updated = await _demoAutoUpdate();
    if (!updated) {
      // No update or no token — just re-seed data
      _seedDemoData(true);
    }
    // If updated, process.exit(0) is called inside _demoAutoUpdate — no need to seed
  }, DEMO_RESET_H * 3600000);
}
...
app.post("/api/spond/sync", auth, adminOnly, async function(req, res) {
  ...
  spondEvents.forEach(function(se) {
    ...
    if (ed && ed !== d) {
      ...
      spondByDate[ed].push((se.heading || "Ukjent") + " (sluttdato)");
    }
  });
...
app.put("/api/users/:id/access", auth, async function(req, res) {
  ...
  if (me.role !== "admin") {
    ...
    const isDeptMgr = myAccess.some(function(t) { return t.role === "department_manager" || t.role === "avdelingsleder"; });
    if (!isDeptMgr) return res.status(403).json({ error: "Access denied" });
...
app.post("/api/ai/parse-pdf", auth, managerOrAdmin, rateLimit(5, 60000), async function(req, res) {
  ...
  if (MOCK_AI) {
    ...
    return res.json({ events: [
      ...
      { title:"Kickoff, frivillige", date:`${y}-03-02T17:00`, location:"Klubbhuset", description:"Kickoff for frivillige.", eventType:"stand", department:av, subgroup:null },
...
JSON format:
{
  ...
  "publicColors": {
    ...
    "standBgDark":    "<Stand-type badge background dark>",
    "standTxtDark":   "<Stand-type badge text dark>",
    "standBgLight":   "<Stand-type badge background light>",
    "standTxtLight":  "<Stand-type badge text light>",
    "kursBgDark":     "<Kurs-type badge background dark>",
    "kursTxtDark":    "<Kurs-type badge text dark>",
    "kursBgLight":    "<Kurs-type badge background light>",
    "kursTxtLight":   "<Kurs-type badge text light>",
    "turBgDark":      "<Tur-type badge background dark>",
    "turTxtDark":     "<Tur-type badge text dark>",
    "turBgLight":     "<Tur-type badge background light>",
    "turTxtLight":    "<Tur-type badge text light>",
    "moteBgDark":     "<Møte-type badge background dark>",
    "moteTxtDark":    "<Møte-type badge text dark>",
    "moteBgLight":    "<Møte-type badge background light>",
    "moteTxtLight":   "<Møte-type badge text light>",
    "badgeMote":      "<event-page hero Møte badge background>",
    "badgeMoteT":     "<event-page hero Møte badge text>",
    "badgeKurs":      "<event-page hero Kurs badge background>",
    "badgeKursT":     "<event-page hero Kurs badge text>",
    "badgeTur":       "<event-page hero Tur badge background>",
    "badgeTurT":      "<event-page hero Tur badge text>",
    "badgeStand":     "<event-page hero Stand badge background>",
...
function serveDepartmentPage(dept, req, res) {
  ...
  var domain   = DOMAIN;
  var now      = Date.now();
  // Use Norwegian locale for date formatting on department pages
  var pageLocale = "nb-NO";
  ...
  function evCard(ev) {
    ...
    return '<a class="' + cls + '" href="' + escHtml(url) + '" style="display:flex;align-items:stretch">'
      ...
      + (evDate
        ...
        + '<div class="ev-month"' + (dateColLit ? ' style="color:#333"' : '') + '>' + evDate.toLocaleDateString(pageLocale,{month:"short"}) + '</div>'
      ...
      + (timeStr ? '<div class="ev-meta">🕐 ' + timeStr + (endStr ? '–' + endStr : '') + (ev.endDate && ev.endDate !== ev.date.slice(0,10) ? ' (' + new Date(ev.endDate).toLocaleDateString(pageLocale,{day:"numeric",month:"short"}) + ')' : '') + '</div>' : '')
  ...
  function spotCard(ev, mode) {
    ...
    var monStr = evDate.toLocaleDateString(pageLocale,{month:"short"});
    var timeStr = ev.date.length > 10 ? evDate.toLocaleTimeString(pageLocale,{hour:"2-digit",minute:"2-digit"}) : null;
    var endStr  = ev.endTime ? ev.endTime.slice(0,5) : null;
    var typeLabel = getTypeLabel(ev.eventType, getSettings());
    var typeCls   = ev.eventType === "kurs" ? "type-kurs" : ev.eventType === "tur" ? "type-tur" : "type-stand";
  ...
  var headerBord  = _pc("borderLight",   "borderDark",   "#e0e0e0", "#2a2a2a");
  ...
  var cardBord    = _pc("borderLight",   "borderDark",   "#e0e0e0", "#2a2a2a");
  ...
  var ugBadgeBord = _pc("borderLight",   "borderDark",   "#cccccc", "#3a3a3a");
  ...
  var divBord     = _pc("borderLight",   "borderDark",   "#dddddd", "#333333");
  ...
  var btnBord     = _pc("borderLight",   "borderDark",   "#cccccc", "#333333");
  var btnTxt      = _pc("text2Light",    "text2Dark",    "#555555", "#888888");
  var typeStandBg  = _pc("standBgLight", "standBgDark",  "#e8f5e9", "#1a4a1a");
  var typeStandTxt = _pc("standTxtLight","standTxtDark", "#2e7d32", "#4caf50");
  var typeKursBg   = _pc("kursBgLight",  "kursBgDark",   "#e3f2fd", "#1a2a4a");
  var typeKursTxt  = _pc("kursTxtLight", "kursTxtDark",  "#1565c0", "#64b5f6");
  var typeTurBg    = _pc("turBgLight",   "turBgDark",    "#f3e5f5", "#3a1a4a");
  var typeTurTxt   = _pc("turTxtLight",  "turTxtDark",   "#6a1b9a", "#c084fc");
  var typeMoteBg   = _pc("moteBgLight",  "moteBgDark",   "#fffde7", "#3a3a1a");
  var typeMoteTxt  = _pc("moteTxtLight", "moteTxtDark",  "#f57f17", "#ffd700");
  ...
  var css = '*{box-sizing:border-box;margin:0;padding:0}'
    ...
    + '.header-brand{display:flex;align-items:center;gap:.75rem;flex-shrink:0;padding:.85rem 1.5rem;border-right:1px solid '+headerBord+'}'
    ...
    + '.ug-badge{font-size:.6rem;color:'+ugBadgeTxt+';background:'+ugBadgeBg+';border:1px solid '+ugBadgeBord+';padding:2px 6px;border-radius:20px}'
  ...
  var html = '<!DOCTYPE html><html lang="no"><head>'
    ...
    + (function() {
        ...
        function bannerItem(ev, mode) {
          ...
          return '<a href="' + escHtml(url) + '" class="ev-card banner-item ' + (isActive ? "active is-active" : "is-next") + '">'
            ...
            +     evDate.toLocaleDateString(pageLocale,{month:"short"})
            +   '</div>'
            + '</div>'
            + '<div class="ev-body" style="flex:1;min-width:0">'
            +   '<div class="ev-top">'
    ...
    + '<button onclick="showDeptQR()" title="Del lenke via QR" style="background:none;border:1px solid '+btnBord+';color:'+btnTxt+';border-radius:8px;padding:6px 12px;font-size:.85rem;cursor:pointer;margin-right:.5rem;white-space:nowrap">📱 Del QR</button>'
    + '</div>'   // close header
  ...
  if (filtered.length === 0) {
    ...
    columns.forEach(function(kol, ki) {
      ...
      if (tidligere.length > 0) {
        html += '<div style="font-size:.68rem;text-transform:uppercase;letter-spacing:.8px;color:'+divLbl+';margin:.75rem 0 .4rem;border-bottom:1px solid '+divBord+';padding-bottom:.3rem">Siste 2 uker</div>';
      ...
      if (kommende.length > 0) {
        if (pagaende.length > 0 || tidligere.length > 0) {
          html += '<div style="font-size:.68rem;text-transform:uppercase;letter-spacing:.8px;color:'+divLbl+';margin:.75rem 0 .4rem;border-bottom:1px solid '+divBord+';padding-bottom:.3rem">Kommende</div>';
        }
    ...
    if (historyEvents.length > 0) {
      ...
      html += '<div id="historySection" style="display:none;margin-top:2rem;padding-top:1.5rem;border-top:2px solid '+divBord+'">';
      ...
      if (histSubgroups.length > 0) {
        ...
        html += '<button onclick="filterHistory(null,this)" class="hist-filter-btn hist-active" style="font-size:.72rem;font-weight:700;padding:3px 10px;border-radius:20px;border:1px solid '+divBord+';background:'+txt2+';color:'+bodyBg+';cursor:pointer">Alle</button>';
        histSubgroups.forEach(function(sg) {
          html += '<button onclick="filterHistory(\'' + escHtml(sg.id) + '\',this)" class="hist-filter-btn" style="font-size:.72rem;font-weight:700;padding:3px 10px;border-radius:20px;border:1px solid '+divBord+';background:transparent;color:'+txt2+';cursor:pointer">' + escHtml(sg.label) + '</button>';
        });
        html += '</div>';
      }
  ...
  var histBtnHtml = historyEvents.length > 0
    ? '<button id="historyToggleBtn" onclick="toggleHistory()" style="'
      + 'background:transparent;border:1px solid '+divBord+';color:'+txt2+';'
...
function tabletCSS(accentColor, theme, typography, publicColors) {
  ...
  const _badgeCSS = '.badge-mote{background:' + (pc.badgeMote||'#3a2a0a') + ';color:' + (pc.badgeMoteT||'#f5c500') + '}'
    + '.badge-kurs{background:' + (pc.badgeKurs||'#0d2035') + ';color:' + (pc.badgeKursT||'#7dc8ff') + '}'
    + '.badge-tur{background:' + (pc.badgeTur||'#1e0d35') + ';color:' + (pc.badgeTurT||'#d8b4fe') + '}'
    + '.badge-stand{background:' + (pc.badgeStand||'#0d2a0d') + ';color:' + (pc.badgeStandT||'#7dff7d') + '}';
  return `<style>
  ...
  <div class="ev-panel active" id="tab-reg">
    <div class="ev-card">
      ...
      <p style="color:#bbb;font-size:.92rem;margin-bottom:1.25rem;line-height:1.6" data-i18n="${lotteryOn ? 'desc_stand_lot' : 'desc_stand'}">
        ${lotteryOn ? T('desc_stand_lot',lang) : T('desc_stand',lang)}
      </p>
...
// ── Debug + auto-fix: compare/repair accessList dept IDs vs settings.departments ──
app.get("/api/debug/dept-access", auth, adminOnly, function(req, res) {
  var targetId = req.query.userId || req.session.user.id;
  var users = readJSON(USERS_FILE);
  var user = users.find(function(u) { return u.id === targetId; });
  if (!user) return res.status(404).json({ error: "not found" });
  var settings = getSettings();
  var depts = settings.departments || [];
  var accessList = getAccessList(user);
  var fixed = false;
  var crossCheck = accessList.map(function(t) {
    var match = depts.find(function(d) { return d.id === t.department; });
    var fallback = null;
    if (!match) {
      fallback = depts.find(function(d) {
        return (d.slug && d.slug === t.department) ||
               (d.name && d.name.toLowerCase() === (t.department || '').toLowerCase());
      });
    }
    return { accessDeptId: t.department, role: t.role, assignmentType: t.assignmentType,
             matchedDept: match ? match.name : null, matchedById: !!match,
             fallbackDept: fallback ? fallback.name : null, fallbackId: fallback ? fallback.id : null };
  });
  // Auto-fix if ?fix=1
  var fixLog = [];
  if (req.query.fix === '1') {
    var newAccessList = accessList.map(function(t) {
      var match = depts.find(function(d) { return d.id === t.department; });
      if (match) return t; // already correct
      var fallback = depts.find(function(d) {
        return (d.slug && d.slug === t.department) ||
               (d.name && d.name.toLowerCase() === (t.department || '').toLowerCase());
      });
      if (fallback) {
        fixLog.push('Fixed: ' + t.department + ' → ' + fallback.id + ' (' + fallback.name + ')');
        return Object.assign({}, t, { department: fallback.id });
      }
      fixLog.push('Could not fix: ' + t.department + ' — no match found');
      return t;
    });
    if (fixLog.length) {
      var userIdx = users.findIndex(function(u) { return u.id === targetId; });
      users[userIdx].accessList = newAccessList;
      writeJSON(USERS_FILE, users);
      fixed = true;
    }
  }
  res.json({ userId: targetId, userName: user.name, crossCheck,
             settingsDepts: depts.map(function(d){ return { id: d.id, name: d.name, slug: d.slug }; }),
             fixed, fixLog });
});
...
<div id="root">
  ...
  <div id="strip">
    ...
    <div id="posLabel">${isLeft ? "◀ Venstre" : "Høyre ▶"}</div>  <!-- updated by JS from state.screenNames -->
  </div>
...
var DEFAULT_NAME = IS_LEFT ? "◀ Venstre" : "Høyre ▶";
...
function connect() {
  ...
  es.onmessage = function(e) {
    try {
      ...
      if (d.type === "state")    { Object.assign(state, d); rebuildPhotos(); updateTicker(); updateScreenName(); }
...
function updateScreenName() {
  var names = state.screenNames || {};
  var custom = names[POSITION];
  var el = document.getElementById("posLabel");
  if (el) el.textContent = custom || DEFAULT_NAME;
  // Also update the strip site name if a center name is set
  var strip = document.getElementById("stripName");
  if (strip && names.siteName) strip.textContent = names.siteName;
}
...
function updateCenterName() {
  var el = document.getElementById("tvSiteName");
  if (!el) return;
  var names = state.screenNames || {};
  // Use custom center name if set, otherwise fall back to site name
  if (names.center) el.textContent = names.center;
}
...
function evTypeStyle(type) {
  ...
  if (state.colors) {
    var c = state.colors;
    var fbMap = {
      kurs:  { bg: c.kursBg  || defaults.kurs.bg,  color: c.kursText  || defaults.kurs.color,  border: c.kursBorder  || defaults.kurs.border },
      tur:   { bg: c.turBg   || defaults.tur.bg,   color: c.turText   || defaults.tur.color,   border: c.turBorder   || defaults.tur.border },
      stand: { bg: c.standBg || defaults.stand.bg, color: c.standText || defaults.stand.color, border: c.standBorder || defaults.stand.border },
