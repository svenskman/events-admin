...
app.use(function(req, res, next) {
  ...
  if (host === tvDomain) {
    // tv.naf-events.no → redirect to a dept picker or default dept
    var tvSettings = getSettings();
    var depts = tvSettings.departments || [];
    if (depts.length === 1) {
      return res.redirect("/tv/" + depts[0].id);
    }
    // Show dept picker
    ...
    return res.send("<!DOCTYPE html><html><head><meta charset='UTF-8'/><meta name='viewport' content='width=device-width,initial-scale=1'/><title>" + (tvSettings.siteName||"TV") + "</title>"
      ...
      + depts.map(function(d){ return "<a href='/tv/" + escHtml(d.id) + "'>" + escHtml(d.displayName||d.name) + "</a>"; }).join("")
      + "</body></html>");
  }
...
app.put("/api/settings", auth, adminOnly, function(req, res) {
  ...
  const allowed = ["siteName","eventDomain","contactEmail","gdprRetentionDays",
    ...
    "regFields","orgLevels","orgRoles","departments","emailTemplates","typeNames","roleNames"];
...
// ── Overview page (naf-events.no / www.naf-events.no) ───────────────
...
app.get("/api/tv/:deptId/stream", function(req, res) {
  ...
  const upcoming = events
    ...
    .map(function(e) { return { id: e.id, title: e.title, date: e.date, location: e.location, eventType: e.eventType, image: e.image || null }; });
...
app.post("/api/tv/:deptId/pptx", auth, managerOrAdmin,
  ...
  async function(req, res) {
    ...
    const deptId = req.params.deptId;
    const pptxPath = req.file.path;
    const workDir = path.join(os.tmpdir(), "tv_pptx_" + Date.now());
    fs.mkdirSync(workDir, { recursive: true });
    function cleanup() {
      try { fs.rmSync(workDir, { recursive: true, force: true }); } catch(e) {}
      try { fs.unlinkSync(pptxPath); } catch(e) {}
    ...
    try {
      // Convert PPTX → PNG using LibreOffice
      execSync(
        "python3 /mnt/skills/public/pptx/scripts/office/soffice.py --headless --convert-to png --outdir " + JSON.stringify(workDir) + " " + JSON.stringify(pptxPath),
        { timeout: 180000, stdio: "pipe" }
      );
    } catch(e) {
      // Fallback: use soffice directly
      try {
        execSync("soffice --headless --convert-to png --outdir " + JSON.stringify(workDir) + " " + JSON.stringify(pptxPath),
          { timeout: 180000, stdio: "pipe" });
      } catch(e2) {
        cleanup();
        return res.status(500).json({ error: "PPTX-konvertering feilet: " + e2.message });
      }
    }
    // Find generated PNG files
    const pngs = fs.readdirSync(workDir).filter(f => f.match(/\.png$/i)).sort();
    if (!pngs.length) { cleanup(); return res.status(500).json({ error: "Ingen bilder ble generert fra PPTX" }); }
    // Copy to uploads folder
    const newSlides = [];
    pngs.forEach(function(png) {
      const src = path.join(workDir, png);
      const dest = path.join(UPLOADS, "tv_" + deptId + "_" + Date.now() + "_" + png);
      fs.copyFileSync(src, dest);
...
/* EVENT CARD LAYER */
#eventLayer{position:absolute;inset:0;background:linear-gradient(135deg,#0d0d0d 0%,#111 100%);opacity:0;transition:opacity .8s ease;display:flex;flex-direction:column;justify-content:center;padding:8vh 8vw}
...
/* EVENT CARD */
.ev-card-tv{display:flex;gap:4vw;align-items:flex-start;margin-bottom:5vh;opacity:0;transform:translateY(30px);transition:opacity .5s,transform .5s}
.ev-card-tv.visible{opacity:1;transform:translateY(0)}
.ev-thumb{width:180px;height:120px;object-fit:cover;border-radius:12px;flex-shrink:0;border:2px solid rgba(255,255,255,.1)}
.ev-thumb-placeholder{width:180px;height:120px;border-radius:12px;flex-shrink:0;display:flex;align-items:center;justify-content:center;font-size:2.5rem;border:2px solid rgba(255,255,255,.08)}
.ev-info{}
...
.ev-title-tv{font-size:clamp(1.4rem,3vw,2.2rem);font-weight:900;line-height:1.2;margin-bottom:.5rem}
.ev-meta-tv{font-size:clamp(.85rem,1.5vw,1.1rem);color:#888;display:flex;gap:1.5rem;flex-wrap:wrap}
...
<div id="screen">
  <!-- Header -->
  <div id="tvHeader">
    ${logo ? `<img id="tvLogo" src="${esc(logo)}" alt="${esc(siteName)}"/>` : ""}
...
es.onmessage = function(e) {
  try { applyState(JSON.parse(e.data)); } catch(x) {}
};
es.onerror = function() {
  setTimeout(function() {
    es = new EventSource("/api/tv/" + DEPT_ID + "/stream");
    es.onmessage = function(e) { try { applyState(JSON.parse(e.data)); } catch(x) {} };
  }, 3000);
};

// ── State ─────────────────────────────────────────────────────────
function applyState(s) {
  if (s.type === "state") Object.assign(state, s);
  updateTicker();
  updateDots();
  if (!timer) startCycle();
}
...
function step() {
  ...
  var showEventsNow = state.showEvents && upcoming.length > 0 && state.mode === "events";
  var interval = (state.slideInterval || 15) * 1000;

  // Decide: show slide or event card
  if (showEventsNow && !showingEvents && slides.length > 0 && slideIdx > 0 && slideIdx % 3 === 0) {
    // Every 3 slides, show event cards
    showEventsNow = true;
  } else if (!slides.length && showEventsNow) {
    showEventsNow = true;
  } else if (slides.length > 0) {
    showEventsNow = false;
  }

  if (showEventsNow && !showingEvents) {
    showingEvents = true;
    showEventCards(upcoming);
    startProgress(interval / 1000);
    timer = setTimeout(function() { showingEvents = false; step(); }, interval);
  } else {
    showingEvents = false;
    if (slides.length) {
      showSlide(slideIdx);
...
function showEventCards(upcoming) {
  ...
  var count = Math.min(upcoming.length, state.eventCount || 3, 3);
