...
let displayNextId = 1;
const displayState  = new Map(); // evId → { mode, winnerName, prize, slide, slides[], ticker }
...
app.post("/api/events/:id/lottery/draw", auth, managerOrAdmin, function(req, res) {
  ...
  // Send vinner-e-post med QR-kode
  if (winner.email && !winner.anonymized) {
    const _settings = getSettings();
    const _siteName = _settings.siteName || "Events Admin";
    const _qrData   = winnerToken;
    const _qrUrl    = "https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=" + encodeURIComponent(_qrData);
    const _prize    = ev.lottery.prize ? "<p>🏆 Premie: <strong>" + escHtml(ev.lottery.prize) + "</strong></p>" : "";
    const _subject  = "🎉 Du vant! – " + escHtml(ev.title);
    const _html     = `<div style="font-family:sans-serif;max-width:520px;margin:0 auto;color:#1a1a1a">
  <div style="background:#FFD100;padding:1.25rem 1.5rem;border-radius:8px 8px 0 0">
    <h1 style="margin:0;font-size:1.3rem;color:#1a1a1a">${escHtml(_siteName)}</h1>
  </div>
  <div style="background:#f9f9f9;padding:2rem;border-radius:0 0 8px 8px;border:1px solid #e0e0e0;text-align:center">
    <h2 style="margin-top:0">🎉 Gratulerer, ${escHtml(winner.name)}!</h2>
    ...
    <img src="${_qrUrl}" alt="Vinner-QR" style="display:block;margin:1.5rem auto;width:200px;height:200px;border:4px solid #FFD100;border-radius:8px"/>
    <p style="font-size:.82rem;color:#555;margin-top:1rem">Hvis QR-koden ikke vises, bruk denne teksten i stedet:</p>
    <div style="font-family:monospace;font-size:.7rem;background:#f0f0f0;border:1px solid #ccc;border-radius:6px;padding:.75rem;word-break:break-all;color:#333;margin:.5rem auto;max-width:420px">${winnerToken}</div>
    <p style="font-size:.75rem;color:#999;margin-top:1rem">Koden er personlig og kan kun brukes én gang. Ikke del den med andre.</p>
  </div>
</div>`;
    const _text = "Gratulerer, " + winner.name + "! Du vant i lodtrekningen på " + ev.title + (ev.lottery.prize ? " – Premie: " + ev.lottery.prize : "") + ". Vis QR-koden til arrangøren for å bekrefte gevinsten.";
    sendEmail({ to: winner.email, subject: _subject, html: _html, text: _text }).catch(function(e) { console.error("[lottery] E-post feil:", e && e.message); });
  }
...
app.get("/api/events/:id/display/stream", function(req, res) {
  ...
  res.write("data: " + JSON.stringify(Object.assign({ type: "state" }, state)) + "\n\n");
...
// Upload and convert PPTX to PNG slides
app.post("/api/events/:id/display/pptx", auth, managerOrAdmin,
  multer({ dest: UPLOADS, limits: { fileSize: 50 * 1024 * 1024 },
    fileFilter: function(req, file, cb) {
      const ok = [
  ...
  async function(req, res) {
    ...
    execFile("libreoffice", [
      ...
      "--outdir", workDir, pptxPath
    ], { timeout: 60000 }, function(err, stdout, stderr) {
      if (err) { cleanup(); return res.status(500).json({ error: "LibreOffice-konvertering feilet: " + (err.message||"ukjent feil") }); }

      const pdfFiles = fs.readdirSync(workDir).filter(function(f) { return f.endsWith(".pdf"); });
      if (!pdfFiles.length) { cleanup(); return res.status(500).json({ error: "Ingen PDF generert av LibreOffice" }); }
      const pdfPath = path.join(workDir, pdfFiles[0]);

      // Step 2: PDF → PNG via pdftoppm (150 dpi, 16:9 friendly)
      const slidePrefix = path.join(workDir, "slide");
      execFile("pdftoppm", [
        "-r", "150", "-png", pdfPath, slidePrefix
      ], { timeout: 120000 }, function(err2) {
        if (err2) { cleanup(); return res.status(500).json({ error: "pdftoppm feilet: " + (err2.message||"ukjent feil") }); }

        const pngFiles = fs.readdirSync(workDir)
          .filter(function(f) { return f.match(/slide-\d+\.png$/); })
          .sort();

        if (!pngFiles.length) { cleanup(); return res.status(500).json({ error: "Ingen slides generert" }); }

        // Copy PNGs to UPLOADS dir
        const evId  = req.params.id;
        const saved = [];
        pngFiles.forEach(function(f) {
          const src  = path.join(workDir, f);
          const dest = path.join(UPLOADS, "slide_" + evId + "_" + Date.now() + "_" + f);
          fs.copyFileSync(src, dest);
          saved.push("/uploads/" + path.basename(dest));
        });
        ...
        const prev   = displayState.get(evId) || { mode: "slides", slide: 0, slides: [], ticker: "" };
        const slides = (prev.slides || []).concat(saved);
        const next   = Object.assign({}, prev, { slides, slide: 0 });
        displayState.set(evId, next);
        broadcastDisplay(evId, Object.assign({ type: "state" }, next));

        res.json({ ok: true, slides, count: saved.length });
      });
    });
  }
);
...
<style>
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  #slideImg{max-width:100%;max-height:100%;object-fit:contain;transition:opacity .4s}
  #slideEmpty{color:#333;font-size:1.5rem;text-align:center;padding:2rem}
  #slideNav{position:absolute;bottom:1rem;left:50%;transform:translateX(-50%);display:flex;gap:.5rem}
  .sdot{width:10px;height:10px;border-radius:50%;background:#333;transition:background .2s}
  .sdot.active{background:${accent}}

  /* WINNER MODE */
  #winnerWrap{position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;padding:2rem;opacity:0;transition:opacity .5s}
  #winnerWrap.show{opacity:1}
  #winnerEmoji{font-size:8rem;animation:bounce 1s ease infinite alternate}
  @keyframes bounce{from{transform:scale(1)}to{transform:scale(1.12)}}
  #winnerName{font-size:clamp(2rem,7vw,5.5rem);font-weight:900;color:${accent};margin:.5rem 0;line-height:1.1;text-shadow:0 0 40px ${accent}88}
  #winnerPrize{font-size:clamp(1rem,3vw,2rem);color:#ccc;margin-top:.75rem}
  #winnerTag{font-size:1rem;color:#555;margin-top:1.5rem;letter-spacing:.1em;text-transform:uppercase}

  /* TICKER */
  #ticker{position:absolute;bottom:0;left:0;right:0;background:rgba(0,0,0,.75);padding:.4rem 0;overflow:hidden;height:2.4rem;display:none}
  #tickerInner{white-space:nowrap;display:inline-block;animation:scroll 18s linear infinite;font-size:.95rem;color:#ccc;padding-left:100%}
  @keyframes scroll{from{transform:translateX(0)}to{transform:translateX(-100%)}}
  #tickerInner.paused{animation:none}

  /* CONFETTI */
  .confetti{position:absolute;width:10px;height:14px;border-radius:2px;opacity:0;animation:fall 3s ease-in forwards}
...
<div id="screen">
  <div id="slideWrap">
    ...
    <img id="slideImg" style="display:none"/>
    <div id="slideNav"></div>
  </div>
  <div id="winnerWrap">
    <div id="winnerEmoji">🏆</div>
    <div style="font-size:1.2rem;color:#888;margin-bottom:.5rem;letter-spacing:.08em;text-transform:uppercase">Vinner</div>
    <div id="winnerName">—</div>
    <div id="winnerPrize"></div>
    <div id="winnerTag">${escHtml(siteName)}</div>
  </div>
  <div id="ticker"><span id="tickerInner"></span></div>
</div>
<script>
var evId = "${escHtml(evId)}";
var state = { mode:"slides", slide:0, slides:[], ticker:"", winnerName:"", prize:"" };
var slideTimer = null;

function applyState(s) {
  if (s.slides)      state.slides      = s.slides;
  if (s.mode)        state.mode        = s.mode;
  if (s.slide !== undefined) state.slide = s.slide;
...
function render() {
  ...
  if (state.mode === "winner") {
    ...
    setTimeout(function(){ winnerWrap.classList.add("show"); }, 30);
    ...
    if (state.winnerName) spawnConfetti();
  } else {
  ...
  if (state.ticker) {
    ...
    tickerInner.textContent = state.ticker + "   •   " + state.ticker;
    tickerInner.style.animationDuration = Math.max(10, state.ticker.length * .3) + "s";
  } else {
...
function renderSlides() {
  ...
  img.style.opacity = 0;
  setTimeout(function(){
    img.src = state.slides[idx];
    img.style.opacity = 1;
  }, 200);
  // Dots
  nav.innerHTML = state.slides.map(function(_, i){
    return "<div class=\"sdot" + (i===idx?" active":"") + "\"></div>";
  }).join("");
}

function spawnConfetti() {
  var colors = ["#FFD100","#f87171","#4ade80","#60a5fa","#c084fc","#fb923c"];
  for (var i = 0; i < 80; i++) {
    (function(i){
      setTimeout(function(){
...
es.onerror = function() {
  setTimeout(function(){ es = new EventSource("/api/events/" + evId + "/display/stream"); }, 3000);
};

// Auto-advance slides every 8 seconds in slides mode
setInterval(function(){
  if (state.mode === "slides" && state.slides && state.slides.length > 1) {
