...
<div class="ev-content">
  ...
  <!-- Navigeringsknapp – fast i bunn, vises kun på etappeplan-fanen -->
  ${days.length ? `
  <div id="navFab" style="display:none;position:fixed;bottom:0;left:0;right:0;z-index:160;padding:.75rem 1rem calc(.75rem + env(safe-area-inset-bottom));background:linear-gradient(transparent,var(--b) 35%);pointer-events:none">
    <a id="navFabBtn" href="#" onclick="openGoogleMapsNav();return false;"
      style="pointer-events:all;display:flex;align-items:center;justify-content:center;gap:.6rem;width:100%;padding:16px;background:var(--y);color:#111;font-weight:900;font-size:1rem;border-radius:12px;text-decoration:none;box-shadow:0 4px 20px rgba(0,0,0,.5);min-height:56px">
...
function openGoogleMapsNav(){
  ...
  // Google Maps: origin=start, destination=slutt, waypoints=mellom
...
// Vis/skjul navigeringsknapp basert på aktiv fane
(function(){
  var _orig=showTab;
  showTab=function(id){
    _orig(id);
    var fab=document.getElementById('navFab');
    if(fab)fab.style.display=(id==='rute')?'block':'none';
  };
})();
