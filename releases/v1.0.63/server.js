...
function showEventCards(upcoming, activeIdx) {
  ...
  cards.innerHTML = upcoming.slice(0, count).map(function(ev, i) {
    ...
    // Thumb ring for ongoing
    var thumbStyle = ongoing
      ? 'style="box-shadow:0 0 0 3px ' + ACCENT + ';border-radius:8px"'
      : '';
    var thumbWrapped = ev.image
      ? '<img class="ev-thumb" src="' + esc(ev.image) + '" ' + thumbStyle + ' style="' + (ongoing ? 'border-color:' + ACCENT + ';box-shadow:0 0 0 3px ' + ACCENT : '') + '"/>'
      : '<div class="ev-thumb-placeholder" style="background:' + style.bg + ';' + (ongoing ? 'box-shadow:0 0 0 3px ' + ACCENT : '') + '">'
