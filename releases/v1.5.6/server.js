    ...
    try {
      ...
      while ((trkseg = trksegRe.exec(xml)) !== null) {
        ...
        while ((pt = ptRe.exec(trkseg[1])) !== null) {
          const nameM = pt[3].match(/<name>([^<]*)<\/name>/);
          trkpts.push({ lat: parseFloat(pt[1]), lon: parseFloat(pt[2]), name: nameM ? nameM[1].trim() : null });
        }
      }
      ...
      while ((wpt = wptRe.exec(xml)) !== null) {
        const nameM = wpt[3].match(/<name>([^<]*)<\/name>/);
        wpts.push({ lat: parseFloat(wpt[1]), lon: parseFloat(wpt[2]), name: nameM ? nameM[1].trim() : "Waypoint" });
      }

      // Route points
      const rteptRe = /<rtept\s+lat="([^"]+)"\s+lon="([^"]+)"[^>]*>([\s\S]*?)<\/rtept>/g;
      let rtept;
      while ((rtept = rteptRe.exec(xml)) !== null) {
        const nameM = rtept[3].match(/<name>([^<]*)<\/name>/);
        rtepts.push({ lat: parseFloat(rtept[1]), lon: parseFloat(rtept[2]), name: nameM ? nameM[1].trim() : null });
      }

      // Bygg etapper fra waypoints/routepoints eller thinned trackpoints
      var stops = wpts.length ? wpts : rtepts.length ? rtepts : [];

      // Hvis ingen waypoints, thin trackpoints til maks 20 punkter
      if (!stops.length && trkpts.length) {
        var step = Math.max(1, Math.floor(trkpts.length / 20));
        for (var i = 0; i < trkpts.length; i += step) stops.push(trkpts[i]);
        stops.push(trkpts[trkpts.length - 1]); // alltid ta med siste punkt
      }
