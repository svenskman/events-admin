...
app.get("/api/events/:id/routecoords", auth, rateLimit(10, 60000), async function(req, res) {
  ...
  async function geocode(name) {
    ...
    try {
      const r = await fetch("https://nominatim.openstreetmap.org/search?format=json&limit=1&q=" + encodeURIComponent(name), { headers: { "User-Agent": "EventsAdmin/1.0" } });
      const d = await r.json();
      if (d && d[0]) { global._geocodeCache[name] = { lat: parseFloat(d[0].lat), lon: parseFloat(d[0].lon) }; }
    } catch(e) {}
  ...
  const coordCache = {};
  for (var i = 0; i < allNames.length; i++) {
    const g = await geocode(allNames[i]);
    if (g) coordCache[allNames[i]] = g;
  ...
  // Waypoints (start, slutt og overnattinger)
  const waypoints = [];
  days.forEach(function(d, di) {
    (d.etapper||[]).forEach(function(e) {
      if (e.type === "start" && coordCache[e.fra]) waypoints.push({...coordCache[e.fra], type:"start", name:e.fra});
      if (e.type === "slutt" && coordCache[e.til]) waypoints.push({...coordCache[e.til], type:"end",   name:e.til});
      if (e.type === "hotell" && coordCache[e.til]) waypoints.push({...coordCache[e.til], type:"hotel", name:e.til});
    });
  });

  res.json({ coords: allCoords, waypoints, coordCache });
});
