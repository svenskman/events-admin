...
.ev-thumb{width:clamp(60px,9vw,120px);height:clamp(42px,6.2vw,82px);object-fit:cover;border-radius:8px;flex-shrink:0;border:2px solid rgba(255,255,255,.1)}
.ev-thumb-placeholder{width:clamp(60px,9vw,120px);height:clamp(42px,6.2vw,82px);border-radius:8px;flex-shrink:0;display:flex;align-items:center;justify-content:center;font-size:1.5rem;border:2px solid rgba(255,255,255,.08)}
.ev-info{}
...
.ev-card-tv:not(.active-ev){opacity:.35}
...
#evDetailContent{position:relative;z-index:1;display:flex;flex-direction:column;gap:2vh}
#evDetailImg{width:100%;max-height:26vh;object-fit:cover;border-radius:12px;border:2px solid rgba(255,255,255,.12);display:none}
...
function showEventCards(upcoming) {
  ...
  // ── Left side: render original-style cards (all visible) ──────
  cards.innerHTML = upcoming.slice(0, count).map(function(ev, i) {
    var style   = evTypeStyle(ev.eventType);
    var ongoing = isOngoing(ev);
    var thumb   = ev.image
      ? '<img class="ev-thumb" src="' + esc(ev.image) + '" style="' + (ongoing ? 'border-color:' + ACCENT : '') + '"/>'
      : '<div class="ev-thumb-placeholder" style="background:' + style.bg + '">'
          + (ev.eventType==="stand"?"🟢":ev.eventType==="kurs"?"🔵":ev.eventType==="tur"?"🟣":"🟡")
        + '</div>';
    var ongoingBadge = ongoing
      ? '<span style="background:' + ACCENT + ';color:' + ACCENT_TEXT + ';font-size:.58rem;font-weight:800;padding:1px 7px;border-radius:8px;text-transform:uppercase;letter-spacing:.4px;margin-left:.4rem">Pågår nå</span>'
      : '';
    return '<div class="ev-card-tv" id="evc_' + i + '">'
      + thumb
      + '<div class="ev-info">'
      + '<div><span class="ev-type-badge" style="background:' + style.bg + ';color:' + style.color + ';border:1px solid ' + style.border + '">' + evTypeLabel(ev.eventType) + '</span>' + ongoingBadge + '</div>'
  ...
  // ── Right side: cycle through each event detail ────────────────
  clearInterval(_evCycleTimer);
  _evCurrentIdx = 0;
  _showEvDetail(upcoming, 0, count);

  var perEvMs = Math.max(3000, ((state.slideInterval || 15) * 1000) / Math.max(1, count));
  _evCycleTimer = setInterval(function() {
    _evCurrentIdx = (_evCurrentIdx + 1) % count;
    _showEvDetail(upcoming, _evCurrentIdx, count);
  }, perEvMs);
  } catch(showErr) {
    console.error("showEventCards error:", showErr);
  }
}
...
function _showEvDetail(upcoming, idx, count) {
  ...
  var img = document.getElementById('evDetailImg');
  if (img) {
  ...
  if (metaEl) {
    ...
    if (ev.date)     addRow('📅', fmtDate(ev.date));
    if (ev.location) addRow('📍', ev.location);
    ...
    if (ev.routeSummary) {
      ...
      if (routeParts.length) addRow('🗺️', routeParts.join(' · '));
