...
app.get("/api/stats/visits", auth, function(req, res) {
  const me = req.session.user;
  if (!me.permissions.isManager && !me.permissions.isAdmin) {
  ...
  // Filter to this admin's events if not global admin
  const allowedEvIds = me.permissions.isAdmin
    ? null
    : new Set(events.filter(function(e) {
        return e.department === me.permissions.myDepartment || e.createdBy === me.name;
      }).map(function(e) { return e.id; }));

  let visits = allVisits.filter(function(v) {
  ...
  const sparkDays = 14;
  const sparkSince = new Date(Date.now() - sparkDays * 86400000).toISOString();
  const sparkByEvent = {};
  ...
  const topEvents = Object.entries(byEvent)
    .map(function(e) {
      const ev = eventMap[e[0]] || {};
      return { evId: e[0], title: ev.title || e[0], count: e[1],
               eventType: ev.eventType || 'stand', date: ev.date || null,
...
<nav class="ev-tabs">
  ...
  <div class="ev-tab" data-tab="gb" onclick="showTab('gb')" data-i18n="tab_gb">${T('tab_gb',lang)}</div>
</nav>
...
<nav class="ev-tabs">
  ...
  <div class="ev-tab" data-tab="oppmote" onclick="showTab('oppmote')" data-i18n="tab_oppmote">✅ Oppmøte</div>
  <div class="ev-tab" data-tab="gb" onclick="showTab('gb')">${T('tab_gb',lang)}</div>
</nav>
