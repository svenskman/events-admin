...
app.put("/api/departments/:id/appearance", auth, function(req, res) {
  ...
  if (idx < 0) return res.status(404).json({ error: "Avdeling ikke funnet" });

  // Allow admin or department manager of this dept
  if (u.role !== "admin") {
    ...
    if (!hasAccess) return res.status(403).json({ error: "Ingen tilgang" });
  }

  const app = req.body || {};
  depts[idx].appearance = {
    accent:  (app.accent  || "").trim() || null,
    theme:   app.theme === "light" ? "light" : "dark",
    logo:    (app.logo    || "").trim() || null,
    palette: Array.isArray(app.palette) ? app.palette.slice(0, 8) : [],
  };
  s.departments = depts;
  saveSettings(s);
  ...
  if (inventarPool.length > 0) {
    ...
    if (pool.length === 0) {
      ...
      if (ev.lottery.prize) {
        ...
        return res.status(400).json({ error: "Ingen premier igjen i inventar-poolen. Fyll på lageret i Inventar eller legg til en fritekst-premie." });
      }
    } else {
...
function getDeptAppearance(ev) {
  ...
  if (!dept || !dept.appearance) {
    return { accent: globalAccent, theme: globalTheme, logo: globalLogo, palette: (s.colors && s.colors.palette) || [] };
  }
  return {
    accent:  dept.appearance.accent  || globalAccent,
    theme:   dept.appearance.theme   || globalTheme,
    logo:    dept.appearance.logo    || globalLogo,
    palette: dept.appearance.palette || (s.colors && s.colors.palette) || [],
  };
}
...
.ev-form input,.ev-form textarea,.ev-form select{width:100%;padding:14px;background:#fff;border:2px solid #bbb;border-radius:8px;color:#111;font-size:1rem;font-family:inherit;transition:border-color .15s;-webkit-appearance:none}
...
<!-- Tabs -->
<nav class="ev-tabs">
  <div class="ev-tab" data-tab="info" onclick="showTab('info')">ℹ️ Om standen</div>
  <div class="ev-tab active" data-tab="reg" onclick="showTab('reg')">📧 Hold meg oppdatert</div>
...
app.get("/m", auth, function(req, res) {
  ...
  function evCard(ev) {
    ...
    html += "<div class=\"section\"><div class=\"section-head\">&#128269; Verifiser vinner-QR</div><div class=\"section-body\">"
      ...
      + "<button class=\"btn btn-ghost\" onclick=\"toggleSearch('" + id + "')\">&#128100; S&oslash;k p&aring; navn</button>"
      + "</div>"
