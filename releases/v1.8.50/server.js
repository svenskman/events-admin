...
async function catchUpToTracker() {
  ...
  try {
    ...
    var d = await r.json();
    if (d.ok && d.lat && d.lon) {
      window.location.href = 'https://www.google.com/maps/dir/?api=1&destination=' + d.lat + ',' + d.lon + '&travelmode=driving';
      return;
    }
  } catch(e) {}
  // Fallback: open tracker page so user can get position themselves
  if (LIVE_TRACKING_URL) { window.open(LIVE_TRACKING_URL, '_blank'); }
