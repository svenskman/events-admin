...
function emailTurFinalized(ev, reg, settings) {
  ...
  const stageHtml = days.length ? days.map(function(day, di) {
    const stages = (day.stages || []).filter(function(e){ return e.fra || e.til; });
  ...
  const html = "<div style=\"font-family:sans-serif;max-width:600px;margin:0 auto;color:#1a1a1a\">"
    ...
    + hotelLine + "</div>"
    + (etappeHtml ? "<h2 style=\"font-size:1.1rem;font-weight:700;margin:1.5rem 0 .25rem\">🗺️ Reiserute</h2>" + stageHtml: "")
  ...
  function evCard(ev) {
    ...
    if (ev.eventType === "tur" && ev.route && ev.route.days && ev.route.days.length) {
      ...
      var dayRows = days.map(function(d, di) {
        ...
        var opplevelser = stops.filter(function(s){ return s.type === "opplevelse"; }).map(function(s){ return (OPPL_ICONS[s.opplevelseSubtype] || "🎯") + " " + (s.note || s.til || ""); }).filter(Boolean);
...
function buildTurPage(ev, isAuthenticated) {
  ...
  const stageHtml = (function() {
    if (!(days||[]).some(function(d){ return (d.stages||[]).some(function(e){ return e.fra||e.til; }); }))
    ...
    return totalMapHtml + (days||[]).map(function(day, di) {
      var stages = (day.stages||[]).filter(function(e){ return e.fra||e.til; });
      if (!etapper.length) return '';
    ...
    ${(function(){
      ...
      if (days.length > 1) {
        days.forEach(function(d, di) {
          var fra = (d.stages && d.stages[0] && d.stages[0].fra) || '';
          var lastE = d.stages && d.stages[d.stages.length - 1];
          var til = (lastE && lastE.til) || '';
...
app.post("/api/events/:id/utstyr", auth, managerOrAdmin, function(req, res) {
  const events = readJSON(EVENTS_FILE);
  const idx    = events.findIndex(function(e){ return e.id === req.params.id; });
  if (idx === -1) return res.status(404).json({ err: "Arrangement ikke funnet" });
  const items  = readJSON(INVENTORY_FILE);
  const item   = items.find(function(i){ return i.id === req.body.itemId; });
  if (!item) return res.status(404).json({ err: "Utstyr ikke funnet" });
  const quantity = parseInt(req.body.quantity) || 1;
  if (!events[idx].utstyr) events[idx].utstyr = [];
  // If already on event, increment quantity
  const existing = events[idx].utstyr.find(function(u){ return u.id === item.id; });
  if (existing) {
    existing.quantity = (existing.quantity || 1) + antall;
  } else {
    events[idx].utstyr.push({
      id: item.id,
      name: item.name,
      quantity: antall,
      category: item.category || "annet"
  ...
  writeJSON(EVENTS_FILE, events);
  res.json({ ok: true, utstyr: events[idx].utstyr });
});
...
app.post("/api/inventory/:id/stock-count", auth, managerOrAdmin, function(req, res) {
  ...
  items[idx].stockCountLog.push({ fra: gammelt, til: nyttAntall, dato: new Date().toISOString(), note: (req.body.note||"").trim() });
  writeJSON(INVENTORY_FILE, items);
...
app.post("/api/events/:id/utstyr", auth, managerOrAdmin, function(req, res) {
  ...
  if (aktiv) {
    if (existing >= 0) events[evIdx].utstyr[existing].quantity = parseInt(antall)||1;
    else events[evIdx].utstyr.push({ id: itemId, quantity: parseInt(antall)||1 });
  } else {
...
app.get("/api/public/events/:id/gpx", rateLimit(10, 60000), async function(req, res) {
  ...
  const allNames = [];
  days.forEach(function(d){ (d.stages||[]).forEach(function(e){ if(e.fra&&!allNames.includes(e.fra))allNames.push(e.fra); if(e.til&&!allNames.includes(e.til))allNames.push(e.til); }); });
  ...
  const segCache = {};
  for (var di=0;di<days.length;di++) { var stages=days[di].stages||[]; for(var ei=0;ei<etapper.length;ei++){ var e=etapper[ei]; var fra=coordCache[e.fra],til=coordCache[e.til]; if(!fra||!til)continue; var key=e.fra+"||"+e.til; if(!segCache[key]){segCache[key]=await fetchSegment(fra,til);await sleep(200);} } }
  ...
  function buildGpx(title, dayList) {
    ...
    var seenWpt={};
    dayList.forEach(function(d,dIdx){ (d.stages||[]).forEach(function(e,eIdx){ [[e.fra,eIdx===0&&dIdx===0?"start":"wpt"],[e.til,e.type==="slutt"?"end":e.type==="hotell"?"hotel":"wpt"]].forEach(function(pair){ var nm=pair[0],wt=pair[1]; if(!nm||seenWpt[nm])return; seenWpt[nm]=true; var c=coordCache[nm];if(!c)return; var sym=wt==="start"?"Flag, Green":wt==="end"?"Flag, Red":wt==="hotel"?"Lodging":"Waypoint"; gpx+='  <wpt lat="'+c.lat.toFixed(6)+'" lon="'+c.lon.toFixed(6)+'">\n    <name>'+xmlEsc(nm)+'</name><sym>'+sym+'</sym>\n  </wpt>\n'; }); }); });
    dayList.forEach(function(d){ (d.stages||[]).forEach(function(e,eIdx){ var fra=coordCache[e.fra],til=coordCache[e.til];if(!fra||!til)return; var key=e.fra+"||"+e.til; var pts=segCache[key]||[{lat:fra.lat,lon:fra.lon},{lat:til.lat,lon:til.lon}]; gpx+='  <trk><name>'+xmlEsc(title+" – etappe "+(eIdx+1))+'</name><trkseg>\n'; pts.forEach(function(pt){gpx+='    <trkpt lat="'+pt.lat.toFixed(6)+'" lon="'+pt.lon.toFixed(6)+'"/>\n';}); gpx+='  </trkseg></trk>\n'; }); });
    gpx+='</gpx>';
    return gpx;
  }
  ...
  if (MOCK_AI) {
    ...
    for (let i = 0; i < days; i++) {
      ...
      mockDays.push({ dato, stages: [
        { type:"start",      fra: start, til: start + " sentrum",  km: Math.round(distanceKm*0.1),  profile: usedProfile, note:"Avreise" },
        { type:"opplevelse", fra: start + " sentrum", til: "Utsiktspunkt", km: Math.round(distanceKm*0.2), profile: usedProfile, note:"Spektakulær utsikt" },
        { type:"lunsj",      fra: "Utsiktspunkt", til: "Kafé",     km: Math.round(distanceKm*0.2),  profile: usedProfile, note:"Anbefalt kafé" },
        { type:"opplevelse", fra: "Kafé", til: "Historisk sted",   km: Math.round(distanceKm*0.15), profile: usedProfile, note:"Historisk attraksjon" },
        ...(isMultiday && i < days-1
          ? [{ type:"hotell", fra: "Historisk sted", til: "Hotellby dag "+(i+1), km: Math.round(distanceKm*0.15), profile: usedProfile, note:"Overnatting" }]
          : [{ type:"slutt",  fra: "Historisk sted", til: roundtrip ? start : "Destinasjon", km: Math.round(distanceKm*0.15), profile: usedProfile, note:"" }]),
      ]});
    }
