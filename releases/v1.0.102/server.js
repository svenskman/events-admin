...
app.put("/api/events/:id", auth, managerOrAdmin, upload.fields([{name:"image",maxCount:1},{name:"tvImageTop",maxCount:1},{name:"tvImageBottom",maxCount:1}]), function(req, res) {
  ...
  if (req.body.slots !== undefined) {
    try {
      var rawSlots = JSON.parse(req.body.slots || "[]");
      if (Array.isArray(rawSlots)) {
        events[i].slots = rawSlots.filter(function(s){ return s && s.label; }).map(function(s){
          var _ipool = Array.isArray(s.inventarPool) ? s.inventarPool : [];
          return {
            id:          s.id || require("crypto").randomBytes(8).toString("hex"),
            label:       sanitizeInput((s.label || "").slice(0, 100)),
            location:    sanitizeInput((s.location || "").slice(0, 200)),
            date:        /^\d{4}-\d{2}-\d{2}$/.test(s.date||"") ? s.date : null,
            time:        /^\d{2}:\d{2}$/.test(s.time||"") ? s.time : null,
            capacity:    Math.max(1, Math.min(9999, parseInt(s.capacity) || 10)),
            inventarPool: _ipool.filter(function(p){ return p && p.id; }).map(function(p){
...
app.post("/api/events/:id/lottery/register", rateLimit(10, 60000), function(req, res) {
  ...
  // Deduct inventar for slot inventarPool
  if (_slotId && ev.slots) {
    var _slotDef = ev.slots.find(function(s){ return s.id === _slotId; });
    var _pool = _slotDef && _slotDef.inventarPool;
    if (_pool && _pool.length) {
      var invItems = readJSON(INVENTAR_FILE);
      _pool.forEach(function(p) {
...
app.post("/api/events/:id/register", rateLimit(10, 60000), function(req, res) {
  ...
  if (!ev) return res.status(404).json({ error: "Event not found" });
  if (!ev.registrations) ev.registrations = [];
  // Slot-based capacity check
  var _slotId = (req.body.slotId || "").trim() || null;
  if (ev.slots && ev.slots.length > 0) {
    if (!_slotId) return res.status(400).json({ error: "Velg et sted/tidspunkt for påmeldingen" });
    var _slot = ev.slots.find(function(s){ return s.id === _slotId; });
    if (!_slot) return res.status(400).json({ error: "Ugyldig sted valgt" });
    var _slotRegs = ev.registrations.filter(function(r){ return r.slotId === _slotId && !r.anonymized; });
    if (_slotRegs.length >= _slot.capacity)
      return res.status(400).json({ error: "Dette stedet er fullt" });
  } else {
    if (ev.maxParticipants && ev.registrations.filter(function(r){return !r.anonymized;}).length >= ev.maxParticipants)
      return res.status(400).json({ error: "Full – no available spots" });
  }
  if (ev.registrations.find(function(r) { return r.email === email.toLowerCase() && (!_slotId || r.slotId === _slotId); }))
  ...
  const reg = { id: _regId, name: name, email: email.toLowerCase(), phone: phone, lang: _lang, registeredAt: new Date().toISOString(), checkinPin: _pin, cancelToken: _cancelToken, regToken: _regToken, slotId: _slotId || null };
  if (req.body.walkin) {
    reg.walkin = true;
    reg.checkedIn = true;
...
app.post("/avmeld", rateLimit(10, 60000), function(req, res) {
  ...
  if (!foundEv) return res.send(cancelPageHtml(null, null, "invalid_token"));
  if (foundReg.cancelledAt) return res.send(cancelPageHtml(foundEv, foundReg, "already_cancelled"));
  // Return inventar items for the slot when cancelling
  var _cancelSlotId = foundReg.slotId;
  if (_cancelSlotId && events[evIdx].slots) {
    var _cancelSlot = events[evIdx].slots.find(function(s){ return s.id === _cancelSlotId; });
    var _returnPool = _cancelSlot && _cancelSlot.inventarPool;
    if (_returnPool && _returnPool.length) {
      var _invItems = readJSON(INVENTAR_FILE);
      _returnPool.forEach(function(p) {
