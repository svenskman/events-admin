...
function _showEvDetail(upcoming, idx, count, dayIdx) {
  ...
  if (ev.eventType === 'tur') {
    // Top: alltid totalkartet (hele turen)
    var totalMap = ev.routeMapImage || ev.tvImageTop || null;
    if (topImg) {
      if (totalMap) { topImg.src = totalMap; topImg.style.display = 'block'; }
      else          { topImg.src = ''; topImg.style.display = 'none'; }
    }
    // Bottom: dagskart for gjeldende dag (dayIdx), ellers tvImageBottom
    var dayMap = null;
    if (dayIdx != null && ev.dayMapImages && ev.dayMapImages[dayIdx]) {
      dayMap = ev.dayMapImages[dayIdx];
    }
    var botSrc = dayMap || ev.tvImageBottom || null;
    if (botImg) {
      if (botSrc) { botImg.src = botSrc; botImg.style.display = 'block'; }
      else        { botImg.src = ''; botImg.style.display = 'none'; }
    }
  } else {
    // Andre event-typer: vanlig logikk
    if (topImg) {
