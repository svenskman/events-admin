...
<div class="ev-content">
  ...
  <div class="ev-panel active" id="tab-reg">
    <div class="ev-card">
      ...
      <label style="font-weight:700;margin-bottom:.25rem">📍 Velg sted / tidspunkt *</label>
      <div id="evSlotPicker" style="display:flex;flex-direction:column;gap:.4rem;margin-bottom:.5rem">
        \${_slotsWithCounts.map(s=>{
          ...
          var badge=full?'<span style="color:#e00;font-size:.78rem">(Fullt)</span>':s.available<=3?'<span style="color:#e67e00;font-size:.78rem">('+s.available+' igjen)</span>':'<span style="color:#16a34a;font-size:.78rem">('+s.available+' ledige)</span>';
          var parts=[s.label];if(s.location)parts.push(s.location);if(s.date)parts.push(s.date);if(s.time)parts.push('kl.'+s.time);
  ...
  <div class="ev-panel" id="tab-gb">
    ...
    <div class="ev-card" style="margin-top:1rem">
      ...
      <div class="ev-form">
        ...
        <label>Telefon *</label>
        <input id="gbPhone" type="tel" placeholder="Ditt telefonnummer"/>
        <label data-i18n="gb_msg">Melding <span style="font-weight:400;color:#555">(valgfritt)</span></label>
        <textarea id="gbMsg2" placeholder="Skriv din kommentar… (valgfritt)"></textarea>
      </div>
      ...
      <button id="gbPhotoBtn" onclick="openGbPhotoPicker()" style="display:none;width:100%;background:var(--y);color:#111;font-weight:700;padding:.75rem;border-radius:8px;font-size:.95rem;border:none;cursor:pointer;margin-top:.75rem">📸 Legg ved bilder / video</button>
      ...
      <div id="gbPickerDialog" style="display:none;margin-top:.75rem;background:var(--g2);border:1px solid var(--g3);border-radius:12px;padding:1rem;text-align:center">
        <div id="gbPickerTitle" style="font-size:.85rem;margin-bottom:.85rem;color:var(--text2)">Legg ved bilder fra denne telefonen, eller la noen andre scanne QR-koden.</div>
        <div style="display:flex;flex-direction:column;gap:.6rem">
          <label style="display:flex;align-items:center;gap:.75rem;padding:.75rem 1rem;background:var(--y);color:#111;border:none;border-radius:8px;font-size:.9rem;font-weight:700;cursor:pointer;text-align:left">
            ...
            <div><div>Last opp fra denne telefonen</div><div style="font-size:.75rem;font-weight:400;opacity:.7">Velg bilder eller video fra kamera/galleri</div></div>
          ...
          <button onclick="gbPickQr()" style="display:flex;align-items:center;gap:.75rem;padding:.75rem 1rem;background:var(--g);border:1px solid var(--g3);border-radius:8px;font-size:.9rem;font-weight:700;cursor:pointer;text-align:left;width:100%">
            <span style="font-size:1.5rem">📲</span>
            <div><div>Vis QR-kode</div><div style="font-size:.75rem;font-weight:400;color:var(--text2)">La noen andre scanne og laste opp fra sin telefon</div></div>
          </button>
          <button onclick="document.getElementById('gbPickerDialog').style.display='none'" style="background:none;border:none;color:var(--text2);font-size:.85rem;cursor:pointer;padding:.35rem">Avbryt</button>
        </div>
      </div>
...
async function gbDirectUpload(input){
  ...
  if(!tr.ok||!td.token){btn.disabled=false;btn.textContent='📸 Legg ved bilder / video';return;}
  var uploaded=0;
  ...
  setTimeout(function(){
    ...
    btn.textContent='📸 Legg ved bilder / video';btn.style.display='none';btn.dataset.gbid='';
  },3000);
  input.value='';
}
  ...
  <div class="ev-panel" id="tab-gb">
    ...
    <div class="ev-card" style="margin-top:1rem">
      ...
      <div class="ev-form">
        ...
        <label>Telefon *</label><input id="gbPhone" type="tel" placeholder="Ditt telefonnummer"/>
        <label>Melding <span style="font-weight:400;color:#555">(valgfritt)</span></label><textarea id="gbMsg2" placeholder="Skriv din kommentar… (valgfritt)"></textarea>
      </div>
...
function buildTurPage(ev, isAuthenticated) {
  ...
  const etappeHtml = (function() {
    ...
    var totalMapHtml = ev.routeMapImage
      ? '<img src="'+escHtml(ev.routeMapImage)+'" style="width:100%;height:auto;min-height:260px;max-height:420px;object-fit:cover;border-radius:8px;margin-bottom:1.25rem;display:block"/>'
      : '';
    ...
    return totalMapHtml + (days||[]).map(function(day, di) {
      ...
      var dayMapImg = (ev.dayMapImages && ev.dayMapImages[di])
        ? '<img src="'+escHtml(ev.dayMapImages[di])+'" style="width:100%;height:auto;min-height:200px;max-height:340px;object-fit:cover;border-radius:6px;margin-bottom:.75rem;display:block"/>'
        : '';
      return '<div style="margin-bottom:2rem">'
        + '<div style="display:flex;align-items:center;gap:.6rem;margin-bottom:.6rem;padding-bottom:.4rem;border-bottom:2px solid #f5c50044">'
        + '<span style="font-size:.72rem;font-weight:800;text-transform:uppercase;letter-spacing:.5px;color:var(--y)">Dag '+(di+1)+(day.label ? ' – '+escHtml(day.label) : '')+'</span>'
        ...
        + etapper.map(function(e,ei) {
            ...
            var typeLabel = e.type==='overnatting' ? '🏨 Overnatting' : e.type==='bensin' ? '⛽ Bensinstopp' : e.type==='slutt' ? '🏁 Mål' : '';
