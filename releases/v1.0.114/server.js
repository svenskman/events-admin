...
app.get("/api/events/:id/routemap", auth, rateLimit(10, 60000), async function(req, res) {
  const events = readJSON(EVENTS_FILE);
  const ev = events.find(function(e) { return e.id === req.params.id; });
  if (!ev) return res.status(404).json({ error: "Not found" });
  ...
  const cacheKey = crypto.createHash("md5").update(ev.id + allNames.join("|")).digest("hex").slice(0, 12);
  const dest = path.join(UPLOADS, "routemap_" + cacheKey + ".png");
  ...
  async function geocode(name) {
    try {
      const url = "https://nominatim.openstreetmap.org/search?format=json&limit=1&q=" + encodeURIComponent(name) + "&countrycodes=no,se,dk,fi";
      const r = await fetch(url, { headers: { "User-Agent": "EventsAdmin-RouteMap/1.0" }, signal: AbortSignal.timeout(5000) });
      const d = await r.json();
  ...
  days.forEach(function(d) {
    (d.etapper || []).forEach(function(e) {
      if (e.fra && e._fra_lat && e._fra_lon) coordCache[e.fra] = { lat: e._fra_lat, lon: e._fra_lon };
      if (e.til && e._lat    && e._lon)     coordCache[e.til]  = { lat: e._lat,     lon: e._lon };
    });
  });

  for (var i = 0; i < allNames.length; i++) {
    if (!coordCache[allNames[i]]) {
  ...
  const coords = allNames.map(function(n) { return coordCache[n]; }).filter(Boolean);
  if (!coords.length) return res.status(400).json({ error: "Kunne ikke geocode noen steder" });
  ...
  const padLon = Math.max((maxLon - minLon) * 0.25, 0.06);

  const W = 1200, H = 630;
  const centerLat = (minLat + maxLat) / 2;
  const centerLon = (minLon + maxLon) / 2;
  const pathStr = coords.map(function(c) { return c.lat + "," + c.lon; }).join("|");
  const bbox = (minLon-padLon)+","+(minLat-padLat)+","+(maxLon+padLon)+","+(maxLat+padLat);

  const markerStr = coords.slice(0, 20).map(function(c, i) {
    const label = i === 0 ? "S" : (i === coords.length - 1 ? "E" : String(i));
    const color  = i === 0 ? "green" : (i === coords.length - 1 ? "red" : "blue");
    return c.lat + "," + c.lon + "," + color + "sm-" + label;
  }).join("|");

  const staticUrl = "https://staticmap.openstreetmap.de/staticmap.php" +
    "?center=" + centerLat + "," + centerLon +
    "&bbox=" + bbox +
    "&size=" + W + "x" + H +
    "&maptype=mapnik" +
    "&path=color:0x6366f1cc|weight:4|" + pathStr +
    "&markers=" + markerStr;

  function fetchToPng(url) {
    return new Promise(function(resolve, reject) {
      const protocol = url.startsWith("https") ? require("https") : require("http");
      const urlObj = new (require("url").URL)(url);
      const reqOpts = { hostname: urlObj.hostname, path: urlObj.pathname + urlObj.search, headers: { "User-Agent": "EventsAdmin/1.0" }, timeout: 20000 };
      const req = protocol.get(reqOpts, function(r) {
        if (r.statusCode >= 300 && r.statusCode < 400 && r.headers.location)
          return fetchToPng(r.headers.location).then(resolve).catch(reject);
  ...
  try {
    const buf = await fetchToPng(staticUrl);
    if (!buf || buf.length < 5000) return res.status(500).json({ error: "Tomt kartbilde" });
    fs.writeFileSync(dest, buf);
    const evIdx = events.findIndex(function(e) { return e.id === req.params.id; });
    if (evIdx !== -1) {
      events[evIdx].routeMapImage = "/uploads/" + path.basename(dest);
      writeJSON(EVENTS_FILE, events);
    }
    res.json({ ok: true, url: "/uploads/" + path.basename(dest), cached: false });
  } catch(e) {
    res.status(500).json({ error: "Kartbilde feilet: " + e.message });
  }
});
