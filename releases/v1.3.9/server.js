...
app.post("/api/lab/remote/control", auth, adminOnly, rateLimit(10, 60000), async function(req, res) {
  ...
  try {
    const path = action === "restore" ? "/restore/" + version : "/restart";
    const r    = await watchdogRequest("POST", path);
    res.json({ ok: r.status === 200, result: r.data });
  } catch(e) {
    res.status(502).json({ err: "Kan ikke nå watchdog: " + e.message });
  }
});
