...
async function openGbQrModal(gbId){
  ...
  overlay.innerHTML='<div style="background:var(--g);border-radius:20px 20px 0 0;width:100%;max-width:540px;padding:2rem 1.5rem;padding-bottom:calc(2rem + env(safe-area-inset-bottom));text-align:center;position:relative">'
    ...
    +'<p style="font-size:.85rem;color:var(--text2);margin-bottom:1.25rem;line-height:1.5">Skann QR-koden med mobilen for å laste opp bilder til kommentaren din.</p>'
    ...
    +'</div>'
    +'<div id="gbQrStatus" style="font-size:.85rem;color:var(--text2);min-height:1.4rem;margin-bottom:.75rem">Venter på bilde…</div>'
    +'<button onclick="closeGbQr()" style="width:100%;padding:14px;background:none;color:var(--text);font-weight:700;font-size:.95rem;border:2px solid var(--g3);border-radius:8px;cursor:pointer">Hopp over</button>'
    +'</div>';
  ...
  sse.onmessage=function(e){
    ...
    if(data.ok){var st=document.getElementById('gbQrStatus');if(st)st.innerHTML='<span style="color:#7dff7d">✅ Bilde mottatt!</span>';sse.close();setTimeout(closeGbQr,1800);}
  };
