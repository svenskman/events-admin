...
function serveDepartmentPage(dept, req, res) {
  ...
  if (filtered.length === 0) {
    ...
    if (showFilterBar) {
      ...
      columns.forEach(function(kol, ki) {
        html += '<button class="mob-tab" onclick="switchMobTab(' + ki + ')">' + escHtml(kol.label) + '</button>';
      });
      html += '</div>';
      if (allHistCount > 0) {
  ...
  html += '</div>'
    ...
    + 'function filterAllHistYear(y){var s=document.getElementById(\'colHistoryAll\');if(!s)return;s.querySelectorAll(\'div[id]\').forEach(function(d){if(d.id.indexOf(\'histAllYear-\')==0){var yr=parseInt(d.id.replace(\'histAllYear-\',\'\'));d.style.display=yr===y?\'\': \'none\';}});}\n'
    + 'function switchMobTab(ki){document.querySelectorAll(\'.col-section\').forEach(function(s,i){s.style.display=i===ki?\'\':!\'none\';s.classList.toggle(\'mob-active\',i===ki);});var a=document.getElementById(\'colSectionAll\');if(a){a.style.display=\'none\';a.classList.remove(\'mob-active\');}var hb=document.getElementById(\'histBtnAll\');if(hb)hb.style.display=\'none\';document.querySelectorAll(\'.mob-tab\').forEach(function(b,i){b.classList.toggle(\'active\',i===ki);});var w=document.getElementById(\'colWrap\'+ki);if(w){var ak=w.querySelector(\'.ev-card.active\');var tg=ak||w.querySelector(\'.ev-card\');if(tg){window.scrollTo({top:Math.max(0,tg.getBoundingClientRect().top+window.scrollY-120),behavior:\'smooth\'});}}}\n'
...
<footer style="position:fixed;bottom:0;left:0;right:0;text-align:center;padding:.4rem;font-size:.55rem;color:rgba(255,255,255,.12);pointer-events:none;z-index:1">Event-Admin.online · v1.7.10</footer>
</body>
</html>`;
}
