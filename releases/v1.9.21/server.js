...
app.get("/api/lab/stats", auth, adminOnly, function(req, res) {
  ...
  if (!LAB_DATA_DIR || !fs.existsSync(LAB_DATA_DIR)) {
    return res.status(503).json({ err: "LAB_DATA_DIR not configured" });
  }
  const days = Math.min(parseInt(req.query.days) || 30, 365);
  const since = new Date(Date.now() - days * 86400000).toISOString();

  try {
    const envDirs = fs.readdirSync(LAB_DATA_DIR).filter(function(e) {
      const full = path.join(LAB_DATA_DIR, e);
      return fs.statSync(full).isDirectory() && !e.startsWith("_");
    });

    const results = envDirs.map(function(envName) {
      const dataDir  = path.join(LAB_DATA_DIR, envName);
      const visFile  = path.join(dataDir, "visits.json");
      const setFile  = path.join(dataDir, "settings.json");

      let siteName = envName;
      let domain   = "";
      try {
        const s = JSON.parse(fs.readFileSync(setFile, "utf8"));
        siteName = s.siteName || envName;
        domain   = s.adminDomain || s.domain || "";
      } catch(e2) {}

      let visits = [];
      try { visits = JSON.parse(fs.readFileSync(visFile, "utf8")); } catch(e2) {}

      const recent = visits.filter(function(v) { return v.ts >= since; });
      ...
      // Last 30 days filled array for chart
      const dayArr = [];
      ...
      return {
        ...
        total:     recent.length,
        today:     byDay[today]     || 0,
        yesterday: byDay[yest]      || 0,
        allTime:   visits.length,
        days:      dayArr,
        isDemo:    envName.toLowerCase().includes("demo"),
      };
    });
...
function serveDepartmentPage(dept, req, res) {
  ...
  var css = '*{box-sizing:border-box;margin:0;padding:0}'
    ...
    + '.header-brand{display:flex;align-items:center;gap:.75rem;flex-shrink:0;padding:.85rem 1.5rem;border-right:1px solid '+headerBorder+'}'
    ...
    + '.header-sub{font-size:.75rem;color:'+txt2+';white-space:nowrap}'
    ...
    + '.header-brand{padding:.45rem .6rem;gap:.4rem;border-right:none}'
