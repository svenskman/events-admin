...
function serveDepartmentPage(dept, req, res) {
  ...
  if (filtered.length === 0) {
    ...
    columns.forEach(function(kol, ki) {
      ...
      html += '<div class="col-section' + (ki===0?' mob-active':'') + '" id="colSection' + ki + '">'
        ...
        var colHistCount = historyEvents.filter(function(ev) { return ev.subgroup === kol.id || (!ev.subgroup && kol.id === columns[0].id); }).length;
        html += '<div class="col-head"><h2>' + escHtml(kol.label) + '</h2>'
          ...
          + (colHistCount > 0
            ? '<button id="histBtn-' + ki + '" onclick="toggleColHistory(' + ki + ')" style="'
              ...
              + '">🕐 Historikk (' + colHistCount + ')</button>'
            : '')
          + '</div>';

      // Per-column history section — rendered ABOVE current events, chronological order
      var colHist = historyEvents.filter(function(ev) { return ev.subgroup === kol.id || (!ev.subgroup && kol.id === columns[0].id); });
      if (colHist.length > 0) {
        // Collect unique years in history
        var histYears = [];
        ...
        histYears.sort(function(a,b){return a-b;}); // chronological
        var currentYear = new Date().getFullYear();
        var hasMultipleYears = histYears.length > 1 || (histYears.length === 1 && histYears[0] < currentYear);
        // Default: show most recent history year
        var defaultYear = histYears[histYears.length - 1];
  ...
  html += '</div><footer>' + escHtml(siteName) + ' · ' + escHtml(dept.name) + '</footer>'
    ...
    + '  var m=btn.textContent.match(/\\(([0-9]+)\\)/);\n'
    + '  var n=m?m[1]:"";\n'
