...
app.post("/api/backup/restore", auth, rateLimit(5, 60000), multer({ storage: multer.memoryStorage() }).single("backupFile"), async function(req, res) {
  ...
  // Manager-check: must have manager role on the backup's department
  if (!isAdmin) {
    const acc = getAccessList(readJSON(USERS_FILE).find(function(u){ return u.id === user.id; }) || {});
    const isManager = acc.some(function(a) {
      return a.role === "department_manager" || a.role === "avdelingsleder";
    });
  ...
  // Manager restrictions: can only restore to own department, never settings/inventar
  if (!isAdmin) {
    const acc = getAccessList(readJSON(USERS_FILE).find(function(u){ return u.id === user.id; }) || {});
    const myDeptIds = acc
      .filter(function(a) { return a.role === "department_manager" || a.role === "avdelingsleder"; })
      .map(function(a) { return a.department; });

    // Backup must belong to a department the manager controls
    ...
    // Managers can never restore settings or inventar
