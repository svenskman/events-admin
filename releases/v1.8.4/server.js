...
try {
  ...
  var sessionDir = path.join(DATA_DIR, "sessions");
...
function startNavigation() {
  if (!NAV_LAT || !NAV_LON) { startNavigationByName(); return; }
  var isIOS = /iPhone|iPad|iPod/i.test(navigator.userAgent);
  var url = isIOS
    ? 'maps://?daddr=' + NAV_LAT + ',' + NAV_LON
    : 'https://www.google.com/maps/dir/?api=1&destination=' + NAV_LAT + ',' + NAV_LON + '&travelmode=driving';
...
function startNavigationByName() {
  ...
  var isIOS = /iPhone|iPad|iPod/i.test(navigator.userAgent);
  var url = isIOS
    ? 'maps://?daddr=' + encodeURIComponent(NAV_NAME)
    : 'https://www.google.com/maps/dir/?api=1&destination=' + encodeURIComponent(NAV_NAME) + '&travelmode=driving';
  window.open(url, '_blank');
}
...
<div class="ev-content">
  ...
  <div class="ev-panel" id="tab-info">
    ...
    <div class="ev-card">
      <div class="ev-card-title">${T('practical_info', lang)}</div>
      ${ev.date ? `<div class="info-row"><span class="icon">📅</span><div><div class="label">${T('date_time', lang)}</div><div class="value">${evFmtDate(ev.date)}${ev.date ? ' kl. ' + evFmtTime(ev.date) : ''}</div></div></div>` : ''}
      ${ev.location ? `<div class="info-row"><span class="icon">📍</span><div><div class="label">${T('location', lang)}</div><div class="value">${escHtml(ev.location)}</div></div></div>` : ''}
      ${contactEmail ? `<div class="info-row"><span class="icon">✉️</span><div><div class="label">${T('contact', lang)}</div><div class="value"><a href="mailto:${escHtml(contactEmail)}" style="color:var(--y)">${escHtml(contactEmail)}</a></div></div></div>` : ''}
    </div>
  ...
  <div class="ev-panel" id="tab-gb">
    ...
    <div class="ev-card" style="margin-top:1rem">
      ...
      <div class="ev-form">
        ...
        <label data-i18n="gb_msg">Melding <span style="font-weight:400;color:#555">(valgfritt)</span></label>
      ...
      <button id="gbPhotoBtn" onclick="openGbPhotoPicker()" style="display:none;width:100%;background:var(--y);color:#111;font-weight:700;padding:.75rem;border-radius:8px;font-size:.95rem;border:none;cursor:pointer;margin-top:.75rem">${T('gb_upload', lang)}</button>
      <div id="gbQrArea" style="margin-top:.75rem;text-align:center"></div>

      <!-- Valgdialog: direkte opplasting (mobil) eller QR -->
      <div id="gbPickerDialog" style="display:none;margin-top:.75rem;background:var(--g2);border:1px solid var(--g3);border-radius:12px;padding:1rem;text-align:center">
        <div id="gbPickerTitle" style="font-size:.85rem;margin-bottom:.85rem;color:var(--text2)">${T('gb_picker_hint', lang)}</div>
        <div style="display:flex;flex-direction:column;gap:.6rem">
          <label style="display:flex;align-items:center;gap:.75rem;padding:.75rem 1rem;background:var(--y);color:#111;border:none;border-radius:8px;font-size:.9rem;font-weight:700;cursor:pointer;text-align:left">
            <span style="font-size:1.5rem">📷</span>
            <div><div>${T('gb_upload_direct', lang)}</div><div style="font-size:.75rem;font-weight:400;opacity:.7">${T('gb_upload_hint', lang)}</div></div>
            <input type="file" accept="image/*,video/*" capture="environment" multiple style="display:none" onchange="gbDirectUpload(this)"/>
          </label>
          <button onclick="gbPickQr()" style="display:flex;align-items:center;gap:.75rem;padding:.75rem 1rem;background:var(--g);border:1px solid var(--g3);border-radius:8px;font-size:.9rem;font-weight:700;cursor:pointer;text-align:left;width:100%">
            <span style="font-size:1.5rem">📲</span>
            <div><div>${T('gb_qr', lang)}</div><div style="font-size:.75rem;font-weight:400;color:var(--text2)">${T('gb_qr_hint', lang)}</div></div>
          </button>
          <button onclick="document.getElementById('gbPickerDialog').style.display='none'" style="background:none;border:none;color:var(--text2);font-size:.85rem;cursor:pointer;padding:.35rem">${T('gb_cancel', lang)}</button>
        </div>
      </div>
...
async function submitGb(){
  ...
  if(!name||!phone){msg.innerHTML='<div class="msg msg-err">'+t('gb_err')+'</div>';return;}
...
function openGbPhotoPicker(){
  ...
  if(!isMobile){
    // Desktop: gå direkte til QR
    showGbQr();
    return;
  }
  // Mobil: vis valgdialog med direkte opplasting øverst
  var picker=document.getElementById('gbPickerDialog');
  var pickerTitle=document.getElementById('gbPickerTitle');
  if(pickerTitle)pickerTitle.textContent='Legg ved bilder fra denne telefonen, eller la noen andre scanne QR-koden.';
...
async function gbDirectUpload(input){
  ...
  setTimeout(function(){
    ...
    btn.textContent='${T("gb_upload",lang)}';btn.style.display='none';btn.dataset.gbid='';
  },3000);
  input.value='';
}
      ...
      ${_slotsWithCounts.length > 0
        ? _slotsWithCounts.map(function(s) {
            var badge = s.available === 0
              ? '<span class="spots-full">Fullt</span>'
              : s.available <= 3
...
<div class="ev-content">
  ...
  <div class="ev-panel active" id="tab-info">
    ...
    <div class="ev-card">
      ...
      ${ev.date ? `<div class="info-row"><span class="icon">📅</span><div><div class="label">${T('date_time', lang)}</div><div class="value">${evFmtDate(ev.date)}${ev.date ? ' kl. ' + evFmtTime(ev.date) : ''}</div></div></div>` : ''}
      ${ev.location ? `<div class="info-row"><span class="icon">📍</span><div><div class="label">${T('location', lang)}</div><div class="value">${escHtml(ev.location)}</div></div></div>` : ''}
      ${ev.maxParticipants ? `<div class="info-row"><span class="icon">👥</span><div><div class="label">${T('spots_left', lang)}</div><div class="value ${isFull?'spots-full':spotsLeft&&spotsLeft<=3?'spots-low':'spots-ok'}">${isFull ? 'Kurset er fullt' : spotsLeft + ' av ' + ev.maxParticipants + ' ledige'}</div></div></div>` : ''}
      ${contactEmail ? `<div class="info-row"><span class="icon">✉️</span><div><div class="label">${T('contact', lang)}</div><div class="value"><a href="mailto:${escHtml(contactEmail)}" style="color:var(--y)">${escHtml(contactEmail)}</a></div></div></div>` : ''}
    </div>
    ${!isFull
  ...
  <div class="ev-panel" id="tab-gb">
    ...
    <div class="ev-card" style="margin-top:1rem">
      ...
      <div class="ev-form">
        ...
        <label>Telefon *</label><input id="gbPhone" type="tel" placeholder="${T('ph_phone_num', lang)}"/>
      ...
      <div id="gbQrArea" style="display:none"></div>
...
function buildTurPage(ev, isAuthenticated) {
  ...
  const etappeHtml = (function() {
    if (!(days||[]).some(function(d){ return (d.etapper||[]).some(function(e){ return e.fra||e.til; }); }))
      return '<div class="empty"><span class="empty-icon">🗺️</span>Ingen etappeplan lagt inn ennå</div>';

    // Totalkart øverst i etappeplanen
    var totalMapHtml = ev.routeMapImage
      ? '<img src="'+escHtml(ev.routeMapImage)+'" style="width:100%;height:auto;min-height:320px;max-height:520px;object-fit:cover;border-radius:8px;margin-bottom:1.25rem;display:block"/>'
      : '';
    ...
    return totalMapHtml + (days||[]).map(function(day, di) {
      ...
      var dayMapImg = (ev.dayMapImages && ev.dayMapImages[di])
        ? '<img src="'+escHtml(ev.dayMapImages[di])+'" style="width:100%;height:auto;min-height:260px;max-height:420px;object-fit:cover;border-radius:6px;margin-bottom:.75rem;display:block"/>'
        : '';
...
<div class="ev-content">
  ...
  <div class="ev-panel active" id="tab-info">
    ...
    <div class="ev-card">
      <div class="ev-card-title">${T('practical_info', lang)}</div>
      ${ev.date ? `<div class="info-row"><span class="icon">📅</span><div><div class="label">${T('departure', lang)}</div><div class="value">${evFmtDate(ev.date)} kl. ${evFmtTime(ev.date)}</div></div></div>` : ''}
      ${ev.location ? `<div class="info-row"><span class="icon">📍</span><div><div class="label">${T('meetup', lang)}</div><div class="value">${escHtml(ev.location)}</div></div></div>` : ''}
      ${days.length ? `<div class="info-row"><span class="icon">🗓</span><div><div class="label">${T('days_count', lang)}</div><div class="value">${days.length} dag${days.length>1?'er':''}</div></div></div>` : ''}
      ${ev.maxParticipants ? `<div class="info-row"><span class="icon">🏍</span><div><div class="label">${T('spots_left', lang)}</div><div class="value ${isFull?'spots-full':spotsLeft&&spotsLeft<=3?'spots-low':'spots-ok'}">${isFull ? T('spots_full', lang) : spotsLeft + ' / ' + ev.maxParticipants + ' ' + T('spots_left', lang)}</div></div></div>` : ''}
      ${contactEmail ? `<div class="info-row"><span class="icon">✉️</span><div><div class="label">${T('contact', lang)}</div><div class="value"><a href="mailto:${escHtml(contactEmail)}" style="color:var(--y)">${escHtml(contactEmail)}</a></div></div></div>` : ''}
    </div>
    ${!isFull
  ...
  <div class="ev-panel" id="tab-rute">
    ...
    <button class="btn-primary" style="margin-bottom:.75rem;display:flex;align-items:center;justify-content:center;gap:.5rem;font-size:1rem" onclick="startNavigation()">
      ${T('btn_nav', lang)}
    </button>` : ev.location ? `
    <button class="btn-primary" style="margin-bottom:.75rem;display:flex;align-items:center;justify-content:center;gap:.5rem;font-size:1rem" onclick="startNavigationByName()">
      ${T('btn_nav', lang)}
    </button>` : ''}
    <div class="ev-card">
    ...
    ${days.length && days[0] && days[0].etapper && days[0].etapper[0] && (days[0].etapper[0]._fra_lat || days[0].etapper[0]._fra_lon) ? `
    <button class="btn-primary" style="margin-top:.75rem;display:flex;align-items:center;justify-content:center;gap:.5rem;font-size:1rem" onclick="startNavigation()">
      ${T('btn_nav', lang)}
    </button>` : ev.location ? `
    <button class="btn-primary" style="margin-top:.75rem;display:flex;align-items:center;justify-content:center;gap:.5rem;font-size:1rem" onclick="startNavigationByName()">
      ${T('btn_nav', lang)}
    </button>` : ''}
  </div>
