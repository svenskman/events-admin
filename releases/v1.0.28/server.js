...
app.use(function(req, res, next) {
  ...
  // Root domain → org overview
  if (!slug) {
    return serveOrgPage(req, res);
  }
  ...
  if (slug) {
    ...
    var dept = depts.find(function(a) { return a.slug === slug; });
    if (dept) return serveDepartmentPage(dept, null, req, res);

    // Check dept-subgroup pattern: "romerike-mc" → dept slug "romerike" + subgroup slug "mc"
    var dashIdx = slug.indexOf('-');
    while (dashIdx !== -1) {
      var deptSlug = slug.slice(0, dashIdx);
      var ugSlug   = slug.slice(dashIdx + 1);
      var parentDept = depts.find(function(a) { return a.slug === deptSlug; });
      if (parentDept) {
        var ug = (parentDept.subgroups || []).find(function(u) {
          var s = (u.slug || u.name.toLowerCase().replace(/æ/g,'ae').replace(/ø/g,'o').replace(/å/g,'a').replace(/[^a-z0-9]+/g,'-').replace(/(^-|-$)/g,''));
          return s === ugSlug;
        });
        if (ug) return serveDepartmentPage(parentDept, ug, req, res);
      }
      dashIdx = slug.indexOf('-', dashIdx + 1);
    }
...
// ── Varsle vinner via e-post ──────────────────────────────────────
app.post("/api/events/:id/lottery/notify-winner", auth, managerOrAdmin, async function(req, res) {
  const ev = readJSON(EVENTS_FILE).find(function(e) { return e.id === req.params.id; });
  if (!ev) return res.status(404).json({ error: "Ikke funnet" });
  const { drawNum } = req.body;
  const winners = (ev.lottery && ev.lottery.winners) || [];
  const win = drawNum ? winners.find(function(w) { return w.drawNum === parseInt(drawNum); }) : winners[winners.length - 1];
  if (!win) return res.status(404).json({ error: "Ingen vinner funnet" });

  // Find email from registration
  const reg = (ev.registrations || []).find(function(r) { return r.id === win.regId; });
  const email = reg && reg.email;
  if (!email) return res.status(400).json({ error: "Vinner har ingen e-postadresse registrert" });

  const s = getSettings();
  const siteName  = s.siteName || "Events Admin";
  const accentCol = (s.colors && s.colors.accent) || "#FFD100";
  const evUrl     = "https://" + ev.slug + "." + (s.eventDomain || DOMAIN);
  const drawnDate = new Date(win.drawnAt).toLocaleDateString("nb-NO", { day:"numeric", month:"long", year:"numeric" });

  // Simple certificate HTML → sent as HTML email body
  const certHtml = `<!DOCTYPE html>
<html><head><meta charset="UTF-8"/>
<style>
  body{font-family:"Helvetica Neue",Arial,sans-serif;background:#f5f5f5;margin:0;padding:2rem}
  .cert{max-width:600px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,.1)}
  .top{background:${accentCol};padding:2rem;text-align:center}
  .top h1{margin:0;font-size:1.8rem;color:#111;font-weight:900}
  .top p{margin:.3rem 0 0;color:#333;font-size:.9rem}
  .body{padding:2.5rem;text-align:center}
  .trophy{font-size:4rem;display:block;margin-bottom:1rem}
  .winner-name{font-size:2rem;font-weight:900;color:#111;margin-bottom:.5rem}
  .prize-label{font-size:.8rem;color:#888;text-transform:uppercase;letter-spacing:.6px;margin-bottom:.3rem}
  .prize{font-size:1.3rem;font-weight:700;color:#111;margin-bottom:1.5rem}
  .meta{font-size:.85rem;color:#888;margin-bottom.5rem;line-height:1.6}
  .footer{background:#f9f9f9;border-top:1px solid #eee;padding:1rem 2rem;text-align:center;font-size:.78rem;color:#aaa}
</style></head>
<body>
<div class="cert">
  <div class="top">
    <h1>🎉 Gratulerer!</h1>
    <p>${escHtml(ev.title)} · ${drawnDate}</p>
  </div>
  <div class="body">
    <span class="trophy">🏆</span>
    <div class="winner-name">${escHtml(win.name)}</div>
    ${win.prize ? `<div class="prize-label">Premie</div><div class="prize">${escHtml(win.prize)}</div>` : ""}
    <div class="meta">Trukket av ${escHtml(win.drawnBy || siteName)}<br/>Trekning #${win.drawNum}</div>
  </div>
  <div class="footer">${escHtml(siteName)} · <a href="${escHtml(evUrl)}">${escHtml(evUrl)}</a></div>
</div>
</body></html>`;
  ...
  try {
    await sendMail({
      to:      email,
      subject: "🎉 Du vant! – " + ev.title,
      html:    certHtml
    });
    res.json({ ok: true, sentTo: email });
  } catch(e) {
    res.status(500).json({ error: "E-post feil: " + e.message });
  }
});
...
app.post("/api/events/:id/register", rateLimit(10, 60000), function(req, res) {
  ...
  sendEmail({ to: email, subject: mail.subject, html: mail.html, text: mail.text }).catch(function(e) { console.error('[email] catch:', e && e.message); });
  res.json({ ok: true });
});
...
app.post("/avmeld", rateLimit(10, 60000), function(req, res) {
  ...
  if (foundReg.email) {
    ...
    sendEmail({ to: foundReg.email, subject: mail.subject, html: mail.html, text: mail.text }).catch(function(e) { console.error('[email] catch:', e && e.message); });
  }
...
function cancelPageHtml(ev, reg, state) {
  ...
  return `<!DOCTYPE html><html lang="no" data-theme="${theme}"><head><meta charset="UTF-8">
...
app.delete("/api/events/:id/registrations/:rid", auth, function(req, res) {
  ...
  if (reg && reg.email && !reg.anonymized) {
    ...
    sendEmail({ to: reg.email, subject: mail.subject, html: mail.html, text: mail.text }).catch(function(e) { console.error('[email] catch:', e && e.message); });
  }
  res.json({ ok: true });
});
...
app.post("/api/events/:id/staff/checkin", rateLimit(30, 60000), async function(req, res) {
  const name       = req.body.name;
  const role       = req.body.role || "";
  ...
  // ── Helper: find shift for a staff entry ──────────────────────────
  function findShift(sId) {
    return ev.shifts.find(function(sh) {
      return (sh.staffIds || []).includes(sId);
    }) || null;
  }

  // ── Helper: build fromTime/toTime from shift ──────────────────────
  function shiftTimes(shift) {
    if (!shift) return { fromTime: nowIso, toTime: null };
    // shift.date + shift.startTime / endTime  e.g. "2026-07-03" + "09:00"
    const base  = shift.date || now.toISOString().slice(0, 10);
    const from  = new Date(base + "T" + (shift.startTime || "00:00") + ":00");
    const to    = new Date(base + "T" + (shift.endTime   || "00:00") + ":00");
    // If endTime < startTime it crosses midnight
    if (to <= from) to.setDate(to.getDate() + 1);
    return { fromTime: from.toISOString(), toTime: to.toISOString() };
  }

  // ── Helper: calculate hours between two ISO strings ───────────────
  function calcHours(fromIso, toIso) {
    if (!fromIso || !toIso) return null;
    const diff = (new Date(toIso) - new Date(fromIso)) / 3600000;
    return diff > 0 ? Math.round(diff * 10) / 10 : null;
  }

  // ── Helper: find or create volunteer record ───────────────────────
  function ensureVolunteer(staffName, staffEmail) {
    let vol = volunteers.find(function(v) {
      return (staffEmail && v.email && v.email.toLowerCase() === staffEmail) ||
             v.name.toLowerCase() === staffName.toLowerCase();
    });
    if (!vol) {
      vol = { id: uuid(), name: staffName, email: staffEmail || "",
               phone: "", department: ev.department || "",
               departments: ev.department ? [ev.department] : [],
               events: [], createdAt: nowIso, autoAdded: true };
      volunteers.push(vol);
    }
    if (!vol.hours) vol.hours = [];
    return vol;
  }

  // ── Helper: register hours on volunteer ───────────────────────────
  function registerHours(vol, staffName, fromTime, toTime, shiftRole) {
    const hrs = calcHours(fromTime, toTime);
    const entry = {
      id:           uuid(),
      eventId:      ev.id,
      role:         shiftRole || "",
      ...
      toTime:       toTime || null,
      hours:        hrs,
      note:         toTime ? "Automatisk fra vaktliste" : "Åpen – sluttid ikke satt (walk-in)",
      status:       "pending",
      registeredBy: "system",
      registeredAt: nowIso,
      approvedBy:   null,
    ...
    vol.hours.push(entry);
    return entry;
  }

  // ── Helper: send confirmation email ───────────────────────────────
  async function sendCheckinEmail(toEmail, staffName, shift, hoursEntry) {
    if (!toEmail) return;
    const evDate  = ev.date ? new Date(ev.date).toLocaleDateString("nb-NO", { weekday:"long", day:"numeric", month:"long" }) : "";
    const fmtTime = function(iso) { return iso ? new Date(iso).toLocaleTimeString("nb-NO", { hour:"2-digit", minute:"2-digit" }) : "–"; };
    const siteName = settings.siteName || "Events Admin";
    const shiftLine = shift
      ? `Vakt: ${shift.startTime || "–"} – ${shift.endTime || "–"}`
      : `Innsjekk: ${fmtTime(hoursEntry.fromTime)}`;
    const subject = `✅ Innsjekk bekreftet – ${ev.title}`;
    const bodyText = `Hei ${staffName}!\n\nDu er sjekket inn som personell på:\n\n${ev.title}${evDate ? "\n" + evDate : ""}\n${shiftLine}${ev.location ? "\n📍 " + ev.location : ""}\n\nTimene dine registreres automatisk og sendes til godkjenning av ansvarlig.\n\nTakk for innsatsen!\n\n– ${siteName}`;
    const accent  = (settings.colors && settings.colors.accent) || "#FFD100";
    const html    = `<div style="font-family:sans-serif;max-width:540px;margin:0 auto">
      <div style="background:${accent};padding:1.25rem 1.5rem;border-radius:8px 8px 0 0">
        <span style="font-weight:900;font-size:1.1rem;color:#111">${escHtml(siteName)}</span>
      </div>
      <div style="background:#f9f9f9;padding:2rem;border-radius:0 0 8px 8px;border:1px solid #e0e0e0">
        <h2 style="margin:0 0 1rem;color:#1a1a1a">Hei ${escHtml(staffName)}! ✅</h2>
        <p style="color:#333;line-height:1.7">Du er sjekket inn som personell på <strong>${escHtml(ev.title)}</strong>.</p>
        <div style="background:#fff;border-left:4px solid ${accent};padding:1rem;border-radius:4px;margin:1rem 0;line-height:1.9">
          ${evDate ? `<div>📅 ${escHtml(evDate)}</div>` : ""}
          <div>⏱ ${escHtml(shiftLine)}</div>
          ${ev.location ? `<div>📍 ${escHtml(ev.location)}</div>` : ""}
        </div>
        <p style="color:#555;font-size:.9rem">Timene dine registreres automatisk og sendes til godkjenning av ansvarlig.</p>
        <hr style="border:none;border-top:1px solid #e0e0e0;margin:1.5rem 0"/>
        <p style="font-size:.75rem;color:#999">Takk for innsatsen! – ${escHtml(siteName)}</p>
      </div>
    </div>`;
    await sendEmail({ to: toEmail, subject, html, text: bodyText });
  }

  // ── SCENARIO A: forhåndsregistrert (staffId gitt) ─────────────────
  if (staffId) {
    const s = ev.staff.find(function(s) { return s.id === staffId; });
    if (!s) return res.status(404).json({ error: "Ikke funnet" });
    if (s.checkedIn) return res.json({ ok: true, alreadyCheckedIn: true });

    s.checkedIn   = true;
    s.checkedInAt = nowIso;

    // Find volunteer and shift
    const vol   = ensureVolunteer(s.name, s.email || "");
    const shift = findShift(staffId);
    const times = shiftTimes(shift);
    const hEntry = registerHours(vol, s.name, times.fromTime, times.toTime, s.role || shift && (shift.role || "") || "");

    writeJSON(EVENTS_FILE, events);
    writeJSON(VOLUNTEERS_FILE, volunteers);

    // Send email if volunteer has one
    const toEmail = s.email || vol.email || "";
    if (toEmail) sendCheckinEmail(toEmail, s.name, shift, hEntry).catch(function(){});

    return res.json({ ok: true, id: staffId, hoursEntry: hEntry });
  }

  // ── SCENARIO B: walk-in (ikke på listen) ──────────────────────────
  if (!name) return res.status(400).json({ error: "Navn er påkrevd" });

  const newId  = uuid();
  const entry  = { id: newId, name, role, checkedIn: true,
                   addedAt: nowIso, checkedInAt: nowIso, walkin: true };
  if (department) entry.department = department;
  if (email)      entry.email      = email;
  ev.staff.push(entry);

  // Volunteer record + open hours entry (no endTime)
  const vol    = ensureVolunteer(name, email);
  const hEntry = registerHours(vol, name, nowIso, null, role);

  writeJSON(EVENTS_FILE, events);
  writeJSON(VOLUNTEERS_FILE, volunteers);

  if (email) sendCheckinEmail(email, name, null, hEntry).catch(function(){});

  res.json({ ok: true, id: newId, hoursEntry: hEntry });
});
...
<html lang="no" data-theme="${gbTheme}">
<head>
<meta charset="UTF-8"/>
<meta name="viewport" content="width=device-width,initial-scale=1"/>
...
// ── Forbrukshistorikk for serie (giveaway + salg) ─────────────────
app.get("/api/series/:id/forbruk", auth, function(req, res) {
  ...
  const result = events.map(function(ev) {
    return {
      ...
      title:    ev.title,
      date:     ev.date || null,
      giveaway: (ev.giveaway || []).map(function(g) {
        return { id: g.id, utDelt: g.utDelt||0, retur: g.retur||0, netto: (g.utDelt||0)-(g.retur||0), tomtKl: g.tomtKl||null };
      }),
      salg: (ev.salg || []).map(function(s) {
        return { id: s.id, solgt: s.solgt||0, tomtKl: s.tomtKl||null };
      })
    };
  });
  res.json(result);
...
// Totaloversikt (alle frivillige, alle events): GET /api/volunteers/hours-summary
app.get("/api/volunteers/hours-summary", auth, managerOrAdmin, function(req, res) {
  var all    = readJSON(VOLUNTEERS_FILE) || [];
  var events = readJSON(EVENTS_FILE) || [];
  var me = req.session.user;
  var myDeptIds = me.role === "admin" ? null : getAccessList(readJSON(USERS_FILE).find(function(u){ return u.id === me.id; }) || {}).map(function(t){ return t.department; });
  var summary = all
    .filter(function(v) {
      if (!myDeptIds) return true;
      var vd = Array.isArray(v.departments) ? v.departments : (v.department ? [v.department] : []);
      return vd.some(function(d){ return myDeptIds.indexOf(d) !== -1; });
    })
    .map(function(v) {
      var approved = (v.hours || []).filter(function(h){ return h.status === "approved"; });
      var pending  = (v.hours || []).filter(function(h){ return h.status === "pending"; });
      var totalH   = approved.reduce(function(s, h){ return s + (h.hours || 0); }, 0);
      return {
        id:            v.id,
        name:          v.name,
        email:         v.email || "",
        phone:         v.phone || "",
        departments:   v.departments || (v.department ? [v.department] : []),
        approvedHours: Math.round(totalH * 10) / 10,
        pendingHours:  Math.round(pending.reduce(function(s,h){ return s+(h.hours||0); }, 0) * 10) / 10,
        entriesCount:  (v.hours || []).length,
        pendingCount:  pending.length,
        lastEntry:     (v.hours || []).length ? (v.hours || []).slice(-1)[0].registeredAt : null,
      };
    });
  res.json(summary);
});

// CSV-eksport: GET /api/volunteers/hours-export.csv
app.get("/api/volunteers/hours-export.csv", auth, managerOrAdmin, function(req, res) {
  var all    = readJSON(VOLUNTEERS_FILE) || [];
  var events = readJSON(EVENTS_FILE) || [];
  var me = req.session.user;
  var myDeptIds = me.role === "admin" ? null : getAccessList(readJSON(USERS_FILE).find(function(u){ return u.id === me.id; }) || {}).map(function(t){ return t.department; });
  var filterEvId = req.query.eventId || null;
  var filterStatus = req.query.status || null;

  var rows = [["Frivillig","E-post","Arrangement","Rolle","Fra","Til","Timer","Notat","Status","Registrert av","Registrert","Godkjent av","Godkjent"]];
  all
    .filter(function(v) {
      if (!myDeptIds) return true;
      var vd = Array.isArray(v.departments) ? v.departments : (v.department ? [v.department] : []);
    ...
    .forEach(function(v) {
      (v.hours || []).forEach(function(h) {
        if (filterEvId && h.eventId !== filterEvId) return;
        if (filterStatus && h.status !== filterStatus) return;
        var ev = h.eventId ? events.find(function(e){ return e.id === h.eventId; }) : null;
        var fmtDate = function(d){ return d ? new Date(d).toLocaleString("nb-NO") : ""; };
        rows.push([
          v.name, v.email || "",
          ev ? ev.title : (h.eventId || ""),
          h.role || "", fmtDate(h.fromTime), fmtDate(h.toTime),
          String(h.hours || ""), h.note || "",
          h.status === "approved" ? "Godkjent" : h.status === "rejected" ? "Avvist" : "Venter",
          h.registeredBy || "", fmtDate(h.registeredAt),
          h.approvedBy || "", fmtDate(h.approvedAt),
        ]);
      });
    });
  ...
  res.setHeader("Content-Disposition", 'attachment; filename="timer-frivillige.csv"');
  res.send("\uFEFF" + csv); // BOM for Excel UTF-8
});

// ── Timeregistrering for frivillige ─────────────────────────────
// Datamodell på frivillig: v.hours = [{ id, eventId, role, fromTime, toTime, hours, note, status, registeredBy, registeredAt, approvedBy, approvedAt }]
// status: "pending" | "approved" | "rejected"

app.get("/api/volunteers/:id/hours", auth, managerOrAdmin, function(req, res) {
  var all = readJSON(VOLUNTEERS_FILE) || [];
  var v = all.find(function(x){ return x.id === req.params.id; });
  if (!v) return res.status(404).json({ error: "Ikke funnet" });
  res.json(v.hours || []);
...
app.post("/api/volunteers/:id/hours", auth, function(req, res) {
  var all = readJSON(VOLUNTEERS_FILE) || [];
  var i = all.findIndex(function(x){ return x.id === req.params.id; });
  if (i < 0) return res.status(404).json({ error: "Ikke funnet" });
  var me = req.session.user;
  // Tilgangskontroll: admin/manager kan registrere for alle, andre kun seg selv via staff-pin (ikke implementert her - dette er admin-API)
  if (me.role !== "admin") {
    var myDeptIds = getAccessList(readJSON(USERS_FILE).find(function(u){ return u.id === me.id; }) || {}).map(function(t){ return t.department; });
    var volDepts = Array.isArray(all[i].departments) ? all[i].departments : (all[i].department ? [all[i].department] : []);
    if (!volDepts.some(function(d){ return myDeptIds.indexOf(d) !== -1; }))
      return res.status(403).json({ error: "Ingen tilgang til denne frivillige" });
  }
  var fromTime = req.body.fromTime || null;
  var toTime   = req.body.toTime   || null;
  var hours    = req.body.hours    != null ? parseFloat(req.body.hours) : null;
  // Beregn timer fra tidsintervall hvis ikke oppgitt
  if (fromTime && toTime && !hours) {
    var diff = (new Date(toTime) - new Date(fromTime)) / 3600000;
    hours = Math.round(diff * 10) / 10;
  }
  if (!hours || hours <= 0) return res.status(400).json({ error: "Antall timer må være større enn 0" });
  if (hours > 24) return res.status(400).json({ error: "Maks 24 timer per registrering" });
  var entry = {
    id:           uuid(),
  ...
  if (!all[i].hours) all[i].hours = [];
  all[i].hours.push(entry);
  writeJSON(VOLUNTEERS_FILE, all);
  res.json(entry);
});

app.put("/api/volunteers/:id/hours/:hid", auth, managerOrAdmin, function(req, res) {
  var all = readJSON(VOLUNTEERS_FILE) || [];
  var i = all.findIndex(function(x){ return x.id === req.params.id; });
  if (i < 0) return res.status(404).json({ error: "Ikke funnet" });
  var hi = (all[i].hours || []).findIndex(function(h){ return h.id === req.params.hid; });
  if (hi < 0) return res.status(404).json({ error: "Time-post ikke funnet" });
  var h = all[i].hours[hi];
  if (req.body.role      !== undefined) h.role     = (req.body.role || "").trim().slice(0, 100);
  if (req.body.fromTime  !== undefined) h.fromTime = req.body.fromTime || null;
  if (req.body.toTime    !== undefined) h.toTime   = req.body.toTime   || null;
  if (req.body.note      !== undefined) h.note     = (req.body.note || "").trim().slice(0, 300);
  if (req.body.eventId   !== undefined) h.eventId  = req.body.eventId || null;
  // Recalc hours
  if (h.fromTime && h.toTime) {
    var diff = (new Date(h.toTime) - new Date(h.fromTime)) / 3600000;
    h.hours = Math.round(diff * 10) / 10;
  } else if (req.body.hours != null) {
    h.hours = Math.min(24, Math.max(0, parseFloat(req.body.hours) || 0));
  }
  all[i].hours[hi] = h;
  writeJSON(VOLUNTEERS_FILE, all);
  res.json(h);
...
// Godkjenn/avvis timer
app.post("/api/volunteers/:id/hours/:hid/approve", auth, managerOrAdmin, function(req, res) {
  var all = readJSON(VOLUNTEERS_FILE) || [];
  var i = all.findIndex(function(x){ return x.id === req.params.id; });
  if (i < 0) return res.status(404).json({ error: "Ikke funnet" });
  var hi = (all[i].hours || []).findIndex(function(h){ return h.id === req.params.hid; });
  if (hi < 0) return res.status(404).json({ error: "Time-post ikke funnet" });
  var status = req.body.status === "rejected" ? "rejected" : "approved";
  all[i].hours[hi].status     = status;
  all[i].hours[hi].approvedBy = req.session.user.email;
  all[i].hours[hi].approvedAt = new Date().toISOString();
  writeJSON(VOLUNTEERS_FILE, all);
  res.json(all[i].hours[hi]);
});

app.delete("/api/volunteers/:id/hours/:hid", auth, managerOrAdmin, function(req, res) {
  var all = readJSON(VOLUNTEERS_FILE) || [];
  var i = all.findIndex(function(x){ return x.id === req.params.id; });
  if (i < 0) return res.status(404).json({ error: "Ikke funnet" });
  all[i].hours = (all[i].hours || []).filter(function(h){ return h.id !== req.params.hid; });
  writeJSON(VOLUNTEERS_FILE, all);
  res.json({ ok: true });
});

// Timerapport per event: GET /api/events/:id/hours-report
app.get("/api/events/:id/hours-report", auth, managerOrAdmin, function(req, res) {
  var evId = req.params.id;
  var all  = readJSON(VOLUNTEERS_FILE) || [];
  var rows = [];
  all.forEach(function(v) {
    (v.hours || []).filter(function(h){ return h.eventId === evId; }).forEach(function(h) {
      rows.push({
        volunteerId:   v.id,
...
// ── Org-oversikt (root domain) ────────────────────────────────────
function serveOrgPage(req, res) {
  var events   = readJSON(EVENTS_FILE);
  var settings = getSettings();
  var siteName = settings.siteName || "Events Admin";
  var domain   = settings.eventDomain || DOMAIN;
  var theme    = settings.theme || "dark";
  var accent   = (settings.colors && settings.colors.accent) || "#FFD100";
  ...
  var cssVars = isLight
    ? "--y:" + accent + ";--g:#ffffff;--g2:#e8eaed;--g3:#c8cad0;--b:#f0f2f5;--r:8px;--text:#1a1a1a;--text2:#555;--muted:#888;--border:#c8cad0;--header:#ffffff"
    : "--y:" + accent + ";--g:#1e1e1e;--g2:#2a2a2a;--g3:#333;--b:#111;--r:8px;--text:#f0f0f0;--text2:#aaa;--muted:#666;--border:#2a2a2a;--header:#1a1a1a";

  var css = "*{box-sizing:border-box;margin:0;padding:0}"
    + "body{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;background:var(--b);color:var(--text);min-height:100vh}"
    + ":root{" + cssVars + "}"
    + ".org-header{background:var(--header);border-bottom:3px solid var(--y);padding:1rem 1.5rem;display:flex;align-items:center;gap:1rem}"
    + ".org-logo{background:var(--y);color:#111;font-weight:900;font-size:1.2rem;padding:5px 12px;border-radius:6px;text-decoration:none;flex-shrink:0}"
    + ".org-title{font-size:1.1rem;font-weight:700;color:var(--text)}"
    + ".org-sub{font-size:.8rem;color:var(--text2)}"
    + ".container{max-width:1300px;margin:0 auto;padding:1.5rem 1rem}"
    + ".dept-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(320px,1fr));gap:1.25rem}"
    + "@media(max-width:480px){.dept-grid{grid-template-columns:1fr}}"
    + ".dept-card{background:var(--g);border:1px solid var(--border);border-radius:var(--r);overflow:hidden;text-decoration:none;color:var(--text);display:flex;flex-direction:column;transition:border-color .15s,transform .1s}"
    + ".dept-card:hover{border-color:var(--y);transform:translateY(-2px)}"
    + ".dept-card-head{padding:.85rem 1rem;border-bottom:1px solid var(--border);display:flex;align-items:center;gap:.6rem}"
    + ".dept-name{font-size:.95rem;font-weight:800;flex:1}"
    + ".dept-badge{font-size:.65rem;background:var(--g2);color:var(--text2);padding:2px 8px;border-radius:20px;font-weight:600}"
    + ".dept-events{padding:.5rem .75rem;display:flex;flex-direction:column;gap:.35rem;flex:1}"
    + ".dept-ev-row{display:flex;align-items:center;gap:.5rem;padding:.35rem .5rem;border-radius:5px;background:var(--g2);font-size:.8rem;text-decoration:none;color:var(--text);transition:background .1s}"
    + ".dept-ev-row:hover{background:var(--g3)}"
    + ".dept-ev-date{font-size:.72rem;color:var(--text2);white-space:nowrap;min-width:52px}"
    + ".dept-ev-title{flex:1;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}"
    + ".active-dot{width:7px;height:7px;border-radius:50%;background:var(--y);flex-shrink:0;animation:pulse 2s ease-in-out infinite}"
    + "@keyframes pulse{0%,100%{opacity:1}50%{opacity:.3}}"
    + ".type-stand{background:" + (isLight ? "#dcfce7" : "#1a4a1a") + ";color:" + (isLight ? "#15803d" : "#4caf50") + ";font-size:.58rem;padding:1px 5px;border-radius:10px;font-weight:700;text-transform:uppercase;flex-shrink:0}"
    + ".type-kurs{background:" + (isLight ? "#dbeafe" : "#1a2a4a") + ";color:" + (isLight ? "#1e40af" : "#64b5f6") + ";font-size:.58rem;padding:1px 5px;border-radius:10px;font-weight:700;text-transform:uppercase;flex-shrink:0}"
    + ".type-tur{background:" + (isLight ? "#f3e8ff" : "#3a1a4a") + ";color:" + (isLight ? "#6d28d9" : "#c084fc") + ";font-size:.58rem;padding:1px 5px;border-radius:10px;font-weight:700;text-transform:uppercase;flex-shrink:0}"
    + ".dept-footer{padding:.5rem 1rem;border-top:1px solid var(--border);display:flex;align-items:center;justify-content:space-between;font-size:.72rem;color:var(--text2)}"
    + ".ug-chips{display:flex;flex-wrap:wrap;gap:.25rem;padding:.5rem .75rem;border-top:1px solid var(--border)}"
    + ".ug-chip{font-size:.65rem;font-weight:700;padding:2px 8px;border-radius:20px;text-decoration:none;transition:opacity .15s}"
    + ".ug-chip:hover{opacity:.8}"
    + "footer{text-align:center;padding:2rem 1rem;font-size:.75rem;color:var(--muted)}";

  var logoHtml = logoUrl
    ? '<img src="' + escHtml(logoUrl) + '" style="height:32px;object-fit:contain;border-radius:3px" alt="' + escHtml(siteName) + '"/>'
    : '<span class="org-logo">' + escHtml((siteName.split(" ")[0] || "Events")) + '</span>';

  var deptCards = depts.map(function(dept) {
    var deptUrl = "https://" + (dept.slug || "") + "." + domain;
    var deptEvents = events.filter(function(ev) {
      return ev.department === dept.id && ev.isPublic !== false && ev.date && new Date(ev.date).getTime() >= ago7;
    }).sort(function(a, b) { return new Date(a.date) - new Date(b.date); });

    // Active events first
    var active = deptEvents.filter(function(ev) {
      ...
      return sm <= now && em >= now;
    });
    var upcoming = deptEvents.filter(function(ev) { return new Date(ev.date).getTime() > now; }).slice(0, 4);
    var shown = active.concat(upcoming).slice(0, 5);

    var evRows = shown.map(function(ev) {
      var evUrl = "https://" + ev.slug + "." + domain;
      var evDate = new Date(ev.date);
      var dateStr = evDate.toLocaleDateString("nb-NO", { day: "numeric", month: "short" });
      var timeStr = ev.date.length > 10 ? evDate.toLocaleTimeString("nb-NO", { hour: "2-digit", minute: "2-digit" }) : "";
      var isAct = active.some(function(a) { return a.id === ev.id; });
      ...
      return '<a class="dept-ev-row" href="' + escHtml(evUrl) + '">'
        + (isAct ? '<span class="active-dot"></span>' : '<span style="width:7px;height:7px;flex-shrink:0"></span>')
        + '<span class="dept-ev-date">' + dateStr + (timeStr ? " " + timeStr : "") + '</span>'
        + '<span class="dept-ev-title">' + escHtml(ev.title) + '</span>'
        + '<span class="' + typeCls + '">' + typeLabel + '</span>'
        + '</a>';
    }).join("");

    var emptyMsg = '<div style="padding:.75rem .5rem;font-size:.8rem;color:var(--muted);font-style:italic">Ingen kommende arrangementer</div>';

    // Subgroup chips
    var ugChips = (dept.subgroups || []).map(function(ug) {
      var ugSlugPart = (ug.slug || ug.name.toLowerCase().replace(/æ/g,'ae').replace(/ø/g,'o').replace(/å/g,'a').replace(/[^a-z0-9]+/g,'-').replace(/(^-|-$)/g,''));
      var ugUrl = "https://" + dept.slug + "-" + ugSlugPart + "." + domain;
      var dotColor = ug.color || accent;
      return '<a class="ug-chip" href="' + escHtml(ugUrl) + '" style="background:' + dotColor + '22;color:' + dotColor + ';border:1px solid ' + dotColor + '">'
        + escHtml(ug.name) + '</a>';
    }).join("");

    var totalKommende = deptEvents.filter(function(ev) { return new Date(ev.date).getTime() > now; }).length;

    return '<a class="dept-card" href="' + escHtml(deptUrl) + '">'
      + '<div class="dept-card-head">'
      + '<div class="dept-name">' + escHtml(dept.name) + '</div>'
      + (active.length ? '<span class="dept-badge" style="background:' + accent + '22;color:' + accent + ';border:1px solid ' + accent + '">' + active.length + ' pågående</span>' : '')
      + '</div>'
      + '<div class="dept-events">' + (shown.length ? evRows : emptyMsg) + '</div>'
      + (ugChips ? '<div class="ug-chips">' + ugChips + '</div>' : '')
      + '<div class="dept-footer">'
      + '<span>' + totalKommende + ' kommende</span>'
      + '<span style="color:var(--y);font-size:.8rem">Se alle →</span>'
      + '</div>'
      + '</a>';
  }).join("");

  var html = '<!DOCTYPE html><html lang="no" data-theme="' + theme + '"><head>'
    ...
    + logoHtml
    + '<div><div class="org-title">' + escHtml(siteName) + '</div><div class="org-sub">Aktivitetsoversikt</div></div>'
    + '</div>'
    + '<div class="container">'
    + (depts.length === 0
        ? '<div style="text-align:center;padding:4rem 1rem;color:var(--muted)"><div style="font-size:2.5rem;margin-bottom:.75rem">🏢</div><p>Ingen avdelinger er registrert ennå.</p></div>'
        : '<div class="dept-grid">' + deptCards + '</div>')
    + '</div>'
    + '<footer>' + escHtml(siteName) + '</footer>'
    + '<script>(function(){'
    + 'var es=new EventSource("/api/events/stream");'
    + 'es.onmessage=function(e){try{var m=JSON.parse(e.data);if(m.type==="events_updated")location.reload();}catch(err){}};'
    + 'es.onerror=function(){es.close();setTimeout(function(){location.reload();},10000);};'
    + '})()</s' + 'cript>'
    + '</body></html>';
...
function serveDepartmentPage(dept, subgroup, req, res) {
  var events   = readJSON(EVENTS_FILE);
  var settings = getSettings();
  var siteName = settings.siteName || "Events Admin";
  var domain   = settings.eventDomain || DOMAIN;
  var theme    = settings.theme || 'dark';
  var accentColor = (settings.colors && settings.colors.accent) || '#FFD100';
  var now      = Date.now();
  var ago30    = now - 7 * 24 * 60 * 60 * 1000;

  // Filter by department (+ optional subgroup) + show 7 days back
  var filtered = events.filter(function(ev) {
    if (ev.department !== dept.id) return false;
    if (subgroup && ev.subgroup !== subgroup.id) return false;
  ...
  var isLight = theme === 'light';
  var cssVars = isLight
    ? '--y:' + accentColor + ';--g:#ffffff;--g2:#e8eaed;--g3:#c8cad0;--b:#f0f2f5;--r:8px;--text:#1a1a1a;--text2:#555;--muted:#888;--border:#c8cad0;--card:#ffffff;--header:#ffffff'
    : '--y:' + accentColor + ';--g:#1e1e1e;--g2:#2a2a2a;--g3:#333;--b:#111;--r:8px;--text:#f0f0f0;--text2:#aaa;--muted:#666;--border:#2a2a2a;--card:#1e1e1e;--header:#1a1a1a';
  var css = '*{box-sizing:border-box;margin:0;padding:0}'
    + 'body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:var(--b);color:var(--text);min-height:100vh}'
    + ':root{' + cssVars + '}'
    + '.header{background:var(--header);border-bottom:3px solid var(--y);display:flex;align-items:stretch;gap:0;min-height:90px}'
    + '.header-brand{display:flex;align-items:center;gap:.75rem;flex-shrink:0;padding:.85rem 1.5rem;border-right:1px solid var(--border)}'
    ...
    + '.header-sub{font-size:.75rem;color:var(--text2);white-space:nowrap}'
    ...
    + '.col-count{font-size:.72rem;background:var(--g3);color:var(--text2);padding:1px 7px;border-radius:20px;margin-left:auto}'
    + '.ev-card{display:flex;align-items:stretch;background:var(--g);border-radius:var(--r);margin-bottom:.6rem;text-decoration:none;color:var(--text);overflow:hidden;border:1px solid var(--border);transition:border-color .15s,transform .1s}'
    ...
    + '.today-badge{font-size:.6rem;font-weight:800;padding:2px 7px;border-radius:20px;background:var(--g3);color:var(--y);text-transform:uppercase;letter-spacing:.5px}'
    ...
    + '.ev-date-col{width:52px;min-height:70px;background:var(--g2);display:flex;flex-direction:column;align-items:center;justify-content:center;flex-shrink:0;padding:.4rem 0}'
    + '.ev-day{font-size:1.4rem;font-weight:900;line-height:1;color:var(--y)}'
    + '.ev-month{font-size:.65rem;text-transform:uppercase;color:var(--text2);letter-spacing:.5px}'
    + '.ev-year{font-size:.6rem;color:var(--muted)}'
    ...
    + '.type-stand{background:' + (isLight?'#dcfce7':'#1a4a1a') + ';color:' + (isLight?'#15803d':'#4caf50') + '}'
    + '.type-kurs{background:' + (isLight?'#dbeafe':'#1a2a4a') + ';color:' + (isLight?'#1e40af':'#64b5f6') + '}'
    + '.type-tur{background:' + (isLight?'#f3e8ff':'#3a1a4a') + ';color:' + (isLight?'#6d28d9':'#c084fc') + '}'
    + '.past-badge{font-size:.6rem;color:var(--muted);background:var(--g2);padding:2px 6px;border-radius:20px}'
    + '.ug-badge{font-size:.6rem;color:var(--text2);background:var(--g2);border:1px solid var(--g3);padding:2px 6px;border-radius:20px}'
    + '.ev-title{font-size:.88rem;font-weight:700;margin-bottom:.2rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}'
    + '.ev-meta{font-size:.72rem;color:var(--text2);margin-top:.1rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}'
    + '.ev-arrow{display:flex;align-items:center;padding:0 .6rem;color:var(--g3);font-size:1rem;flex-shrink:0}'
    + '.ev-card:hover .ev-arrow{color:var(--y)}'
    + '.empty-col{text-align:center;padding:2rem .5rem;color:var(--muted);font-size:.82rem}'
    + 'footer{text-align:center;padding:2rem 1rem;font-size:.75rem;color:var(--muted)}'  + '.banner-wrap{background:transparent;position:relative;height:100%;display:flex;align-items:stretch;width:100%}'  + '.banner-track{display:flex;will-change:transform;align-items:stretch;height:100%;padding:.5rem 0;box-sizing:border-box;gap:1rem}'  + '#bannerTrack .banner-item{flex:0 0 auto}'  + '.banner-track.no-anim{width:100%;display:flex}'  + '.banner-track.no-anim{animation:none!important}'  + '@keyframes banner-scroll{0%{transform:translateX(0)}100%{transform:translateX(-50%)}}'  + '.banner-item{display:flex;align-items:stretch;min-width:0;text-decoration:none;color:#fff;transition:background .15s,transform .1s;height:100%;border-radius:var(--r);overflow:hidden}'  + '.banner-item:hover{background:var(--g)}'  + '.banner-item.is-active{border:1px solid var(--y);animation:pulse 2s ease-in-out infinite}'  + '.banner-item.is-next{border:1px solid #2a2a2a}';
  ...
  // ── HTML ─────────────────────────────────────────────────────────
  // Build subgroup navigation chips
  var deptBaseUrl = 'https://' + (dept.slug || '') + '.' + domain;
  var orgUrl = 'https://' + domain;
  var subgroups = dept.subgroups || [];
  var subgroupNav = '';
  if (subgroups.length > 0) {
    var chips = subgroups.map(function(ug) {
      var ugSlugPart = (ug.slug || ug.name.toLowerCase().replace(/æ/g,'ae').replace(/ø/g,'o').replace(/å/g,'a').replace(/[^a-z0-9]+/g,'-').replace(/(^-|-$)/g,''));
      var ugUrl = 'https://' + dept.slug + '-' + ugSlugPart + '.' + domain;
      var isActive = subgroup && subgroup.id === ug.id;
      var dotColor = ug.color || 'var(--y)';
      return '<a href="' + escHtml(ugUrl) + '" style="display:inline-flex;align-items:center;gap:.35rem;padding:5px 12px;border-radius:20px;font-size:.75rem;font-weight:700;text-decoration:none;border:2px solid ' + (isActive ? dotColor : 'var(--border)') + ';color:' + (isActive ? dotColor : 'var(--text2)') + ';background:' + (isActive ? dotColor + '22' : 'transparent') + ';transition:all .15s">'
        + '<span style="width:7px;height:7px;border-radius:50%;background:' + dotColor + ';flex-shrink:0"></span>'
        + escHtml(ug.name) + '</a>';
    }).join('');
    var allUrl = subgroup ? deptBaseUrl : '#';
    var allActive = !subgroup;
    var allChip = '<a href="' + escHtml(allUrl) + '" style="display:inline-flex;align-items:center;gap:.35rem;padding:5px 12px;border-radius:20px;font-size:.75rem;font-weight:700;text-decoration:none;border:2px solid ' + (allActive ? 'var(--y)' : 'var(--border)') + ';color:' + (allActive ? 'var(--y)' : 'var(--text2)') + ';background:' + (allActive ? 'rgba(245,197,0,.1)' : 'transparent') + '">Alle</a>';
    subgroupNav = '<div style="padding:.6rem 1rem;background:var(--header);border-bottom:1px solid var(--border);display:flex;flex-wrap:wrap;gap:.4rem;align-items:center">'
      + allChip + chips + '</div>';
  }

  // Breadcrumb for subgroup view
  var breadcrumb = '';
  if (subgroup) {
    breadcrumb = '<div style="padding:.4rem 1rem;background:var(--b);border-bottom:1px solid var(--border);font-size:.75rem;color:var(--text2)">'
      + '<a href="' + escHtml(orgUrl) + '" style="color:var(--text2);text-decoration:none">' + escHtml(siteName) + '</a>'
      + ' → <a href="' + escHtml(deptBaseUrl) + '" style="color:var(--text2);text-decoration:none">' + escHtml(dept.name) + '</a>'
      + ' → <span style="color:var(--text)">' + escHtml(subgroup.name) + '</span>'
      + '</div>';
    ...
    breadcrumb = '<div style="padding:.4rem 1rem;background:var(--b);border-bottom:1px solid var(--border);font-size:.75rem;color:var(--text2)">'
      + '<a href="' + escHtml(orgUrl) + '" style="color:var(--text2);text-decoration:none">' + escHtml(siteName) + '</a>'
      + ' → <span style="color:var(--text)">' + escHtml(dept.name) + '</span>'
      + '</div>';
  }

  var pageTitle = subgroup ? (escHtml(dept.name) + ' – ' + escHtml(subgroup.name)) : escHtml(dept.name);
  var html = '<!DOCTYPE html><html lang="no" data-theme="' + theme + '"><head>'
    + '<meta charset="UTF-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/>'
    + '<title>' + pageTitle + ' – ' + escHtml(siteName) + '</title>'
    ...
    + breadcrumb
    + '<div class="header">'
    + '<div class="header-brand" style="position:relative;z-index:2;background:var(--header)">'
    + '<a href="' + escHtml(orgUrl) + '" style="text-decoration:none"><div class="header-logo">'+(settings && settings.siteName ? escHtml(settings.siteName.split(' ')[0]) : 'Events')+'</div></a>'
    + '<div>'
    + '<div class="header-title"><a href="' + escHtml(deptBaseUrl) + '" style="color:inherit;text-decoration:none">' + escHtml(dept.name) + '</a>' + (subgroup ? ' <span style="color:var(--text2);font-weight:400">/ ' + escHtml(subgroup.name) + '</span>' : '') + '</div>'
    + '<div class="header-sub">' + escHtml(siteName) + '</div>'
    + '</div>'
    + '</div>'
...
function tabletCSS(accentColor, theme) {
  ...
  const vars = isLight
    ? `--y:${y};--yd:color-mix(in srgb,${y} 70%,#000);--b:#f0f2f5;--g:#ffffff;--g2:#e8eaed;--g3:#c8cad0;--red:#E2001A;--text:#1a1a1a;--text2:#555;--ok:#166534;--ok-bg:#f0fdf4;--ok-border:#86efac;--err:#b91c1c;--err-bg:#fef2f2;--err-border:#fca5a5;--warn:#92640a;--warn-bg:#fffbeb;--warn-border:#fcd34d;--info:#1e40af;--info-bg:#eff6ff;--info-border:#93c5fd;--purp:#6d28d9;--purp-bg:#f5f3ff;--purp-border:#c4b5fd`
    : `--y:${y};--yd:color-mix(in srgb,${y} 70%,#000);--b:#111;--g:#1c1c1c;--g2:#252525;--g3:#333;--red:#E2001A;--text:#f0f0f0;--text2:#aaa;--ok:#4ade80;--ok-bg:#0a2a0a;--ok-border:#166534;--err:#f87171;--err-bg:#2a0a0a;--err-border:#7f1d1d;--warn:#f5c500;--warn-bg:#1a1500;--warn-border:#3a3000;--info:#7dc8ff;--info-bg:#0d1a2a;--info-border:#1e4a6a;--purp:#d8b4fe;--purp-bg:#1a0a2e;--purp-border:#5b3a7a`;
  return `<style>
:root{${vars}}
...
.ev-hero-content{position:relative;padding:1.5rem 1.25rem 1.25rem;background:linear-gradient(transparent,rgba(0,0,0,.75) 40%)}
...
.badge-mote{background:var(--warn-bg);color:var(--warn);border:1px solid var(--warn-border)}
.badge-kurs{background:var(--info-bg);color:var(--info);border:1px solid var(--info-border)}
.badge-tur{background:var(--purp-bg);color:var(--purp);border:1px solid var(--purp-border)}
.badge-stand{background:var(--ok-bg);color:var(--ok);border:1px solid var(--ok-border)}
.ev-title{font-size:1.7rem;font-weight:900;line-height:1.15;color:var(--text)}
.ev-meta{display:flex;flex-wrap:wrap;gap:.6rem 1.5rem;margin-top:.6rem;font-size:.88rem;color:var(--text2)}
...
.msg-info{background:var(--info-bg);border:1px solid var(--info-border);color:var(--info)}
/* Spots / count */
...
.spots-ok{color:var(--ok);font-weight:700}
...
.checkin-done{color:var(--ok);font-size:.85rem;font-weight:700}
...
.gb-msg{font-size:.92rem;color:var(--text);line-height:1.6}
...
/* Lottery */
.lottery-box{background:var(--g);border:2px solid var(--y);border-radius:12px;padding:2rem;text-align:center}
...
.winner-pop{background:var(--ok-bg);border:2px solid var(--ok);border-radius:10px;padding:1.25rem;margin:1rem 0;animation:wpop .4s cubic-bezier(.175,.885,.32,1.275)}
@keyframes wpop{0%{transform:scale(.7);opacity:0}100%{transform:scale(1);opacity:1}}
.winner-name{font-size:1.6rem;font-weight:900;color:var(--ok)}
.past-winner{display:flex;justify-content:space-between;background:var(--b);border-radius:6px;padding:.6rem .9rem;margin-bottom:.3rem;font-size:.88rem}
/* Offline stripe */
...
function tabletPinSheet(ev) {
  ...
  // Build shift lookup: staffId → shift
  const shiftMap = {};
  shifts.forEach(function(sh) {
    (sh.staffIds || []).forEach(function(sid) {
      shiftMap[sid] = sh;
    });
  });
  ...
  const staffRows = staff.length
    ? staff.map(function(s) {
        ...
        const shiftLabel = sh
          ? `<div class="staff-role" style="color:var(--y);font-size:.72rem">⏱ ${escHtml(sh.startTime||'')}–${escHtml(sh.endTime||'')}</div>`
          : '';
        return `<div class="staff-item" id="si-${escHtml(s.id)}" data-name="${escHtml((s.name||'').toLowerCase())}">
          ...
          ${s.checkedIn
            ...
            : `<button class="checkin-btn" onclick="staffCheckIn('${escHtml(s.id)}')">Møtt opp</button>`}
        </div>`;
      }).join('')
...
<div class="pin-overlay" id="staffOverlay">
  <div class="pin-sheet" style="position:relative;max-height:85vh;overflow-y:auto">
    ...
    <hr style="border-color:var(--g3);margin:1.25rem 0"/>
    <div style="font-size:.72rem;font-weight:800;color:var(--y);text-transform:uppercase;letter-spacing:.5px;margin-bottom:.75rem">Ikke på listen? Registrer deg</div>
    <div id="staffMsg"></div>
    <div class="ev-form">
      <label>Navn *</label>
      <input id="sName" type="text" placeholder="Ditt fulle navn" autocomplete="name"/>
      <label>E-post <span style="font-size:.75rem;color:var(--text2)">(for bekreftelse og timeregistrering)</span></label>
    ...
    <button class="btn-primary" onclick="selfCheckIn()" style="margin-top:.75rem">✅ Registrer oppmøte</button>
  </div>
</div>`;
}
...
function tabletSharedJS(ev) {
  const hasPin = !!ev.staffPin;
  return `<script src="https://cdnjs.cloudflare.com/ajax/libs/qrcode-generator/1.4.4/qrcode.min.js"><\/script><script>
var EV_ID="${ev.id}";
...
async function staffCheckIn(id){
  ...
  var r=await fetch('/api/events/'+EV_ID+'/staff/checkin',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({staffId:id})});
  var d=await r.json();
  if(r.ok){
    ...
    // Show confirmation with hours info
    var msg=document.getElementById('staffMsg');
    if(msg&&d.hoursEntry){
      var hrs=d.hoursEntry.hours?(' – '+d.hoursEntry.hours+'t'):'';
      msg.innerHTML='<div class="msg msg-ok" style="margin-bottom:.75rem">✅ Innsjekket'+hrs+'<br><span style="font-size:.78rem;opacity:.8">Timer registrert og venter godkjenning</span></div>';
    }
  } else {
    if(btn){btn.disabled=false;btn.textContent='Møtt opp';}
  }
}
...
async function selfCheckIn(){
  ...
  var btn=document.querySelector('#staffOverlay .btn-primary:last-of-type');
  if(btn){btn.disabled=true;btn.textContent='Registrerer…';}
  var r=await fetch('/api/events/'+EV_ID+'/staff/checkin',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name,email,role,department:dept})});
  var d=await r.json();
  if(btn){btn.disabled=false;btn.textContent='✅ Registrer oppmøte';}
  if(r.ok){
    var confirmLine=email?'<br><span style="font-size:.78rem;opacity:.8">Bekreftelse sendt til '+esc(email)+'</span>':'';
    if(msg)msg.innerHTML='<div class="msg msg-ok">✅ Velkommen, '+esc(name)+'!'+confirmLine+'<br><span style="font-size:.78rem;opacity:.8">Timer registrert og venter godkjenning</span></div>';
    document.getElementById('sName').value='';
    if(document.getElementById('sEmail'))document.getElementById('sEmail').value='';
    if(document.getElementById('sRole'))document.getElementById('sRole').value='';
  } else {
    if(msg)msg.innerHTML='<div class="msg msg-err">'+esc(d.error||'Feil')+'</div>';
  }
}
...
// ── QR-generator via qrcodejs (cdnjs, ingen tracking) ───────────
window.drawQRCanvas=function(text,canvas,px){
  // Bruk qrcode-generator bibliotek
  var qr=qrcode(0,'M');
  ...
  ctx.fillStyle='#000';
  for(var r=0;r<N;r++)for(var c=0;c<N;c++)if(qr.isDark(r,c))ctx.fillRect((c+4)*Q,(r+4)*Q,Q,Q);
  return true;
};
<\/script>`;
}
...
function buildStandPage(ev, isAuthenticated) {
  ...
  // Tema-bevisste fargehjelper for template strings
  const isLt = theme === 'light';
  const TC_B = isLt?'#f0f2f5':'#111';
  const TC_BG = isLt?'#f0f2f5':'#1e1e1e';
  const TC_CARD = isLt?'#ffffff':'#1e1e1e';
  const TC_G = isLt?'#ffffff':'#1c1c1c';
  const TC_G3 = isLt?'#c8cad0':'#333';
  const TC_BORDER = isLt?'#c8cad0':'#2a2a2a';
  const TC_TEXT = isLt?'#1a1a1a':'#f0f0f0';
  const TC_TEXT2 = isLt?'#555':'#aaa';
  const TC_MUTED = isLt?'#666':'#888';
  const TC_OK = isLt?'#166534':'#4ade80';
  const TC_OK_BG = isLt?'#f0fdf4':'#0a2a0a';
  const TC_OK_BORDER = isLt?'#86efac':'#166534';
  const TC_ERR = isLt?'#b91c1c':'#f87171';
  const TC_ERR_BG = isLt?'#fef2f2':'#2a0a0a';
  const TC_ERR_BORDER = isLt?'#fca5a5':'#7f1d1d';
  const TC_WARN = isLt?'#92640a':'#f5c500';
  const TC_WARN_BG = isLt?'#fffbeb':'#1a1500';
  const TC_WARN_BORDER = isLt?'#fcd34d':'#3a3000';
  const TC_INFO = isLt?'#1e40af':'#7dc8ff';
  const TC_INFO_BG = isLt?'#eff6ff':'#0d1a2a';
  const TC_INFO_BORDER = isLt?'#93c5fd':'#1e4a6a';
  const TC_PURP = isLt?'#6d28d9':'#d8b4fe';
  const TC_PURP_BG = isLt?'#f5f3ff':'#1a0a2e';
  const TC_PURP_BORDER = isLt?'#c4b5fd':'#5b3a7a';
    ...
    ? gb.map(function(g){
        ...
        const photoHtml = photos.length
          ? '<div style="display:flex;flex-wrap:wrap;gap:.4rem;margin:.6rem 0">'
            + photos.map(function(p){ return '<a href="'+p+'" target="_blank" rel="noopener"><img src="'+p+'" style="width:80px;height:80px;object-fit:cover;border-radius:6px;border:1px solid ${TC_G3}"/></a>'; }).join('')
            + '</div>'
          : '';
...
<div class="ev-content">
  ...
  <div class="ev-panel active" id="tab-info">
    ${ev.description ? `<div class="ev-card"><div class="ev-card-title">Om standen</div><p style="line-height:1.7;color:${TC_TEXT}">${escHtml(ev.description).replace(/\n/g,'<br>')}</p></div>` : ''}
    <div class="ev-card">
  ...
  <div class="ev-panel" id="tab-reg">
    <div class="ev-card">
      ...
      ${regs.length > 0 ? `<div style="text-align:center;margin-top:1rem;font-size:.85rem;color:${TC_MUTED}">${regs.length} påmeldte</div>` : ''}
  ...
  <div class="ev-panel" id="tab-lottery">
    <div class="lottery-box">
      ...
      <div id="lWinner"></div>
      <div id="lEligible" style="font-size:.85rem;color:${TC_MUTED};margin-top:.75rem"></div>
    </div>
    <div id="pastWrap" style="margin-top:1rem;display:none">
      <div style="font-size:.72rem;text-transform:uppercase;letter-spacing:.5px;color:${TC_MUTED};margin-bottom:.5rem">Tidligere vinnere</div>
      <div id="pastList"></div>
    </div>
  </div>` : ''}
  ...
  <div class="ev-panel" id="tab-gb">
    ...
    <div class="ev-card" style="margin-top:1rem">
      ...
      <p style="font-size:.75rem;color:${TC_MUTED};margin-top:.75rem">Kommentarer godkjennes før de vises.</p>
    </div>
  </div>
...
async function submitGb(){
  ...
  if(r.ok){
    ...
    if(d.id) showGbQrBtn(d.id);
  } else {
    msg.innerHTML='<div class="msg msg-err">'+esc(d.err||d.error||'Feil')+'</div>';
  }
}

function showGbQrBtn(gbId){
  var existing=document.getElementById('gbQrBtn');
  if(existing){clearTimeout(existing._timer);existing.remove();}
  var btn=document.createElement('button');
  btn.id='gbQrBtn';
  btn.innerHTML='📸 Legg ved bilde via QR-kode';
  btn.style.cssText='width:100%;margin-top:.6rem;padding:14px;background:none;color:var(--y);font-weight:700;font-size:.95rem;border:2px solid var(--y);border-radius:8px;cursor:pointer;min-height:52px';
  btn.onclick=function(){ clearTimeout(btn._timer); openGbQrModal(gbId); btn.remove(); };
  var form=document.querySelector('#tab-gb .ev-card');
  if(form) form.appendChild(btn);
  btn._timer=setTimeout(function(){ if(btn.parentNode) btn.remove(); }, 10000);
}

async function openGbQrModal(gbId){
  var tr=await fetch('/api/events/'+EV_ID+'/guestbook/'+gbId+'/photo-token',{method:'POST'});
  if(!tr.ok) return;
  var td=await tr.json();
  var uploadUrl=location.origin+'/gb-photo/'+td.token;
  var overlay=document.createElement('div');
  overlay.id='gbQrOverlay';
  ...
  overlay.innerHTML='<div style="background:var(--g);border-radius:20px 20px 0 0;width:100%;max-width:540px;padding:2rem 1.5rem;padding-bottom:calc(2rem + env(safe-area-inset-bottom));text-align:center;position:relative">'
    +'<button onclick="closeGbQr()" style="position:absolute;top:1rem;right:1rem;background:none;border:none;font-size:1.5rem;color:var(--text2);cursor:pointer;padding:4px 10px">×</button>'
    +'<div style="font-size:1.1rem;font-weight:800;margin-bottom:.4rem">📸 Legg ved bilde</div>'
    +'<p style="font-size:.85rem;color:var(--text2);margin-bottom:1.25rem;line-height:1.5">Skann QR-koden og last opp ett eller flere bilder fra mobilen.</p>'
    +'<div style="display:inline-block;padding:12px;background:#fff;border-radius:10px;margin-bottom:1rem">'
    +'<canvas id="gbQrCanvas" style="display:block;image-rendering:pixelated"></canvas>'
    +'</div>'
    +'<div id="gbQrStatus" style="font-size:.85rem;color:var(--text2);min-height:1.4rem;margin-bottom:.5rem">Venter på bilde…</div>'
    +'<div id="gbQrPhotos" style="display:flex;flex-wrap:wrap;gap:.4rem;justify-content:center;margin-bottom:.75rem"></div>'
    +'<button onclick="closeGbQr()" style="width:100%;padding:14px;background:none;color:var(--text);font-weight:700;font-size:.95rem;border:2px solid var(--g3);border-radius:8px;cursor:pointer">Ferdig</button>'
    +'</div>';
  document.body.appendChild(overlay);
  var canvas=document.getElementById('gbQrCanvas');
  if(canvas&&window.drawQRCanvas){drawQRCanvas(uploadUrl,canvas,6);}
  var sse=new EventSource('/api/gb-photo-wait/'+td.token);
  sse.onmessage=function(e){
    var data=JSON.parse(e.data);
    if(data.ok){
      var st=document.getElementById('gbQrStatus');
      var grid=document.getElementById('gbQrPhotos');
      if(st)st.innerHTML='<span style="color:#7dff7d">✅ '+data.count+' bilde'+(data.count===1?'':'r')+' lastet opp</span>';
      if(grid&&data.photoUrl){
        var img=document.createElement('img');
        img.src=data.photoUrl;
        img.style.cssText='width:64px;height:64px;object-fit:cover;border-radius:6px;border:2px solid ${TC_OK}';
...
function closeGbQr(){
  ...
  if(overlay){if(overlay._sse)overlay._sse.close();overlay.remove();}
}
...
<div class="ev-content">
  ...
  <!-- Info -->
  <div class="ev-panel active" id="tab-info">
    ${ev.description ? `<div class="ev-card"><div class="ev-card-title">Om møtet</div><p style="line-height:1.7;color:${TC_TEXT}">${escHtml(ev.description).replace(/\n/g,'<br>')}</p></div>` : ''}
    <div class="ev-card">
      <div class="ev-card-title">Praktisk info</div>
      ${ev.date ? `<div class="info-row"><span class="icon">📅</span><div><div class="label">Dato og tid</div><div class="value">${evFmtDate(ev.date)}${ev.date ? ' kl. ' + evFmtTime(ev.date) : ''}</div></div></div>` : ''}
      ${ev.location ? `<div class="info-row"><span class="icon">📍</span><div><div class="label">Sted</div><div class="value">${escHtml(ev.location)}</div></div></div>` : ''}
  ...
  <div class="ev-panel" id="tab-oppmote">
    <div class="ev-card">
      ...
      <div style="text-align:center;color:${TC_MUTED};font-size:.85rem;margin-top:.75rem">${regs.filter(r=>!r.anonymized).length} påmeldte · ${regs.filter(r=>r.checkedIn&&!r.anonymized).length} sjekket inn</div>
    </div>
  </div>
...
<div class="ev-content">
  ...
  <!-- Info -->
  <div class="ev-panel active" id="tab-info">
    ${ev.description ? `<div class="ev-card"><div class="ev-card-title">Om kurset</div><p style="line-height:1.7;color:${TC_TEXT}">${escHtml(ev.description).replace(/\n/g,'<br>')}</p></div>` : ''}
    <div class="ev-card">
      <div class="ev-card-title">Praktisk info</div>
      ${ev.date ? `<div class="info-row"><span class="icon">📅</span><div><div class="label">Dato og tid</div><div class="value">${evFmtDate(ev.date)}${ev.date ? ' kl. ' + evFmtTime(ev.date) : ''}</div></div></div>` : ''}
      ${ev.location ? `<div class="info-row"><span class="icon">📍</span><div><div class="label">Sted</div><div class="value">${escHtml(ev.location)}</div></div></div>` : ''}
  ...
  <div class="ev-panel" id="tab-oppmote">
    <div class="ev-card">
      ...
      <div style="text-align:center;color:${TC_MUTED};font-size:.85rem;margin-top:.75rem">
    ...
    <div id="gbEntries">
      ${(ev.guestbook||[]).filter(g=>g.approved).length
        ? (ev.guestbook||[]).filter(g=>g.approved).map(function(g){
            const ph=(g.photos||[]).length?'<div style="display:flex;flex-wrap:wrap;gap:.4rem;margin:.6rem 0">'+(g.photos||[]).map(function(p){return'<a href="'+p+'" target="_blank" rel="noopener"><img src="'+p+'" style="width:80px;height:80px;object-fit:cover;border-radius:6px;border:1px solid ${TC_G3}"/></a>';}).join('')+'</div>':'';return `<div class="gb-entry"><div class="gb-name">${escHtml(g.name)}</div><div class="gb-msg">${escHtml(g.message).replace(/\n/g,'<br>')}</div>${ph}<div class="gb-date">${new Date(g.createdAt).toLocaleDateString('nb-NO',{day:'numeric',month:'long',year:'numeric'})}</div></div>`;
          }).join('')
...
async function submitGb(){
  ...
  var r=await fetch('/api/events/'+EV_ID+'/guestbook',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name,message:msg2})});
  var d=await r.json();
  if(r.ok){msg.innerHTML='<div class="msg msg-ok">✅ Takk! Godkjennes av administrator.</div>';document.getElementById('gbName').value='';document.getElementById('gbMsg2').value='';if(d.id) showGbQrBtn(d.id);}
  else{msg.innerHTML='<div class="msg msg-err">'+esc(d.err||d.error||'Feil')+'</div>';}
}
...
function buildTurPage(ev, isAuthenticated) {
  ...
  const opplevelseIcons = { museum:'🏛️', natur:'🌿', utsikt:'🏔️', historisk:'🏰', aktivitet:'🎭', kultur:'🎨', mat:'🍴', annet:'⭐' };
  const typeIcons      = { start:'🏁', stopp:'📍', lunsj:'🍽️', middag:'🍷', hotell:'🏨', slutt:'🏁', bensin:'⛽', opplevelse:'⭐' };
  // Color gradient per type – matching route planner palette
  const typeColors = {
    start:     accentColor,
    slutt:     accentColor,
    stopp:     isLt ? '#555' : '#aaa',
    ...
    hotell:    '#3b82f6',
    bensin:    '#ef4444',
    opplevelse:{ museum:'#8b5cf6', natur:'#22c55e', utsikt:'#06b6d4', historisk:'#f59e0b',
                 aktivitet:'#ec4899', kultur:'#f97316', mat:'#84cc16', annet:'#6366f1' },
  };
  function _etappeColor(e) {
    if (e.type === 'opplevelse') return typeColors.opplevelse[e.opplevelseSubtype] || typeColors.opplevelse.annet;
    return typeColors[e.type] || (isLt ? '#555' : '#aaa');
  }
  // Build flat list with colour for map + list
  const allEtapper = days.flatMap(function(day, di) {
    return (day.etapper || []).filter(function(e){ return e.fra || e.til; }).map(function(e, ei) {
      return Object.assign({}, e, { _dayIdx: di, _etIdx: ei,
        _dato: day.dato || null, _color: _etappeColor(e) });
    });
  });
  const etappeHtml = days.length
    ? days.map(function(day, di){
        ...
        return `<div style="margin-bottom:1.5rem">
          <div style="font-size:.72rem;font-weight:800;text-transform:uppercase;letter-spacing:.5px;color:var(--y);margin-bottom:.5rem">${escHtml(dayLabel)}</div>
          ${etapper.map(function(e, ei){
            const icon  = e.type === 'opplevelse' ? (opplevelseIcons[e.opplevelseSubtype] || '⭐') : (typeIcons[e.type] || '📍');
            const color = _etappeColor(e);
            const globalIdx = allEtapper.findIndex(function(x){ return x._dayIdx===di && x._etIdx===ei; });
            return `<div class="etappe-item" id="ei-${globalIdx}" style="border-left-color:${color}">
              <div style="display:flex;align-items:flex-start;gap:.75rem">
                <div style="flex-shrink:0;width:28px;height:28px;border-radius:50%;background:${color}22;border:2px solid ${color};display:flex;align-items:center;justify-content:center;font-size:.85rem;margin-top:.1rem">${icon}</div>
                <div style="flex:1;min-width:0">
                  <div style="font-size:.68rem;font-weight:800;text-transform:uppercase;letter-spacing:.5px;color:${color};margin-bottom:.1rem">${escHtml(e.type || 'stopp')}</div>
                  <div class="etappe-name" style="font-size:.95rem">${escHtml(e.fra || '')}${e.fra && e.til ? ' <span style="opacity:.5">→</span> ' : ''}${escHtml(e.til || '')}</div>
                  ${e.km || e.notat ? `<div class="etappe-meta" style="margin-top:.3rem">
                    ${e.km ? `<span style="color:${color};font-weight:700">📏 ${e.km} km</span>` : ''}
                    ${e.notat ? `<span style="color:var(--text2)">${escHtml(e.notat)}</span>` : ''}
                  </div>` : ''}
                </div>
              </div>
            </div>`;
...
<style>
.leaflet-container { background: ${isLt ? '#e8e8e8' : '#1a1a1a'}; }
</style>
</head>
<body>
...
<div class="ev-content">
  ...
  <!-- Info -->
  <div class="ev-panel active" id="tab-info">
    ${ev.description ? `<div class="ev-card"><div class="ev-card-title">Om turen</div><p style="line-height:1.7;color:${TC_TEXT}">${escHtml(ev.description).replace(/\n/g,'<br>')}</p></div>` : ''}
    <div class="ev-card">
      <div class="ev-card-title">Praktisk info</div>
  ...
  <div class="ev-panel" id="tab-rute">
    ${days.length ? `
    <!-- Leaflet rutekart -->
    <div id="turMapWrap" style="border-radius:10px;overflow:hidden;margin-bottom:1rem;border:1px solid var(--g3);position:relative;background:var(--g2)">
      <div id="turMap" style="height:300px;width:100%"></div>
      <div id="turMapMsg" style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;font-size:.85rem;color:var(--text2);pointer-events:none">Laster kart…</div>
    </div>
    <div class="ev-card">
      <div class="ev-card-title">🗺️ Etappeplan</div>
      ${etappeHtml}
    </div>
    <!-- Navigasjonsknapp sticky bunn inne i panel -->
    <div style="position:sticky;bottom:0;padding:.5rem 0 calc(.5rem + env(safe-area-inset-bottom));background:linear-gradient(transparent,var(--b) 40%);margin-top:.5rem;text-align:center">
      <button id="navFabBtn" onclick="handleNavBtn()"
        style="display:inline-flex;align-items:center;justify-content:center;gap:.5rem;padding:10px 28px;background:var(--y);color:#111;font-weight:800;font-size:.9rem;border-radius:50px;border:none;cursor:pointer;box-shadow:0 3px 14px rgba(0,0,0,.35);min-height:44px;white-space:nowrap">
        🗺 Start turen i Google Maps
      </button>
    </div>` : '<div class="empty"><span class="empty-icon">🗺️</span>Ingen etappeplan lagt inn ennå</div>'}
  </div>
  ...
  <div class="ev-panel" id="tab-reg">
    <div class="ev-card">
      <div class="ev-card-title">📋 Påmelding</div>
      ${ev.isFinalized ? `<div class="msg" style="background:${TC_OK_BG};border:1px solid ${TC_OK_BORDER};color:${TC_OK};border-radius:6px;padding:.75rem 1rem;margin-bottom:.75rem;font-size:.9rem">✅ Turen er ferdigplanlagt! Du vil få en detaljert reisebeskrivelse på e-post.</div>` : ''}
...
(function(){
  ...
  script.onload=function(){ if(_mapInited) initTurMap(); };
  document.head.appendChild(script);
})();

var _turMap=null,_turBounds=[],_mapInited=false;

function ensureHex(c){
  if(!c||c[0]!='#')return '#888888';
  if(c.length===4)return '#'+c[1]+c[1]+c[2]+c[2]+c[3]+c[3];
  return c.length===7?c:'#888888';
}
function lerpColor(a,b,t){
  a=ensureHex(a);b=ensureHex(b);
...
function drawSegment(seg){
  if(!_turMap||!seg.coords||seg.coords.length<2)return;
  var n=seg.coords.length,cA=ensureHex(seg.colorA),cB=ensureHex(seg.colorB);
  for(var i=0;i<n-1;i++){
    L.polyline([seg.coords[i],seg.coords[i+1]],{
      color:lerpColor(cA,cB,n>1?i/(n-1):0),weight:5,opacity:.9,lineCap:'round',lineJoin:'round'
    }).addTo(_turMap);
    _turBounds.push(seg.coords[i]);
  }
  _turBounds.push(seg.coords[n-1]);
  if(_turBounds.length>1)_turMap.fitBounds(_turBounds,{padding:[28,28],maxZoom:12});
}
function drawStops(stops){
  if(!_turMap)return;
  stops.forEach(function(s){
    var sz=s.isEnd?16:11,r=sz/2;
    var icon=L.divIcon({className:'',
      html:'<div style="width:'+sz+'px;height:'+sz+'px;border-radius:50%;background:'+s.color+';border:2.5px solid rgba(255,255,255,.7);box-shadow:0 1px 6px rgba(0,0,0,.7)"></div>',
      iconSize:[sz,sz],iconAnchor:[r,r]});
    L.marker([s.lat,s.lon],{icon:icon}).bindPopup('<b>'+s.name+'</b>').addTo(_turMap);
  });
}
function setMapMsg(txt){
  var el=document.getElementById('turMapMsg');
  if(el){el.textContent=txt;el.style.display=txt?'flex':'none';}
}
function initTurMap(){
  if(!document.getElementById('turMap')||_turMap)return;
  var isDark=${theme==='dark'?'true':'false'};
  _turMap=L.map('turMap',{zoomControl:true,attributionControl:false,scrollWheelZoom:true});
  L.tileLayer(isDark
    ?'https://{s}.basemaps.cartocdn.com/dark_nolabels/{z}/{x}/{y}{r}.png'
    :'https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',
    {maxZoom:18,subdomains:'abcd'}).addTo(_turMap);
  _turMap.setView([65,14],5);
  var evId='${ev.id}';
  // Check if already cached
  fetch('/api/events/'+evId+'/map-data')
    .then(function(r){return r.json();})
    .then(function(d){
      if(d.ready&&d.segments&&d.segments.length){
        drawStops(d.stops||[]);
        d.segments.forEach(drawSegment);
        setMapMsg('');
      } else {
        // Stream via SSE
        setMapMsg('Geocoder steder…');
        var sse=new EventSource('/api/events/'+evId+'/map-data-stream');
        sse.onmessage=function(ev){
          try{
            var m=JSON.parse(ev.data);
            if(m.type==='status')setMapMsg(m.message);
            else if(m.type==='stops')drawStops(m.stops);
            else if(m.type==='segment'){drawSegment(m.segment);setMapMsg('');}
            else if(m.type==='done'){setMapMsg('');sse.close();}
            else if(m.type==='error'){setMapMsg('Feil: '+m.message);sse.close();}
          }catch(e){}
        };
        sse.onerror=function(){setMapMsg('Kart ikke tilgjengelig');sse.close();};
      }
    })
    .catch(function(){setMapMsg('Kart ikke tilgjengelig');});
}
var _origShowTab=showTab;
showTab=function(id){
  _origShowTab(id);
  if(id==='rute'&&!_mapInited){
    _mapInited=true;
    if(window.L)initTurMap();
  }
};
if(document.getElementById('tab-rute')&&document.getElementById('tab-rute').classList.contains('active')){
  _mapInited=true;
  var _lt=setInterval(function(){if(window.L){clearInterval(_lt);initTurMap();}},80);
}
<\/script>
</body>
</html>`;
...
app.post("/api/events/:id/giveaway", auth, managerOrAdmin, function(req, res) {
  ...
  const { itemId, utDelt, retur, tomtKl } = req.body;
  const existing = events[evIdx].giveaway.findIndex(g => g.id === itemId);
  const tomtKlVal = (typeof tomtKl === 'string' && tomtKl.trim()) ? tomtKl.trim() : null;
  if (existing >= 0) {
    events[evIdx].giveaway[existing].utDelt = parseInt(utDelt) || 0;
    events[evIdx].giveaway[existing].retur  = parseInt(retur)  || 0;
    if (tomtKlVal !== undefined) events[evIdx].giveaway[existing].tomtKl = tomtKlVal;
  } else {
    events[evIdx].giveaway.push({ id: itemId, utDelt: parseInt(utDelt)||0, retur: parseInt(retur)||0, tomtKl: tomtKlVal });
  }
  writeJSON(EVENTS_FILE, events);
  broadcastEventUpdate(ev.department);
  res.json({ ok: true });
...
app.post("/api/events/:id/salg", auth, managerOrAdmin, function(req, res) {
  ...
  const { itemId, solgt, tomtKl } = req.body;
  const existing = events[evIdx].salg.findIndex(s => s.id === itemId);
  const tomtKlVal = (typeof tomtKl === 'string' && tomtKl.trim()) ? tomtKl.trim() : null;
  if (existing >= 0) {
    events[evIdx].salg[existing].solgt = parseInt(solgt) || 0;
    if (tomtKlVal !== undefined) events[evIdx].salg[existing].tomtKl = tomtKlVal;
  } else {
    events[evIdx].salg.push({ id: itemId, solgt: parseInt(solgt)||0, tomtKl: tomtKlVal });
  }
  writeJSON(EVENTS_FILE, events);
  broadcastEventUpdate(ev.department);
  res.json({ ok: true });
...
// ── Tur map data: geocode + OSRM – streames via SSE ─────────────────
// GET /api/events/:id/map-data   → JSON (cached after first build)
// GET /api/events/:id/map-data-stream → SSE stream while building
if (!global._mapDataCache) global._mapDataCache = {};

// Synchronous check: return cached data instantly, or trigger background build
app.get("/api/events/:id/map-data", rateLimit(30, 60000), async function(req, res) {
  const ev = readJSON(EVENTS_FILE).find(function(e) { return e.id === req.params.id || e.slug === req.params.id; });
  if (!ev || ev.eventType !== "tur") return res.status(404).json({ error: "Ikke funnet" });
  const route = ev.route || {};
  const days  = route.days || [];
  if (!days.length) return res.json({ segments: [], stops: [], ready: true });

  const cacheKey = req.params.id + "_" + JSON.stringify(days).length;
  if (global._mapDataCache[cacheKey]) return res.json(Object.assign({ ready: true }, global._mapDataCache[cacheKey]));

  // Not cached: start background build and return "building" status immediately
  res.json({ ready: false, message: "Bygger kart…" });
  _buildMapData(req.params.id, cacheKey, ev, days);
});

// SSE stream: client connects, gets segments as they're built
app.get("/api/events/:id/map-data-stream", rateLimit(20, 60000), function(req, res) {
  const ev = readJSON(EVENTS_FILE).find(function(e) { return e.id === req.params.id || e.slug === req.params.id; });
  if (!ev || ev.eventType !== "tur") { res.status(404).end(); return; }
  const route = ev.route || {};
  const days  = route.days || [];
  ...
  const cacheKey = req.params.id + "_" + JSON.stringify(days).length;
  if (global._mapDataCache[cacheKey]) {
    send({ type: "done", data: global._mapDataCache[cacheKey] });
    res.end(); return;
  }
  if (!days.length) { send({ type: "done", data: { segments: [], stops: [] } }); res.end(); return; }

  // Register SSE client so background builder can push to it
  if (!global._mapSSEClients) global._mapSSEClients = {};
  if (!global._mapSSEClients[cacheKey]) global._mapSSEClients[cacheKey] = [];
  global._mapSSEClients[cacheKey].push(res);
  req.on("close", function() {
    if (global._mapSSEClients[cacheKey])
      global._mapSSEClients[cacheKey] = global._mapSSEClients[cacheKey].filter(function(r){ return r !== res; });
  });

  // Start build if not already running
  if (!global._mapBuilding) global._mapBuilding = {};
  if (!global._mapBuilding[cacheKey]) {
    global._mapBuilding[cacheKey] = true;
    _buildMapData(req.params.id, cacheKey, ev, days);
  }
});

async function _buildMapData(evId, cacheKey, ev, days) {
  const accentColor = (getSettings().colors && getSettings().colors.accent) || "#FFD100";
  const typeColors  = {
    start: accentColor, slutt: accentColor, stopp: "#888888",
    lunsj: "#f97316", middag: "#a855f7", hotell: "#3b82f6", bensin: "#ef4444",
    opplevelse: { museum:"#8b5cf6", natur:"#22c55e", utsikt:"#06b6d4",
                  historisk:"#f59e0b", aktivitet:"#ec4899", kultur:"#f97316",
                  mat:"#84cc16", annet:"#6366f1" }
  };
  function etappeColor(e) {
    if (e.type === "opplevelse") return typeColors.opplevelse[e.opplevelseSubtype] || "#6366f1";
    return typeColors[e.type] || "#888888";
  }
  function sleep(ms) { return new Promise(function(r){ setTimeout(r, ms); }); }
  function pushSSE(obj) {
    if (!global._mapSSEClients || !global._mapSSEClients[cacheKey]) return;
    global._mapSSEClients[cacheKey].forEach(function(r){ try{ r.write("data: "+JSON.stringify(obj)+"\n\n"); }catch(e){} });
  }

  async function geocode(name) {
    if (!name) return null;
    if (global._geocodeCache && global._geocodeCache[name]) return global._geocodeCache[name];
    try {
      const r = await fetch("https://nominatim.openstreetmap.org/search?format=json&limit=1&countrycodes=no,se,dk,fi&q=" + encodeURIComponent(name),
        { headers: { "User-Agent": "EventsAdmin-Map/1.0" } });
      const d = await r.json();
      if (d && d[0]) {
        const pt = { lat: parseFloat(d[0].lat), lon: parseFloat(d[0].lon) };
        if (!global._geocodeCache) global._geocodeCache = {};
        global._geocodeCache[name] = pt;
        return pt;
      }
    } catch(e) {}
    return null;
  ...
  async function osrmSegment(from, to) {
    try {
  ...
  try {
    const allEt = days.flatMap(function(day){ return (day.etapper||[]).filter(function(e){ return e.fra||e.til; }); });
    if (!allEt.length) { pushSSE({ type:"done", data:{ segments:[], stops:[] } }); return; }

    // Step 1: geocode all unique names
    const names = [];
    allEt.forEach(function(e){
      if (e.fra && !names.includes(e.fra)) names.push(e.fra);
      if (e.til && !names.includes(e.til)) names.push(e.til);
    });
    pushSSE({ type: "status", message: "Geocoder steder (" + names.length + ")…" });
    const coordMap = {};
    for (var i = 0; i < names.length; i++) {
      const pt = await geocode(names[i]);
      if (pt) coordMap[names[i]] = pt;
      if (i < names.length - 1) await sleep(220);
    }

    // Step 2: build stops
    const n = allEt.length;
    const stops = allEt.map(function(e, i) {
      const pt = coordMap[e.til] || coordMap[e.fra];
      if (!pt) return null;
      const isEnd = (e.type==="start" || e.type==="slutt" || i===0 || i===n-1);
      return { lat: pt.lat, lon: pt.lon, name: e.til||e.fra||"", color: etappeColor(e), type: e.type||"stopp", isEnd };
    }).filter(Boolean);
    pushSSE({ type: "stops", stops });

    // Step 3: fetch OSRM segments one by one, push each as it arrives
    pushSSE({ type: "status", message: "Henter kjørerute (" + (allEt.length-1) + " segmenter)…" });
    const segments = [];
    for (var j = 0; j < allEt.length - 1; j++) {
      const eA = allEt[j], eB = allEt[j+1];
      const ptA = coordMap[eA.til] || coordMap[eA.fra];
      const ptB = coordMap[eB.til] || coordMap[eB.fra];
      if (!ptA || !ptB) continue;
      const coords = await osrmSegment(ptA, ptB);
      const seg = { coords, colorA: etappeColor(eA), colorB: etappeColor(eB) };
      segments.push(seg);
      // Push segment immediately so map draws progressively
      pushSSE({ type: "segment", segment: seg, index: j });
      if (j < allEt.length - 2) await sleep(100);
    }

    const result = { segments, stops };
    global._mapDataCache[cacheKey] = result;
    pushSSE({ type: "done", data: result });

    // Close all SSE connections for this key
    if (global._mapSSEClients && global._mapSSEClients[cacheKey]) {
      global._mapSSEClients[cacheKey].forEach(function(r){ try{ r.end(); }catch(e){} });
      delete global._mapSSEClients[cacheKey];
    }
  } catch(err) {
    pushSSE({ type: "error", message: err.message });
  } finally {
    if (global._mapBuilding) delete global._mapBuilding[cacheKey];
  }
}
