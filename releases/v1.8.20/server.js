...
app.get("/api/lab/export/context", auth, adminOnly, function(req, res) {
  ...
  ["events.json","users.json","settings.json","members.json","volunteers.json",
   "wishes.json","email_log.json","inventar.json","blocks.json","series.json","devlog.json","visits.json"].forEach(function(f) {
...
app.get("/api/lab/export/data", auth, adminOnly, function(req, res) {
  if (process.env.NODE_ENV === "production") return res.status(404).json({ error: "Not found" });
  const dfs=["events.json","users.json","settings.json","members.json","volunteers.json","wishes.json","email_log.json","inventar.json","blocks.json","series.json","devlog.json","visits.json"];
  const entries=[];
...
const IS_DEMO         = !!(process.env.DEMO_MODE === "true" || (process.env.DATA_DIR || "").includes("-demo") || (process.env.DATA_DIR || "").includes("demo"));
...
app.post("/api/provision/customer", auth, adminOnly, rateLimit(5, 60000), async function(req, res) {
  const { slug, orgName, domain, contactEmail, portainerUrl, portainerKey, endpointId, cf } = req.body || {};
  ...
  const composeYaml = [
    ...
    "      - " + Q + "traefik.http.routers." + slug + "-sub.rule=HostRegexp(`^[a-z0-9-]+\\." + domainEscaped + "$`) && !Host(`admin." + domain + "`)" + Q,
...
app.post("/api/provision/init-admin", async function(req, res) {
  ...
  try {
    ...
    // Set org name in settings if provided
    if (orgName) {
      const s = getSettings();
      s.siteName = orgName;
...
app.put("/api/events/:id", auth, managerOrAdmin, function(req, res, next) {
  ...
  if (req.body.gbPosition !== undefined) {
    var gbPos = req.body.gbPosition;
    if (['bottom','top','button'].includes(gbPos)) events[i].gbPosition = gbPos;
  }
  try {
    writeJSON(EVENTS_FILE, events);
  } catch(writeErr) {
...
function serveOverviewPage(req, res) {
  ...
  // show all offentlige events – full historikk
...
// Auto-activate first tab per saved order on page load
(function(){
...
function applyEvLang() {
  ...
  setAll('label[for="rEmail"], .ev-form:not(.gb-form) label:nth-of-type(2)', 'text', t('label_email'));
...
<div class="ev-content">
  ...
  <div class="ev-panel" id="tab-gb">
    ${ev.gbPosition==='button' ? `<div id="gbFab" style="position:fixed;bottom:1.5rem;right:1.5rem;z-index:999">
      <button id="gbFabBtn" onclick="var f=document.getElementById('gbFormCard');var open=f.style.display!=='none';f.style.display=open?'none':'block';document.getElementById('gbFabBtn').innerHTML=open?'✍️ Gi tilbakemelding':'✕ Lukk';if(!open)f.scrollIntoView({behavior:'smooth',block:'nearest'})" style="background:var(--y);color:#111;font-weight:800;border:none;border-radius:30px;padding:.75rem 1.25rem;font-size:.9rem;cursor:pointer;box-shadow:0 4px 16px rgba(0,0,0,.3);white-space:nowrap">✍️ Gi tilbakemelding</button>
    </div>` : ''}
    <div id="gbEntries" style="order:${(ev.gbPosition==='top')?'1':'0'}">${gbHtml}</div>
    <div id="gbFormCard" class="ev-card" style="margin-top:1rem;order:${(ev.gbPosition==='top')?'0':'1'};display:${(ev.gbPosition==='button')?'none':'block'}">
      <div class="ev-card-title" data-i18n="gb_title">✍️ Legg igjen en kommentar</div>
      <div id="gbMsg"></div>
      <div class="ev-form gb-form">
...
async function gbDirectUpload(input){
  ...
  setTimeout(function(){
    ...
    // Auto-close the whole form card after upload completes
    _closeGbForm(0);
  },3000);
  input.value='';
}
...
function _closeGbForm(delay) {
  clearTimeout(window._gbFormCloseTimer);
  window._gbFormCloseTimer = setTimeout(function() {
    var card = document.getElementById('gbFormCard');
    var fab = document.getElementById('gbFabBtn');
    if (card) card.style.display = 'none';
    if (fab) fab.innerHTML = '✍️ Gi tilbakemelding';
  }, delay || 0);
}
async function submitGb(){
  ...
  if(r.ok){
    ...
    if(photoArea){
      ...
      // Auto-close form after 15s if no upload started
      clearTimeout(window._gbAutoHideTimer);
      window._gbAutoHideTimer = setTimeout(function(){
      ...
      // No photo area - close form after 5s
      _closeGbForm(5000);
    }
...
function buildMotePage(ev, isAuthenticated) {
  ...
  var _defs = [
    ...
    {id:'reg',html:'<div class="ev-tab" data-tab="reg" onclick="showTab(\'reg\')" data-i18n="tab_reg_mote">' + '📋 Meld deg på' + '</div>'},
...
function buildKursPage(ev, isAuthenticated) {
  ...
  var _defs = [
    ...
    {id:'reg',html:'<div class="ev-tab" data-tab="reg" onclick="showTab(\'reg\')">' + '📋 Påmelding' + '</div>'},
    {id:'oppmote',html:'<div class="ev-tab" data-tab="oppmote" onclick="showTab(\'oppmote\')" data-i18n="tab_oppmote">' + '✅ Oppmøte' + '</div>'},
    {id:'gb',html:'<div class="ev-tab" data-tab="gb" onclick="showTab(\'gb\')" data-i18n="tab_gb">' + T('tab_gb',lang) + '</div>'},
  ];
  var _order = ev.tabOrder && ev.tabOrder.length ? ev.tabOrder : _defs.map(function(d){return d.id;});
...
function showSlide(idx) {
  ...
  setTimeout(function() {
    ...
    if (isMp4) {
      ...
      if (pp !== undefined) {
        pp.catch(function(e) {
          console.warn("[TV] autoplay:", e.message);
          // Prøv muted som fallback
          vid.muted = true;
...
function _seedDemoData() {
  ...
  const demoEmail = "demo@events-admin.no";

  // Always reset users to demo user
  const demoUser = {
    id: "demo-admin",
    email: demoEmail,
    password: demoHash,
  ...
  // Only seed events/settings if they don't exist yet
  ...
  if (!settings.siteName) {
    writeJSON(SETTINGS_FILE, Object.assign(settings, {
      siteName: "Demo – Events Admin",
      contactEmail: demoEmail,
      eventDomain: "demo.events-admin.no",
      theme: "dark",
      colors: { accent: "#f5c500" }
    }));
  }

  // Demo departments (via settings)
  const settingsFull = readJSON(SETTINGS_FILE) || {};
  if (!settingsFull.departments || !settingsFull.departments.length) {
    settingsFull.departments = [
      { id: "dept-oslo", name: "Oslo", slug: "oslo" },
      { id: "dept-bergen", name: "Bergen", slug: "bergen" },
      { id: "dept-trondheim", name: "Trondheim", slug: "trondheim" }
    ];
    writeJSON(SETTINGS_FILE, settingsFull);
  }

  // Demo events
  const now = new Date();
  const future = (days) => new Date(now.getTime() + days*86400000).toISOString().slice(0,10);
  const past   = (days) => new Date(now.getTime() - days*86400000).toISOString().slice(0,10);

  const demoEvents = [
    // Stand
    {
      id: uuidv4(), eventType: "stand", title: "MC-messe Oslo",
      date: future(14), time: "10:00", location: "Oslo spektrum",
      department: "dept-oslo", slug: "mc-messe-oslo-demo",
      description: "Bli med oss på årets største MC-messe! Kom innom standen vår.",
      isPublished: true, lang: "no",
      registrations: [
        { id: uuidv4(), name: "Frode Hansen", email: "frode@example.com", phone: "91234567", createdAt: past(2) },
        { id: uuidv4(), name: "Kari Olsen", email: "kari@example.com", phone: "98765432", createdAt: past(1) }
      ],
      guestbook: [
        { id: uuidv4(), name: "Bjørn Arne", message: "Gleder meg til å se dere der!", phone: "99887766", approved: true, createdAt: past(1) }
      ],
      lottery: { enabled: true, prize: "MC-jakke (verdi 3000,-)", winners: [] },
      staff: [
        { id: uuidv4(), name: "Demo Admin", role: "Standansvarlig", email: demoEmail }
      ],
      createdAt: past(10), createdBy: demoEmail
    },
    // Kurs
    {
      id: uuidv4(), eventType: "kurs", title: "Sikkerhetskurs MC",
      date: future(30), time: "09:00", location: "Gardemoen kjøresenter",
      department: "dept-oslo", slug: "sikkerhetskurs-mc-demo",
      description: "Grunnleggende sikkerhetskurs for MC-kjørere. Maks 20 deltakere.",
      isPublished: true, lang: "no", maxParticipants: 20,
      registrations: [
        { id: uuidv4(), name: "Per Andersen", email: "per@example.com", phone: "91111111", createdAt: past(5) },
        { id: uuidv4(), name: "Lise Dahl", email: "lise@example.com", phone: "92222222", createdAt: past(3) },
        { id: uuidv4(), name: "Tor Bakke", email: "tor@example.com", phone: "93333333", createdAt: past(2) }
      ],
      guestbook: [],
      staff: [],
      createdAt: past(15), createdBy: demoEmail
    },
    // Tur
    {
      id: uuidv4(), eventType: "tur", title: "Sommertur Bergen–Flåm",
      date: future(45), time: "08:00", location: "Bergen sentrum",
      department: "dept-bergen", slug: "sommertur-bergen-flam-demo",
      description: "Spektakulær MC-tur langs Sognefjorden. 2 dager, 280 km.",
      isPublished: true, lang: "no", maxParticipants: 30,
      days: [
        {
          date: future(45), profile: "driving",
          etapper: [
            { fra: "Bergen", til: "Voss", km: 100, tid: 1.5, type: "stopp" },
            { fra: "Voss", til: "Flåm", km: 55, tid: 1.2, type: "stopp" }
          ]
        },
        {
          date: future(46), profile: "driving",
          etapper: [
            { fra: "Flåm", til: "Gudvangen", km: 35, tid: 0.8, type: "stopp" },
            { fra: "Gudvangen", til: "Bergen", km: 95, tid: 1.8, type: "stopp" }
          ]
        }
      ],
      registrations: [
        { id: uuidv4(), name: "Arne Fjord", email: "arne@example.com", phone: "94444444", createdAt: past(7) }
      ],
      guestbook: [],
      staff: [],
      createdAt: past(20), createdBy: demoEmail
    },
    // Møte (past)
    {
      id: uuidv4(), eventType: "mote", title: "Årsmøte 2025",
      date: past(60), time: "18:00", location: "Clubhuset, Oslo",
      department: "dept-oslo", slug: "arsmote-2025-demo",
      description: "Ordinært årsmøte. Valg av nytt styre, regnskap og planer for 2026.",
      isPublished: true, lang: "no", maxParticipants: 50,
      registrations: [
        { id: uuidv4(), name: "Ingrid Berg", email: "ingrid@example.com", phone: "95555555", createdAt: past(70), checkedIn: true },
        { id: uuidv4(), name: "Olav Strand", email: "olav@example.com", phone: "96666666", createdAt: past(68), checkedIn: true },
        { id: uuidv4(), name: "Maja Lund", email: "maja@example.com", phone: "97777777", createdAt: past(65), checkedIn: false }
      ],
      guestbook: [
        { id: uuidv4(), name: "Ingrid Berg", message: "Bra møte! Gleder meg til neste år.", phone: "95555555", approved: true, createdAt: past(59) }
      ],
      staff: [],
      createdAt: past(90), createdBy: demoEmail
    }
  ];

  writeJSON(EVENTS_FILE, demoEvents);
  console.log("[demo] Seeded " + demoEvents.length + " demo events and 1 demo user.");
  console.log("[demo] Login: " + demoEmail + " / Demo1234!");
}
