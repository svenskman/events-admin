...
// favicon.ico — browsers request this directly
app.get("/favicon.ico", function(req, res) {
  res.redirect(301, "/api/favicon");
});

app.get("/api/favicon", function(req, res) {
  var s       = getSettings();
  var logoUrl = s.logoUrl;   // e.g. "/logo/abc123" or a data: URL
  var crop    = s.faviconCrop; // { x, y, w, h } 0-1 fractions

  var accent = (s.colors && s.colors.accent) || "#FFD100";
  var letter = (s.siteName || "E")[0].toUpperCase();

  // ── Fallback: letter SVG (no logo uploaded) ───────────────────
  function sendLetterSvg() {
  ...
  if (!logoUrl) return sendLetterSvg();

  // ── Try to read logo from disk and embed as base64 ────────────
  var logoData = null;
  ...
  if (logoUrl.startsWith("data:")) {
    ...
    logoData = logoUrl;
  } else if (logoUrl.startsWith("/logo/")) {
    // Served from disk — read and base64-encode
    var filename = logoUrl.replace(/^\/logo\//, "");
    ...
    try {
      ...
      if (fileBytes[0] === 0x89 && fileBytes[1] === 0x50) logoMime = "image/png";
      else if (fileBytes[0] === 0xFF && fileBytes[1] === 0xD8) logoMime = "image/jpeg";
      else if (fileBytes[0] === 0x47 && fileBytes[1] === 0x49) logoMime = "image/gif";
      else if (fileBytes.slice(0,4).toString() === "<svg" || fileBytes.slice(0,5).toString() === "<?xml") logoMime = "image/svg+xml";
      else logoMime = "image/png";
      logoData = "data:" + logoMime + ";base64," + fileBytes.toString("base64");
    } catch(e) {
      console.warn("[favicon] Could not read logo file:", filePath, e.message);
      return sendLetterSvg();
    }
  } else {
    return sendLetterSvg();
  }

  // ── Build SVG favicon with embedded (base64) image ────────────
  // Use viewBox + clipPath to implement crop
  ...
  // The image is positioned so that the crop region fills the 32x32 viewBox
  var imgX = -(cx / cw) * 32;
  var imgY = -(cy / ch) * 32;
  var imgW = 32 / cw;
  var imgH = 32 / ch;
  ...
  var svg = '<?xml version="1.0" encoding="UTF-8"?>'
    ...
    + '<defs><clipPath id="fc"><rect x="0" y="0" width="32" height="32"/></clipPath></defs>'
    + '<image clip-path="url(#fc)" preserveAspectRatio="none"'
    + ' href="' + logoData + '"'
...
app.post("/api/tv/:deptId/video-slides", auth, managerOrAdmin,
  multer({ dest: UPLOADS, limits: { fileSize: 500 * 1024 * 1024 },
    fileFilter: function(req, file, cb) {
      const ok = /^video\//.test(file.mimetype) || /\.(mp4|webm|mov|m4v)$/i.test(file.originalname);
      cb(null, !!ok);
    }
  }).single("video"),
  async function(req, res) {
    ...
    // Konverter til H.264/AAC for bred nettleser-støtte
    try {
      await new Promise(function(resolve, reject) {
        const { spawn } = require("child_process");
        const ff = spawn("ffmpeg", [
          ...
          "-c:a", "aac",         // AAC audio
          "-b:a", "128k",
          "-movflags", "+faststart", // Streaming-optimalisert
          "-y",                  // Overskriv
          outPath
        ]);
        ff.on("close", function(code) {
          if (code === 0) resolve();
          else reject(new Error("ffmpeg feil: " + code));
        });
        ff.on("error", reject);
      });
      // Slett original
      ...
      // ffmpeg feilet – bruk original med riktig filnavn
      try { fs.renameSync(origPath, outPath); } catch(e2) {}
    }

    const videoUrl = "/uploads/" + outName;
    const slide    = { type: "mp4-slide", url: videoUrl, name: req.file.originalname };
    const prev     = getTvState(deptId);
...
<div id="screen">
  ...
  <div id="slideLayer">
    ...
    <video id="slideVideo" muted playsinline loop style="display:none;position:absolute;inset:0;width:100%;height:100%;object-fit:contain;background:#000;z-index:3"></video>
...
function showSlide(idx) {
  ...
  setTimeout(function() {
    ...
    if (isMp4) {
      ...
      if (vid.src !== slide.url) {
        vid.src = slide.url;
        vid.load();
      }
      vid.play().catch(function(e) {
        console.warn("[TV] video play:", e.message);
      });
    } else {
      // PNG slide
