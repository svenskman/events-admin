...
app.post("/api/ai/route-from-waypoints", auth, managerOrAdmin, rateLimit(10, 60000), async function(req, res) {
  const { waypoints, profile, altCount, roundtrip, startDate, endDate } = req.body;
  ...
  for (let i = 0; i < allWps.length - 1; i++) {
    segments.push({ fra: allWps[i], til: allWps[i + 1] });
  }
  ...
  const segList = segments.map((s, i) => `  Segment ${i+1}: ${s.fra} → ${s.til}`).join("\n");
