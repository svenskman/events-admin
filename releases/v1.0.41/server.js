...
// Unik kryptert QR-kode per påmelding — brukes til innsjekk og lotteri-verifisering
  ...
  const qrSection = qrUrl ? `
    <div style="text-align:center;margin:1.5rem 0;padding:1.25rem;background:#fff;border-radius:8px;border:1px solid #e0e0e0">
      <div style="font-size:.85rem;font-weight:700;color:#555;margin-bottom:.75rem">
        ${isStand ? "🎟️ Din personlige deltaker-QR" : "📋 Din innsjekk-QR"}
      </div>
      ...
      <div style="font-size:.75rem;color:#888;margin-top:.75rem;line-height:1.5">
        ${isStand ? "Ta vare på denne – vis den til arrangøren hvis du vinner i trekningen." : "Vis denne til staben ved ankomst."}
      </div>
    </div>` : "";
...
app.post("/api/events/:id/lottery/draw", auth, managerOrAdmin, function(req, res) {
  ...
  if (minP > 0 && eligible.length < minP)
    return res.status(400).json({ error: "For få deltakere – trenger minst " + minP + ", har " + eligible.length });
  ...
  if (ev.lottery.startAfter) {
    ...
    if (nowMin < saMin)
      return res.status(400).json({ error: "Trekning er ikke åpnet før kl. " + ev.lottery.startAfter });
  }
  ...
  if (inventarPool.length > 0) {
    ...
    // Bygg vektet liste: én oppføring per tilgjengelig enhet
    const pool = [];
    ...
    if (pool.length === 0) return res.status(400).json({ error: "Ingen premier igjen i inventar-poolen" });
...
function buildWinnerEmail(ev, draw, settings) {
  ...
  const subject   = "🎉 Du vant! – " + escHtml(ev.title);
...
// Sender til alle vinnere som ikke har innløst premien (eller til én spesifikk regId)
app.post("/api/events/:id/lottery/send-winner-email", auth, managerOrAdmin, function(req, res) {
  ...
  if (!winners.length) return res.status(400).json({ error: "Ingen vinnere å sende til" });
  ...
  if (!targets.length) return res.status(400).json({ error: "Ingen ikke-innløste vinnere å sende til" });
  ...
  targets.forEach(function(draw) {
    ...
    sendEmail({ to: draw.email, subject: mail.subject, html: mail.html, text: mail.text })
      .catch(function(e) { console.error("[lottery] Send feil:", e && e.message); });
    sent++;
  });
...
// ── API: Verifiser registrerings-QR ─────────────────────────────
// Brukes til å scanne påmeldingsbekreftelsens QR ved premie-utdeling
...
app.post("/api/events/:id/lottery/verify", auth, function(req, res) {
  ...
  // Sjekk at token tilhører dette eventet
  const ev = readJSON(EVENTS_FILE).find(function(e) { return e.id === req.params.id; });
  if (!ev) return res.status(404).json({ error: "Event ikke funnet" });
...
app.post("/api/events/:id/register", rateLimit(10, 60000), function(req, res) {
  ...
  const reg = { id: _regId, name: name, email: email.toLowerCase(), phone: phone, registeredAt: new Date().toISOString(), checkinPin: _pin, cancelToken: _cancelToken, regToken: _regToken };
  if (req.body.walkin) {
    reg.walkin = true;
    reg.checkedIn = true;
...
app.get("/api/events/:id/display/stream", function(req, res) {
  ...
  displayClients.set(clientId, { res, evId: req.params.id });
  // Send current state immediately — include winners from events file
  const state = displayState.get(req.params.id) || { mode: "slides", slide: 0 };
  const evForSse = readJSON(EVENTS_FILE).find(function(e) { return e.id === req.params.id; });
  const winnersForSse = evForSse && evForSse.lottery && evForSse.lottery.winners
    ? evForSse.lottery.winners.map(function(w) { return { name: w.name, prize: w.prize || "", redeemedAt: w.redeemedAt || null }; })
    : [];
  res.write("data: " + JSON.stringify(Object.assign({ type: "state" }, state, { winners: winnersForSse })) + "\n\n");
  req.on("close", function() { displayClients.delete(clientId); });
});

// Get current display state
app.get("/api/events/:id/display/state", function(req, res) {
  const state = displayState.get(req.params.id) || { mode: "slides", slide: 0, slides: [], ticker: "" };
  // Always inject current winners from events file (stays fresh after reload)
  const ev = readJSON(EVENTS_FILE).find(function(e) { return e.id === req.params.id; });
  const winners = ev && ev.lottery && ev.lottery.winners
    ? ev.lottery.winners.map(function(w) { return { name: w.name, prize: w.prize || "", redeemedAt: w.redeemedAt || null }; })
    : [];
...
app.post("/api/events/:id/display/pptx", auth, managerOrAdmin,
  ...
  async function(req, res) {
    ...
    const workDir  = path.join(os.tmpdir(), "pptx_" + Date.now());
...
`;
    ...
    const pollInterval = setInterval(function() {
      ...
      if (pollCount > 600) {
        clearInterval(pollInterval);
        cleanup();
        res.status(500).json({ error: "LibreOffice timeout – filen kan være for stor for denne serveren" });
      }
    }, 1000);
    ...
    function proceedWithPdf(pdfFile) {
      ...
      execFile("pdftoppm", ["-r","96","-png", pdfFiles[0].replace(workDir+"/",""), slidePrefix],
        ...
        pngFiles.forEach(function(f, fi) {
          ...
          const dest = path.join(UPLOADS, "slide_" + evId + "_" + ts + "_" + f);
...
<style>
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  #slideImg{max-width:100%;max-height:100%;object-fit:contain;transition:opacity .5s}
...
var state  = { mode:"slides", slide:0, slides:[], ticker:"", winnerName:"", prize:"", slideInterval:10, winnerDuration:300, winners:[] };
...
function applyState(s) {
  ...
  // New winner revealed
  var newWinner = state.mode === "winner" && state.winnerName && state.winnerName !== lastWinnerName;
...
function render(newWinner) {
  ...
  if (state.mode === "winner") {
    ...
    if (newWinner) {
      ...
      // Already in winner mode, just update text
...
// ── Trommehvirvel + avsløringssekvens ──────────────────────────
function startWinnerReveal() {
  ...
  // Trommehvirvel-tekst (rullerer navn raskt)
  var allNames = ["???","***","…","###","…","???"];
  ...
  // Etter 2.5s: stopp trommehvirvel, avsløre vinner
  setTimeout(function(){
...
// ── Nedtelling til tilbake til slides ──────────────────────────
...
function startSlideTimer() {
  ...
  slideAdvanceTimer = setTimeout(function(){
    ...
    renderSlides();
    startSlideTimer(); // restart with fresh progress
  }, interval);
}
...
app.get("/lottery/:evId", auth, function(req, res) {
  ...
  if (!ev) return res.status(404).send("Event ikke funnet");
  const lottery = ev.lottery || {};
  if (!lottery.enabled) return res.status(400).send("Lotteri er ikke aktivert for dette eventet");
...
async function doDraw() {
  ...
  if (d.ok && d.winner) {
    ...
    res.innerHTML = "✗ " + esc(d.error || "Feil");
  }
}
...
async function redeemWinner(token) {
  ...
  if (d.valid && d.isWinner && !d.alreadyRedeemed) {
    ...
    res.className="result result-err"; res.textContent="✗ " + esc(d.message||d.reason||"Feil");
  }
}
...
async function onScanFound(data) {
  ...
  setTimeout(async function(){
    ...
    var d=await r.json();
    if(!d.valid){ res.className="result result-err"; res.innerHTML="❌ " + esc(d.message||"Ugyldig kode"); return; }
...
const MOBILE_HTML = '<!DOCTYPE html>\n<html lang="no">\n<head>\n<meta charset="UTF-8"/>\n<meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1"/>\n<title>__SITENAME__ – Mobil</title>\n<style>\n  *{margin:0;padding:0;box-sizing:border-box;-webkit-tap-highlight-color:transparent}\n  body{background:#0d0d0d;color:#fff;font-family:system-ui,sans-serif;min-height:100dvh;padding-bottom:3rem}\n  header{background:#111;border-bottom:2px solid __ACCENT__;padding:.85rem 1rem;display:flex;justify-content:space-between;align-items:center;position:sticky;top:0;z-index:100}\n  header h1{font-size:1rem;font-weight:900;color:__ACCENT__}\n  header a{color:#555;font-size:.8rem;text-decoration:none;padding:.35rem .65rem;border:1px solid #333;border-radius:6px}\n  .section-label{font-size:.65rem;font-weight:800;color:#444;text-transform:uppercase;letter-spacing:.08em;padding:.75rem 1rem .35rem}\n  .evcard{background:#161616;border-bottom:1px solid #1e1e1e;padding:.85rem 1rem;cursor:pointer;user-select:none}\n  .evcard:active{background:#1e1e1e}\n  .evcard-head{display:flex;justify-content:space-between;align-items:center;gap:.5rem}\n  .evcard-title{font-weight:700;font-size:.95rem;margin-bottom:.15rem}\n  .evcard-meta{font-size:.75rem;color:#555}\n  .evcard-arrow{font-size:1.1rem;color:#333;transition:transform .2s;flex-shrink:0}\n  .evcard-arrow.open{transform:rotate(180deg);color:__ACCENT__}\n  .evactions{background:#0d0d0d;border-bottom:2px solid #1a1a1a;padding:.75rem 1rem;display:flex;flex-direction:column;gap:.6rem}\n  .action-group{background:#161616;border:1px solid #222;border-radius:10px;padding:.75rem .85rem}\n  .action-label{font-size:.68rem;font-weight:800;color:#444;text-transform:uppercase;letter-spacing:.06em;margin-bottom:.5rem}\n  .action-row{display:flex;gap:.4rem;flex-wrap:wrap;margin-bottom:.3rem}\n  .action-row:last-of-type{margin-bottom:0}\n  .abtn{flex:1;min-width:80px;padding:.65rem .4rem;border-radius:8px;border:none;font-size:.82rem;font-weight:700;cursor:pointer;text-align:center;text-decoration:none;display:inline-block;transition:opacity .15s;line-height:1.3}\n  .abtn:active{opacity:.7}\n  .abtn-y{background:__ACCENT__;color:#111}\n  .abtn-blue{background:#0d1a2e;border:1px solid #2563eb;color:#60a5fa}\n  .abtn-green{background:#0a2a0a;border:1px solid #16a34a;color:#4ade80}\n  .abtn-ghost{background:#1a1a1a;border:1px solid #2a2a2a;color:#888}\n  .action-msg{font-size:.82rem;min-height:1.2rem;margin-top:.35rem;font-weight:600}\n  .winner-row{display:flex;justify-content:space-between;align-items:center;padding:.45rem 0;border-bottom:1px solid #1e1e1e;font-size:.85rem}\n  .winner-row:last-child{border:none}\n  .winner-row.redeemed{opacity:.5}\n  .badge-green{background:#0a2a0a;color:#4ade80;border:1px solid #166534;border-radius:4px;padding:2px 7px;font-size:.72rem;white-space:nowrap}\n  .badge-yellow{background:#1a1400;color:#f5c500;border:1px solid #713f12;border-radius:4px;padding:2px 7px;font-size:.72rem;white-space:nowrap}\n  #scanOverlay{display:none;position:fixed;inset:0;background:rgba(0,0,0,.92);z-index:999;flex-direction:column;align-items:center;justify-content:center;gap:1rem;padding:1rem}\n  #scanBox{position:relative;width:min(280px,80vw);height:min(280px,80vw);border:3px solid __ACCENT__;border-radius:12px;overflow:hidden;background:#111}\n  #scanVideo{width:100%;height:100%;object-fit:cover}\n  .corner{position:absolute;width:22px;height:22px;border-color:__ACCENT__;border-style:solid}\n  .tl{top:5px;left:5px;border-width:3px 0 0 3px;border-radius:3px 0 0 0}\n  .tr{top:5px;right:5px;border-width:3px 3px 0 0;border-radius:0 3px 0 0}\n  .bl{bottom:5px;left:5px;border-width:0 0 3px 3px;border-radius:0 0 0 3px}\n  .br{bottom:5px;right:5px;border-width:0 3px 3px 0;border-radius:0 0 3px 0}\n  #scanStatus{color:#aaa;font-size:.88rem;text-align:center;max-width:280px}\n  #scanResult{border-radius:10px;padding:.85rem 1rem;font-weight:700;font-size:.95rem;text-align:center;max-width:300px;width:100%;display:none}\n  .empty{color:#333;text-align:center;padding:3rem 1rem;font-size:.9rem}\n</style>\n</head>\n<body>\n<header>\n  <h1>&#9889; __SITENAME__</h1>\n  <a href="/">&#128187; Full admin</a>\n</header>\n__EVCARDS__\n<div id="scanOverlay">\n  <div style="color:#fff;font-weight:700;font-size:1rem">&#128247; Scan vinner-QR</div>\n  <div id="scanBox">\n    <video id="scanVideo" autoplay playsinline muted></video>\n    <div class="corner tl"></div><div class="corner tr"></div>\n    <div class="corner bl"></div><div class="corner br"></div>\n  </div>\n  <div id="scanStatus">Rett kameraet mot QR-koden&hellip;</div>\n  <div id="scanResult"></div>\n  <button onclick="closeScan()" style="background:#2a0a0a;border:1px solid #7f1d1d;color:#f87171;padding:.75rem 1.5rem;border-radius:8px;font-size:.95rem;cursor:pointer">&#10005; Avbryt</button>\n</div>\n<script>\nvar _scanEvId=null,_stream=null,_animFrame=null,_scanFound=false;\nvar _winners=__WINNERS_DATA__;\n\nfunction esc(s){return String(s||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;");}\n\nfunction toggleEv(id){\n  var a=document.getElementById("actions-"+id);\n  var arr=document.getElementById("arrow-"+id);\n  var open=a.style.display!=="none";\n  a.style.display=open?"none":"";\n  if(arr)arr.classList.toggle("open",!open);\n}\n\nasync function api(method,url,body){\n  var opts={method,credentials:"include",headers:{}};\n  if(body){opts.headers["Content-Type"]="application/json";opts.body=JSON.stringify(body);}\n  try{var r=await fetch(url,opts);return await r.json();}catch(e){return{err:e.message};}\n}\n\nfunction msg(id,text,color){\n  var el=document.getElementById(id);\n  if(!el)return;\n  el.textContent=text;\n  el.style.color=color||"#4ade80";\n  if(text)setTimeout(function(){el.textContent="";},3500);\n}\n\nasync function drawLottery(evId,btn){\n  btn.disabled=true;btn.textContent="&#9203; Trekker...";\n  var d=await api("POST","/api/events/"+evId+"/lottery/draw");\n  btn.disabled=false;btn.textContent="&#127920; Trekk vinner";\n  if(d.ok&&d.winner){\n    msg("lottery-msg-"+evId,"&#127881; "+d.winner.name+" vant"+(d.winner.prize?" \\u2013 "+d.winner.prize:"")+"!","#4ade80");\n    await api("POST","/api/events/"+evId+"/display/state",{mode:"winner",winnerName:d.winner.name,prize:d.winner.prize||"",prizeImage:d.winner.prizeImage||null});\n    setTimeout(function(){location.reload();},2000);\n  }else{\n    msg("lottery-msg-"+evId,"\\u2717 "+(d.error||d.err||"Feil"),"#f87171");\n  }\n}\n\nasync function sendEmails(evId){\n  var d=await api("POST","/api/events/"+evId+"/lottery/send-winner-email");\n  if(d.ok)msg("lottery-msg-"+evId,"\\u2713 Sendt til "+d.sent+" vinner(e)","#60a5fa");\n  else msg("lottery-msg-"+evId,"\\u2717 "+(d.error||d.err||"Feil"),"#f87171");\n}\n\nfunction openDisplay(evId){window.open("/display/"+evId,"_blank");}\n\nasync function setMode(evId,mode){\n  await api("POST","/api/events/"+evId+"/display/state",{mode});\n  msg("lottery-msg-"+evId,mode==="slides"?"\\u2713 Skjerm viser slides":"\\u2713 Skjerm i vinnermodus",mode==="slides"?"#60a5fa":"#f5c500");\n}\n\nfunction toggleNameSearch(evId){\n  var el=document.getElementById("namesearch-"+evId);\n  if(el)el.style.display=el.style.display==="none"?"":"none";\n}\n\nfunction nameSearch(evId,q){\n  var out=document.getElementById("namesearch-results-"+evId);\n  if(!out)return;\n  q=(q||"").trim().toLowerCase();\n  if(q.length<2){out.innerHTML="";return;}\n  var winners=_winners[evId]||[];\n  var matches=winners.filter(function(w){return w.name.toLowerCase().includes(q);});\n  if(!matches.length){out.innerHTML=\'<div style="color:#555;font-size:.82rem;padding:.3rem">Ikke funnet</div>\';return;}\n  out.innerHTML=matches.map(function(w){\n    var redeemed=!!w.redeemedAt;\n    return \'<div style="display:flex;justify-content:space-between;align-items:center;background:#1a1a1a;border-radius:6px;padding:.5rem .75rem;margin-bottom:.3rem">\'\n      +\'<div><div style="font-weight:700">\'+esc(w.name)+\'</div>\'+(w.prize?\'<div style="font-size:.75rem;color:#f5c500">\'+esc(w.prize)+\'</div>\':\'\')+\'</div>\'\n      +(redeemed?\'<span class="badge-green">&#9989; Hentet</span>\'\n        :\'<button onclick="redeemW(\\\'\'+evId+\'\\\',\\\'\'+esc(w.winnerToken||"")+\'\\\',this)" style="background:#166534;border:none;color:#4ade80;padding:.4rem .75rem;border-radius:6px;font-size:.8rem;font-weight:700;cursor:pointer">&#9989; Innl&oslash;s</button>\')\n      +\'</div>\';\n  }).join("");\n}\n\nasync function redeemW(evId,token,btn){\n  if(btn){btn.disabled=true;btn.textContent="&#9203;...";}\n  var d=await api("POST","/api/events/"+evId+"/lottery/verify-reg",{token});\n  if(d.valid&&d.isWinner&&!d.alreadyRedeemed){\n    msg("verify-msg-"+evId,"\\u2705 Innl\\u00f8st \\u2013 "+d.name,"#4ade80");\n    setTimeout(function(){location.reload();},1500);\n  }else if(d.alreadyRedeemed){\n    msg("verify-msg-"+evId,"\\u26a0 Allerede innl\\u00f8st","#f59e0b");\n  }else{\n    msg("verify-msg-"+evId,"\\u2717 "+(d.message||d.reason||"Feil"),"#f87171");\n    if(btn){btn.disabled=false;btn.textContent="&#9989; Innl\\u00f8s";}\n  }\n}\n\nfunction openQR(evId){\n  _scanEvId=evId;_scanFound=false;\n  var ov=document.getElementById("scanOverlay");\n  var res=document.getElementById("scanResult");\n  ov.style.display="flex";res.style.display="none";\n  if(!navigator.mediaDevices){document.getElementById("scanStatus").textContent="Kamera ikke tilgjengelig";return;}\n  navigator.mediaDevices.getUserMedia({video:{facingMode:"environment"}}).then(function(s){\n    _stream=s;\n    var v=document.getElementById("scanVideo");v.srcObject=s;v.play();\n    if(window.BarcodeDetector){\n      var det=new BarcodeDetector({formats:["qr_code"]});\n      function tick(){\n        if(_scanFound)return;\n        if(v.readyState>=2)det.detect(v).then(function(codes){\n          if(codes&&codes.length){onQR(codes[0].rawValue);return;}\n          _animFrame=requestAnimationFrame(tick);\n        }).catch(function(){_animFrame=requestAnimationFrame(tick);});\n        else _animFrame=requestAnimationFrame(tick);\n      }\n      _animFrame=requestAnimationFrame(tick);\n    }else{\n      document.getElementById("scanStatus").textContent="BarcodeDetector ikke st\\u00f8ttet \\u2013 bruk navnes\\u00f8k";\n    }\n  }).catch(function(e){closeScan();alert("Kamerafeil: "+e.message);});\n}\n\nfunction closeScan(){\n  _scanFound=true;\n  if(_animFrame)cancelAnimationFrame(_animFrame);\n  if(_stream)_stream.getTracks().forEach(function(t){t.stop();});_stream=null;\n  document.getElementById("scanOverlay").style.display="none";\n}\n\nasync function onQR(data){\n  if(_scanFound)return;_scanFound=true;\n  document.getElementById("scanStatus").textContent="\\u2705 Kode funnet!";\n  setTimeout(async function(){\n    closeScan();\n    if(!_scanEvId)return;\n    var d=await api("POST","/api/events/"+_scanEvId+"/lottery/verify-reg",{token:data});\n    var res=document.getElementById("scanResult");\n    res.style.display="block";\n    if(!d.valid){\n      res.style.cssText="background:#2a0a0a;border:2px solid #7f1d1d;color:#f87171;border-radius:10px;padding:.85rem;font-weight:700;text-align:center;display:block";\n      res.textContent="\\u274c "+(d.message||"Ugyldig kode");\n    }else if(d.isWinner&&d.alreadyRedeemed){\n      res.style.cssText="background:#1a1400;border:2px solid #713f12;color:#f59e0b;border-radius:10px;padding:.85rem;font-weight:700;text-align:center;display:block";\n      res.textContent="\\u26a0\\ufe0f Allerede innl\\u00f8st \\u2013 "+d.name;\n    }else if(d.isWinner){\n      res.style.cssText="background:#0a2a0a;border:2px solid #166534;color:#4ade80;border-radius:10px;padding:.85rem;font-weight:700;text-align:center;display:block";\n      res.textContent="\\ud83c\\udfc6 VINNER! "+d.name+(d.prize?" \\u2013 "+d.prize:"");\n      msg("verify-msg-"+_scanEvId,"\\u2705 Innl\\u00f8st \\u2013 "+d.name,"#4ade80");\n    }else{\n      res.style.cssText="background:#0d1a2e;border:2px solid #2563eb;color:#60a5fa;border-radius:10px;padding:.85rem;font-weight:700;text-align:center;display:block";\n      res.textContent="\\u2705 "+d.name+" er registrert (ikke vinner)";\n    }\n    setTimeout(function(){res.style.display="none";},5000);\n  },400);\n}\n</script>\n</body>\n</html>\n';
...
app.get("/m", auth, function(req, res) {
  ...
  function evCard(ev) {
    ...
    const title = escHtml(ev.title);
    const loc = ev.location ? " &middot; " + escHtml(ev.location) : "";

    let html = "<div class=\"evcard\" onclick=\"toggleEv('" + id + "')\">"+
      "<div class=\"evcard-head\">"+
      "<div><div class=\"evcard-title\">" + title + "</div>"+
      "<div class=\"evcard-meta\">" + dateStr + loc + " &middot; " + regs + " p&aring;meldt</div></div>"+
      "<div class=\"evcard-arrow\" id=\"arrow-" + id + "\">&or;</div>"+
      "</div></div>"+
      "<div class=\"evactions\" id=\"actions-" + id + "\" style=\"display:none\">";

    if (hasLottery) {
      html += "<div class=\"action-group\">"+
        "<div class=\"action-label\">&#127920; Lotteri &middot; " + drawn + "/" + prizeCount + " trukket &middot; " + regs + " kvalifisert</div>"+
        "<div class=\"action-row\">"+
        "<button class=\"abtn abtn-y\" onclick=\"drawLottery('" + id + "',this)\">&#127920; Trekk vinner</button>"+
        (unredeemed > 0 ? "<button class=\"abtn abtn-blue\" onclick=\"sendEmails('" + id + "')\">&#128231; Send QR (" + unredeemed + ")</button>" : "")+
        "</div><div class=\"action-row\">"+
        "<button class=\"abtn abtn-ghost\" onclick=\"openDisplay('" + id + "')\">&#8599; Skjerm</button>"+
        "<button class=\"abtn abtn-ghost\" onclick=\"setMode('" + id + "','slides')\">&#128444; Slides</button>"+
        "<button class=\"abtn abtn-ghost\" onclick=\"setMode('" + id + "','winner')\">&#127942; Vinnermodus</button>"+
        "</div>"+
        "<div id=\"lottery-msg-" + id + "\" class=\"action-msg\"></div>"+
        "</div>";
    }

    html += "<div class=\"action-group\">"+
      "<div class=\"action-label\">&#128269; Verifiser QR</div>"+
      "<div class=\"action-row\">"+
      "<button class=\"abtn abtn-green\" onclick=\"openQR('" + id + "')\">&#128247; Scan QR</button>"+
      "<button class=\"abtn abtn-ghost\" onclick=\"toggleNameSearch('" + id + "')\">&#128100; S&oslash;k navn</button>"+
      "</div>"+
      "<div id=\"namesearch-" + id + "\" style=\"display:none;margin-top:.5rem\">"+
      "<input type=\"text\" placeholder=\"Skriv navn\u2026\" oninput=\"nameSearch('" + id + "',this.value)\""+
      " style=\"width:100%;background:#1a1a1a;border:1px solid #2a2a2a;color:#fff;border-radius:8px;padding:.65rem .85rem;font-size:.95rem\"/>"+
      "<div id=\"namesearch-results-" + id + "\" style=\"margin-top:.4rem\"></div>"+
      "</div>"+
      "<div id=\"verify-msg-" + id + "\" class=\"action-msg\"></div>"+
      "</div>";

    if (winners.length) {
      html += "<div class=\"action-group\"><div class=\"action-label\">&#127942; Vinnere</div>";
      winners.slice().reverse().forEach(function(w) {
        html += "<div class=\"winner-row" + (w.redeemedAt?" redeemed":"") + "\">"+
          "<div>" + escHtml(w.name) + (w.prize ? " <span style=\"color:#f5c500\">&middot; " + escHtml(w.prize) + "</span>" : "") + "</div>"+
          (w.redeemedAt ? "<span class=\"badge-green\">&#9989; Hentet</span>" : "<span class=\"badge-yellow\">&#9203; Venter</span>")+
          "</div>";
      });
      html += "</div>";
    }

    html += "<div class=\"action-group\"><div class=\"action-row\">"+
      "<a href=\"/display/" + id + "\" target=\"_blank\" class=\"abtn abtn-ghost\">&#8599; Skjerm</a>"+
      "<a href=\"/lottery/" + id + "\" class=\"abtn abtn-ghost\">&#127920; Lotteri-side</a>"+
      "</div></div></div>";
  ...
  res.send(MOBILE_HTML
    .replace("__SITENAME__", escHtml(siteName))
...
app.get("*", function(req, res) {
  ...
  if (req.path === "/" && isMobileBrowser(req) && req.session && req.session.user) {
...
// Kjøres ved oppstart hvis verktøyene mangler (Alpine/Debian-støtte)
(function ensureConversionTools() {
  ...
  console.log("[tools] Installerer konverteringsverktøy (dette tar 1-3 min første gang)…");
  try {
    ...
    console.log("[tools] Installasjon fullført ✓");
  } catch(e) {
