...
// GET /api/lab/remote/status  – hent status fra watchdog
app.get("/api/lab/remote/status", auth, adminOnly, async function(req, res) {
  if (!WATCHDOG_URL) return res.json({ ok: false, online: false, url: "", error: "WATCHDOG_URL ikke konfigurert" });
  try {
    const r = await watchdogRequest("GET", "/status");
    res.json({ ok: true, online: r.data.online, version: r.data.version || null, url: WATCHDOG_URL, status: r.data });
  } catch(e) {
    res.json({ ok: false, online: false, url: WATCHDOG_URL, error: e.message });
  }
});
...
app.get("/api/lab/remote/backups", auth, adminOnly, async function(req, res) {
  try {
    const r = await watchdogRequest("GET", "/backups");
    res.json(r.data);
  } catch(e) {
...
app.post("/api/lab/remote/control", auth, adminOnly, rateLimit(10, 60000), async function(req, res) {
  ...
  if (action === "restore") {
    ...
    try {
      ...
      // Restart containeren etter restore
