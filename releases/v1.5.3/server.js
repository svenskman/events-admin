...
function serveDepartmentPage(dept, req, res) {
  ...
  html += '</div><footer>' + escHtml(siteName) + ' · ' + escHtml(dept.name) + '</footer>'
    ...
    + '  var aktiv=document.querySelector(".ev-card.active,.aktiv-lbl");\n'
    + '  var target=aktiv||document.querySelector(".ev-card");\n'
    + '  if(target){var top=target.getBoundingClientRect().top+window.scrollY-100;window.scrollTo({top:Math.max(0,top),behavior:"smooth"});}\n'
    + '});\n'
    + '(function(){\n'
