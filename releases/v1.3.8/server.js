...
// ── Crash guard – logg feil men la Docker restart policy håndtere restart ──
process.on("uncaughtException", function(err) {
  console.error("[CRASH] Uncaught exception:", err.message);
  console.error(err.stack);
  process.exit(1); // Docker restart=always starter opp igjen
});
...
const APP_VERSION = "1.3.0"; // Oppdateres automatisk ved installasjon via update/apply
...
var _eventsCacheDirty = true;

function readEventsJSON() {
  if (!_eventsCacheDirty && _eventsCache !== null) return _eventsCache;
  try {
    _eventsCache = JSON.parse(fs.readFileSync(EVENTS_FILE, "utf8"));
...
function getTvState(deptId) {
  return tvState.get(deptId) || {
    ...
    showLogo: true,
    ticker: "",
    colorIntensity: 40,   // 0-100: bakgrunnsfarge-intensitet (default 40%)
  };
}
...
app.use("/api", function(req, res, next) {
  ...
  // Also allow event subdomains (e.g. mitt-event.naf-romerike.no)
...
app.get("/index.html", function(req, res) {
  var _s = getSettings();
  var _theme = (_s && _s.theme) || "dark";
  var _indexHtml = fs.readFileSync(path.join(__dirname, "index.html"), "utf8");
  _indexHtml = _indexHtml.replace(
    "var t = localStorage.getItem('naf_theme') || 'dark';",
    "var t = localStorage.getItem('naf_theme') || '" + _theme + "';"
  );
...
app.get("/api/search", auth, rateLimit(30, 60000), function(req, res) {
  ...
  events.forEach(function(ev) {
    if (!me.permissions.isAdmin && !me.permissions.isManager) {
      if (ev.department && ev.department !== me.permissions.myDepartment) return;
    }
    const haystack = [ev.title, ev.location, ev.description, ev.eventType].filter(Boolean).join(" ").toLowerCase();
...
app.get("/api/lab/files", auth, adminOnly, function(req, res) {
  if (!IS_LAB) return res.status(404).json({ error: "Not found" });
...
// Returnerer om dette er lab-instansen – ingen ekstern avhengighet
app.get("/api/is-lab", auth, adminOnly, function(req, res) {
  res.json({ isLab: IS_LAB });
});

// ── Lab-kontroll fra produksjon ──────────────────────────────────
// Produksjons-admin kan styre lab-containeren via LAB_URL env-variabel
const LAB_URL        = process.env.LAB_URL        || "";      // f.eks. https://ddos.lab
const LAB_HOOK_SECRET= process.env.LAB_HOOK_SECRET|| "";      // delt hemmelighet

// Webhook-mottaker på lab (kjøres kun om IS_LAB)
// POST /api/lab/webhook { action: "restore"|"restart", version?: "1.0.x" }
if (IS_LAB && LAB_HOOK_SECRET) {
  app.post("/api/lab/webhook", function(req, res) {
    const sig = (req.headers["x-lab-signature"] || "").trim();
    const body = JSON.stringify(req.body);
    const expected = require("crypto").createHmac("sha256", LAB_HOOK_SECRET).update(body).digest("hex");
    if (sig !== "sha256=" + expected) return res.status(403).json({ err: "Ugyldig signatur" });

    const action  = req.body.action;
    const version = req.body.version;

    if (action === "restart") {
      res.json({ ok: true, message: "Restarting lab…" });
      setTimeout(function() { process.exit(0); }, 500);
      return;
    }

    if (action === "restore" && version) {
      // Hent backup og gjenopprett
      const backupDir = path.join(DATA_DIR, "backups");
      const backupPath = path.join(backupDir, "v" + version);
      if (!fs.existsSync(backupPath)) {
        return res.status(404).json({ err: "Versjon v" + version + " ikke funnet" });
      }
      try {
        const files = fs.readdirSync(backupPath);
        files.forEach(function(f) {
          fs.copyFileSync(path.join(backupPath, f), path.join(__dirname, f));
        });
        res.json({ ok: true, message: "Gjenoppretter v" + version + ", restartes…", files });
        setTimeout(function() { process.exit(0); }, 500);
      } catch(e) {
    ...
    res.status(400).json({ err: "Ukjent handling: " + action });
  });

  // Signatur-beskyttet backup-liste for prod → lab-henting (ingen session kreves)
  app.get("/api/lab/backups-list", function(req, res) {
    const sig      = (req.headers["x-lab-signature"] || "").trim();
    const expected = require("crypto").createHmac("sha256", LAB_HOOK_SECRET)
      .update("list-backups").digest("hex");
    if (sig !== "sha256=" + expected) return res.status(403).json({ err: "Ugyldig signatur" });
    try {
      const dirs = fs.readdirSync(DATA)
        .filter(function(d) {
          return d.startsWith("_update_backup_") && fs.statSync(path.join(DATA, d)).isDirectory();
        })
        .map(function(d) {
...
// Endepunkter som produksjons-admin bruker for å styre lab (kun på produksjon)
if (!IS_LAB && LAB_URL && LAB_HOOK_SECRET) {
  // Hent status fra lab
  app.get("/api/lab/remote/status", auth, adminOnly, async function(req, res) {
    try {
      const r = await httpGet(LAB_URL + "/api/version");
      const data = JSON.parse(r.body.toString());
      res.json({ ok: true, online: r.status === 200, version: data.version || null, url: LAB_URL });
    } catch(e) {
      res.json({ ok: false, online: false, error: e.message, url: LAB_URL });
    }
  });

  // Hent versjonshistorikk fra lab
  app.get("/api/lab/remote/backups", auth, adminOnly, async function(req, res) {
    try {
      const sig = require("crypto").createHmac("sha256", LAB_HOOK_SECRET)
        .update("list-backups").digest("hex");
      const r = await httpGet(LAB_URL + "/api/lab/backups-list", {
        "x-lab-signature": "sha256=" + sig
      });
      if (r.status !== 200) return res.status(502).json({ err: "Lab svarte HTTP " + r.status });
      const data = JSON.parse(r.body.toString());
      res.json(data);
    } catch(e) {
  ...
  // Send kommando til lab (restore eller restart)
  app.post("/api/lab/remote/control", auth, adminOnly, rateLimit(10, 60000), async function(req, res) {
    const { action, version } = req.body;
    if (!["restart", "restore"].includes(action)) return res.status(400).json({ err: "Ugyldig handling" });
    try {
      const body = JSON.stringify({ action, version });
      const sig  = require("crypto").createHmac("sha256", LAB_HOOK_SECRET).update(body).digest("hex");
      const r    = await new Promise(function(resolve, reject) {
        const mod  = LAB_URL.startsWith("https") ? require("https") : require("http");
        const opts = require("url").parse(LAB_URL + "/api/lab/webhook");
        opts.method  = "POST";
        opts.headers = {
          "Content-Type":    "application/json",
          "Content-Length":  Buffer.byteLength(body),
          "x-lab-signature": "sha256=" + sig,
          "User-Agent":      "EventsAdmin-Prod/" + APP_VERSION,
        };
        const req2 = mod.request(opts, function(resp) {
          const chunks = [];
          resp.on("data", function(c) { chunks.push(c); });
          resp.on("end",  function()  { resolve({ status: resp.statusCode, body: Buffer.concat(chunks).toString() }); });
        });
      ...
      res.json({ ok: r.status === 200, status: r.status, result });
    } catch(e) {
      res.status(502).json({ err: "Kan ikke nå lab: " + e.message });
    }
  });
}
...
app.post("/api/update/apply", auth, rateLimit(3, 60000), adminOnly, async function(req, res) {
...
// Restore from backup
app.post("/api/update/restore/:version", auth, rateLimit(3, 60000), adminOnly, function(req, res) {
...
app.post("/api/update/publish", auth, rateLimit(3, 60000), adminOnly, async function(req, res) {
...
app.post("/api/update/upload", auth, rateLimit(5, 60000), adminOnly, function(req, res) {
  if (!IS_LAB) return res.status(400).json({ err: "Only available on the lab instance" });
  ...
  labUploadMulter(req, res, function(err) {
    ...
    // Hent versionsnummer fra ZIP/server.js FØR vi skriver filer
    function extractVersionFromBuf(buf) {
      const m = buf.toString("utf8").match(/const APP_VERSION\s*=\s*["']([^"']+)["']/);
      return m ? m[1] : null;
    }

    // Samle alle filer først slik at vi kan lese version før vi skriver
    const pendingFiles = {};
    ...
    files.forEach(function(f) {
      ...
      if (filename.toLowerCase().endsWith(".zip")) {
        try {
          ...
          Object.assign(pendingFiles, zipFiles);
        } catch(e) {
          errors.push("ZIP-feil: " + e.message);
        }
      } else if (allowedFiles.includes(filename)) {
        pendingFiles[filename] = buf;
      } else {
        errors.push("Ugyldig filnavn: " + filename);
      }
    });

    if (!Object.keys(pendingFiles).length && errors.length > 0)
      return res.status(400).json({ err: errors.join(", ") });

    // Les ny versjon fra server.js i ZIP (om den finnes)
    const newVersion = pendingFiles["server.js"]
      ? extractVersionFromBuf(pendingFiles["server.js"])
      : null;

    // Patch ny versjon inn i server.js hvis versionsnummeret ikke er satt
    if (newVersion === null && pendingFiles["server.js"]) {
      // Versjon ikke funnet – patch inn APP_VERSION fra req.body.version om oppgitt
      const reqVersion = req.body && req.body.version;
      if (reqVersion) {
        const patched = pendingFiles["server.js"].toString("utf8").replace(
          /const APP_VERSION\s*=\s*["'][^"']*["']/,
          'const APP_VERSION = "' + reqVersion + '"'
        );
        pendingFiles["server.js"] = Buffer.from(patched, "utf8");
      }
    }

    // Bruk versjonsnummer for å navngi backup
    const uploadedVersion = (pendingFiles["server.js"]
      ? extractVersionFromBuf(pendingFiles["server.js"])
      : null) || APP_VERSION;

    // Lag versjonert backup av eksisterende filer (samme format som remote/backups forventer)
    const backupDir = path.join(DATA, "_update_backup_" + APP_VERSION);
    try { fs.mkdirSync(backupDir, { recursive: true }); } catch(e) {}
    Object.keys(pendingFiles).forEach(function(name) {
      const src = path.join(process.cwd(), name);
      if (fs.existsSync(src)) {
        try { fs.copyFileSync(src, path.join(backupDir, name)); } catch(e) {}
      }
    });
    console.log("[lab-upload] Backup lagret til _update_backup_" + APP_VERSION);

    // Skriv nye filer til disk
    ...
    Object.keys(pendingFiles).forEach(function(name) { saveFile(name, pendingFiles[name]); });

    if (uploaded.length === 0 && errors.length > 0)
      return res.status(400).json({ err: errors.join(", ") });

    console.log("[lab-upload] Lastet opp:", uploaded.map(function(f) { return f.name; }).join(", "), "→ v" + uploadedVersion);
    res.json({ ok: true, uploaded, errors, version: uploadedVersion, message: "Filer lastet opp! Restart for å aktivere v" + uploadedVersion + "." });
  });
});

// LAB: Restart serveren (aktiverer opplastede filer)
app.post("/api/update/restart", auth, rateLimit(3, 60000), adminOnly, function(req, res) {
  if (!IS_LAB) return res.status(400).json({ err: "Only available on the lab instance" });
...
function buildDeptBackup(deptId, segments) {
  ...
  if (segs.includes("events")) {
    payload.events = readEventsJSON().filter(function(e) {
...
function buildFullBackup(segments) {
  ...
  if (segs.includes("events"))     payload.events     = readEventsJSON();
...
app.post("/api/backup/download", auth, rateLimit(10, 60000), async function(req, res) {
...
app.post("/api/backup/peek", auth, rateLimit(10, 60000), multer({ storage: multer.memoryStorage() }).single("backupFile"), function(req, res) {
  ...
  if (payload.events && payload.events.length) {
    ...
    if (mode === "replace") {
      ...
      const keep = payload.deptId
        ...
        : [];
      writeEventsJSON([...keep, ...payload.events]);
...
app.post("/api/breakglass/restore", auth, rateLimit(3, 3600000), breakglassOnly,
  ...
  async function(req, res) {
    ...
    if (payload.events) {
      writeEventsJSON(payload.events);
      summary.restored += payload.events.length;
...
app.post("/api/breakglass/reset-admin-password", auth, rateLimit(3, 3600000), breakglassOnly, async function(req, res) {
...
app.get("/api/events/:id/group-range", auth, function(req, res) {
  const ev = readEventsJSON().find(function(e) { return e.id === req.params.id; });
...
// Offentlig branding-info for login-siden (ingen auth nødvendig)
app.get("/api/branding", function(req, res) {
  const s = getSettings();
  res.json({
    siteName: s.siteName || "Events Admin",
    logoUrl:  s.logoUrl  || null,
    theme:    s.theme    || "dark",
    colors:   { accent: (s.colors && s.colors.accent) || null },
  });
});
...
app.put("/api/settings", auth, adminOnly, function(req, res) {
  ...
  const allowed = ["siteName","eventDomain","contactEmail","gdprRetentionDays",
    ...
    "regFields","orgLevels","orgRoles","departments","emailTemplates","typeNames","roleNames","faviconCrop","layout"];
...
// ── Ventende gjestebok-kommentarer (for varsling) ─────────────────
app.get("/api/guestbook/pending", auth, managerOrAdmin, function(req, res) {
  const events = readEventsJSON();
  const result = [];
  events.forEach(function(ev) {
    if (!ev.guestbook || !ev.guestbook.length) return;
    const pending = ev.guestbook.filter(function(g) { return !g.approved; });
    if (pending.length) {
      result.push({
        evId:    ev.id,
        evTitle: ev.title,
        ...
        entries: pending.map(function(g) {
          return { id: g.id, name: g.name, message: (g.message||"").slice(0, 120), createdAt: g.createdAt };
        })
      });
    }
  });
...
app.get("/api/events", auth, function(req, res) {
  ...
  res.json(readEventsJSON().filter(function(e) {
    // Admin ser alt
    if (user.role === "admin") return true;
...
app.put("/api/events/:id", auth, managerOrAdmin, function(req, res, next) {
  ...
  if (req.body.liveTrackingUrl !== undefined) {
    const url = (req.body.liveTrackingUrl || "").trim();
    events[i].liveTrackingUrl = url || null;
  }
  ...
  if (req.body.isPublished !== undefined)      events[i].isPublished     = req.body.isPublished === "true";
...
app.delete("/api/events/:id", auth, managerOrAdmin, function(req, res) {
  ...
  writeEventsJSON(events.filter(function(e) { return e.id !== req.params.id; }));
...
// ║  IKKE rediger andre deler av filen inne i denne blokken          ║
// ╚══════════════════════════════════════════════════════════════════╝
// ── Guestbook API ────────────────────────────────────────────────
...
app.post("/api/events/:id/guestbook/:gid/photo-token", rateLimit(10, 60000), function(req, res) {
  ...
  const ev   = readEventsJSON().find(function(e) { return e.id === evId || e.slug === evId; });
  if (!ev) return res.status(404).json({ error: "Not found" });
...
app.post("/api/admin/gdpr/anonymize", auth, rateLimit(5, 60000), adminOnly, function(req, res) {
...
function serveEventPage(req, res) {
  const ev = readEventsJSON().find(function(e) { return e.slug === req.eventSlug; });
...
async function submitShiftSignup() {
  ...
  if (r.ok) {
    ...
    console.error("[submitShiftSignup]", _e.message);
  }}
...
async function submitGb(){
  ...
  if(r.ok){
    ...
    console.error("[submitGb]", _e.message);
  }}
...
async function doRegister(){
  ...
  if(r.ok){
    ...
    console.error("[doRegister]", _e.message);
  }}
...
async function checkInReg(id){
  ...
  if(r.ok){
    ...
    console.error("[checkInReg]", _e.message);
  }}
...
async function refreshRegList(){
  ...
  if(counter)counter.textContent=active.length+' påmeldte · '+active.filter(function(x){
  try {return x.checkedIn;
  } catch(_e) {
    console.error("[doRegister]", _e.message);
  }}).length+' sjekket inn';
}
async function doRegister(){
  ...
  if(r.ok){
    ...
    msg.innerHTML='<div class="msg msg-
  try {err">'+esc(d.error||'Feil')+'</div>';
...
function filterRegs(q){
  q=q.toLowerCase();
  document.querySelectorAll('#regList .reg-item').forEach(function(el){
    el.style.display=(!q||el.dat
  try {aset.name.includes(q))?'flex':'none';
...
async function refreshRegList(){
  ...
  if(counter)c
  try {ounter.textContent=active.length+' påmeldte · '+active.filter(function(x){return x.checkedIn;}).length+' sjekket inn';
...
function filterRegs(q){
  ...
  try {egList .reg-item').forEach(function(el){
    el.style.display=(!q||el.dataset.name.includes(q))?'flex':'none';
  });
...
// ── Inventar bilde via QR (mobil) ────────────────────────────────
const _invPhotoTokens = new Map(); // token → { itemId, expires }
const _invPhotoSSE    = new Map(); // token → res

app.post("/api/inventar/:id/photo-token", auth, managerOrAdmin, rateLimit(10, 60000), function(req, res) {
  const token   = require("crypto").randomBytes(16).toString("hex");
  const expires = Date.now() + 15 * 60 * 1000;
  _invPhotoTokens.set(token, { itemId: req.params.id, expires });
  res.json({ token, url: "/inv-photo/" + token });
});

app.get("/api/inv-photo-wait/:token", function(req, res) {
  if (!_invPhotoTokens.has(req.params.token)) return res.status(404).end();
  res.setHeader("Content-Type", "text/event-stream");
  res.setHeader("Cache-Control", "no-cache");
  res.setHeader("Connection", "keep-alive");
  res.flushHeaders();
  _invPhotoSSE.set(req.params.token, res);
  req.on("close", function() { _invPhotoSSE.delete(req.params.token); });
});

app.post("/api/inv-photo-done/:token", function(req, res) {
  var sseRes = _invPhotoSSE.get(req.params.token);
  if (sseRes) { sseRes.write("data: " + JSON.stringify({ done: true }) + "\n\n"); sseRes.end(); _invPhotoSSE.delete(req.params.token); }
  res.json({ ok: true });
});

app.get("/inv-photo/:token", function(req, res) {
  const info = _invPhotoTokens.get(req.params.token);
  if (!info || Date.now() > info.expires) return res.send('<html><body style="font-family:sans-serif;text-align:center;padding:2rem"><h2>⏰ Lenken er utløpt</h2></body></html>');
  const s = getSettings();
  const accent = (s.colors && s.colors.accent) || "#FFD100";
  res.send(`<!DOCTYPE html><html lang="no"><head><meta charset="UTF-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/>
<title>Last opp bilde</title>
<style>*{box-sizing:border-box}body{font-family:sans-serif;background:#111;color:#eee;display:flex;flex-direction:column;align-items:center;min-height:100vh;padding:2rem 1rem;text-align:center}
.btn{display:block;width:100%;max-width:320px;padding:.9rem;border-radius:12px;border:none;font-size:1rem;font-weight:700;cursor:pointer;margin:.5rem 0}
.btn-y{background:${accent};color:#111}.btn-ghost{background:#222;color:#eee;border:1px solid #444}
img.preview{width:100%;max-width:320px;border-radius:10px;margin:.75rem 0;border:3px solid ${accent}}
</style></head><body>
<h2 style="margin-bottom:.25rem">📸 Last opp bilde</h2>
<p style="color:#888;margin-bottom:1.5rem;font-size:.9rem">Velg et bilde til utstyrsregisteret</p>
...
<button class="btn btn-ghost" onclick="document.getElementById('f').click()">📷 Velg / ta bilde</button>
<button class="btn btn-y" id="upBtn" style="display:none" onclick="upload()">⬆ Last opp</button>
<div id="msg" style="margin-top:1rem;font-size:.9rem"></div>
<script>
var file;
function preview(inp){file=inp.files[0];if(!file)return;var r=new FileReader();r.onload=function(e){var i=document.getElementById('prev');i.src=e.target.result;i.style.display='block';document.getElementById('upBtn').style.display='block';};r.readAsDataURL(file);}
async function upload(){
  if(!file)return;
  document.getElementById('msg').textContent='Laster opp…';
  document.getElementById('upBtn').disabled=true;
  var fd=new FormData();fd.append('photo',file);
  var r=await fetch('/api/inv-photo-upload/${req.params.token}',{method:'POST',body:fd});
  var d=await r.json();
  if(d.ok){document.getElementById('msg').textContent='✅ Bilde lastet opp!';document.getElementById('upBtn').style.display='none';await fetch('/api/inv-photo-done/${req.params.token}',{method:'POST'});}
...
app.post("/api/inv-photo-upload/:token", upload.single("photo"), function(req, res) {
  const info = _invPhotoTokens.get(req.params.token);
  if (!info || Date.now() > info.expires) return res.status(403).json({ error: "Utløpt" });
  if (!req.file) return res.status(400).json({ error: "Ingen fil" });
  const photoUrl = "/uploads/" + path.basename(req.file.filename);
  // Send til SSE-lytter (admin-browseren)
  const sseRes = _invPhotoSSE.get(req.params.token);
  if (sseRes) sseRes.write("data: " + JSON.stringify({ photoUrl, itemId: info.itemId }) + "\n\n");
  res.json({ ok: true, photoUrl });
});
...
app.get("/api/geocode/nominatim", auth, rateLimit(60, 60000), async function(req, res) {
  const q           = (req.query.q || "").trim().slice(0, 200);
  const countrycodes = req.query.countrycodes || "no,se,dk,fi,is";
  ...
  try {
    ...
    const r = await fetch(url, {
      headers: {
        "User-Agent": "EventsAdmin/1.3 (" + (process.env.BASE_DOMAIN || DOMAIN) + ")",
        "Accept": "application/json",
        "Accept-Language": "no,nb;q=0.9,sv;q=0.8"
      }
    });
    if (!r.ok) return res.status(502).json({ error: "Nominatim feil: " + r.status });
    const d = await r.json();
    res.json(d);
  } catch(e) {
    res.status(502).json({ error: "Geocoding utilgjengelig: " + e.message });
  }
});
...
app.put("/api/events/:id/route", auth, managerOrAdmin, function(req, res) {
  ...
  const clean = days.map(d => ({
    ...
    etapper: Array.isArray(d.etapper) ? d.etapper.slice(0, 50).map(e => ({
      ...
      // Fleksibilitets-data
      activeAlternative: e.activeAlternative != null ? parseInt(e.activeAlternative) : undefined,
      alternatives: Array.isArray(e.alternatives) ? e.alternatives.slice(0, 10).map(a => ({
        fra:     (a.fra   || "").trim().slice(0, 200),
        til:     (a.til   || "").trim().slice(0, 200),
        km:      a.km  != null && a.km  !== "" ? parseFloat(a.km)  || null : null,
        tid:     a.tid != null && a.tid !== "" ? parseFloat(a.tid) || null : null,
        profile: ["driving","scenic","gravel","foot"].includes(a.profile) ? a.profile : "driving",
        notat:   (a.notat || "").trim().slice(0, 300),
      })) : undefined,
      poi: Array.isArray(e.poi) ? e.poi.filter(p => p.added && !p.noCoords && !p.type).slice(0, 20).map(p => ({
        name: (p.name || "").slice(0, 200),
        cat:  (p.cat  || "annet").slice(0, 50),
        icon: (p.icon || "📍").slice(0, 4),
        lat:  typeof p.lat === "number" ? p.lat : undefined,
        lon:  typeof p.lon === "number" ? p.lon : undefined,
        added: true,
      })) : undefined,
      votes: e.votes && typeof e.votes === "object" ? Object.fromEntries(
        Object.entries(e.votes).map(([k, v]) => [k, Array.isArray(v) ? v.slice(0, 100).map(n => String(n).slice(0, 100)) : []])
      ) : undefined,
    })) : [],
  }));
  ...
  // Lagre ankerpunkt-lista for seinere redigering
  if (typeof req.body.waypointListText === "string") {
    events[i].waypointListText = req.body.waypointListText.slice(0, 5000);
  }
  writeEventsJSON(events);
  broadcastEventUpdate(events[i].department);
  res.json({ ok: true });
...
// ── OSM tile-proxy med stilvalg ───────────────────────────────────
app.get("/api/tile/:style/:z/:x/:y", rateLimit(500, 60000), async function(req, res) {
  const { style, z, x, y } = req.params;
  const zi=parseInt(z), xi=parseInt(x), yi=parseInt(y);
  if (isNaN(zi)||isNaN(xi)||isNaN(yi)||zi<0||zi>19) return res.status(400).end();
  const servers = ["a","b","c"];
  const s = servers[(xi+yi) % 3];
  const urls = {
    voyager: `https://${s}.basemaps.cartocdn.com/rastertiles/voyager/${zi}/${xi}/${yi}.png`,
    light:   `https://${s}.basemaps.cartocdn.com/light_all/${zi}/${xi}/${yi}.png`,
    osm:     `https://${s}.tile.openstreetmap.org/${zi}/${xi}/${yi}.png`,
  };
  const url = urls[style] || urls.voyager;
  try {
    const r = await fetch(url, { headers: { "User-Agent": "EventsAdmin/1.0 (self-hosted)", "Referer": "https://admin.ddos.nu" } });
...
// Bakoverkompatibel redirect for gammel URL uten stil
app.get("/api/tile/:z/:x/:y", rateLimit(500, 60000), async function(req, res) {
  res.redirect("/api/tile/voyager/"+req.params.z+"/"+req.params.x+"/"+req.params.y);
});

// ── Hent rutede koordinater for kartbilde ─────────────────────────
app.get("/api/events/:id/routecoords", auth, rateLimit(10, 60000), async function(req, res) {
  const events = readEventsJSON();
  const ev = events.find(function(e) { return e.id === req.params.id; });
  if (!ev) return res.status(404).json({ error: "Not found" });
  ...
  const INTERNAL_PATTERNS = /^(etappe\s*\d*\s*(til|fra)?:?\s*\S*|dag\s*\d|rasteplass|bensin.*stopp|stopp\s*\d|—|–|-{2,}|\.{2,}|\s*)$/i;

  // Event-spesifikk geocache – ikke global, slik at gamle steder fra andre events ikke blandes inn
  if (!global._evGeocodeCache) global._evGeocodeCache = {};
  const evId = req.params.id;
  if (req.query.refresh === "1") delete global._evGeocodeCache[evId]; // tøm ved refresh
  if (!global._evGeocodeCache[evId]) global._evGeocodeCache[evId] = {};
  const evCache = global._evGeocodeCache[evId];

  async function geocode(name) {
    if (!name || name.trim().length < 3) return null;
    if (INTERNAL_PATTERNS.test(name.trim())) return null;
    if (evCache[name]) return evCache[name];
    try {
      const url = "https://nominatim.openstreetmap.org/search?format=json&limit=3&q="
        + encodeURIComponent(name)
        + "&countrycodes=no,se,dk,fi,is"
        + "&viewbox=3.0,55.0,32.0,72.0&bounded=0";
      const r = await fetch(url, { headers: { "User-Agent": "EventsAdmin/1.0" } });
      const d = await r.json();
      if (d && d[0]) {
        const nordic = d.find(function(r2) {
          const la = parseFloat(r2.lat), lo = parseFloat(r2.lon);
          return la >= 54 && la <= 72 && lo >= -5 && lo <= 35;
        });
        if (nordic) {
          evCache[name] = { lat: parseFloat(nordic.lat), lon: parseFloat(nordic.lon), display: nordic.display_name };
        }
      }
    } catch(e) {}
    return evCache[name] || null;
  }

  async function fetchSegment(from, to, profile) {
    try {
      if (profile === "scenic" && process.env.GRAPHHOPPER_KEY) {
        const pts = "point="+from.lat+"%2C"+from.lon+"&point="+to.lat+"%2C"+to.lon;
        const r = await fetch("https://graphhopper.com/api/1/route?"+pts+"&profile=car&avoid=motorway&locale=no&calc_points=true&points_encoded=false&key="+process.env.GRAPHHOPPER_KEY);
        const d = await r.json();
        if (d.paths && d.paths[0]) return d.paths[0].points.coordinates.map(function(c){ return {lat:c[1],lon:c[0]}; });
      } else {
        const coordStr = from.lon+","+from.lat+";"+to.lon+","+to.lat;
  ...
  const allNames = [];
  days.forEach(function(d) { (d.etapper||[]).forEach(function(e) {
    if (e.fra && !allNames.includes(e.fra)) allNames.push(e.fra);
    if (e.til && !allNames.includes(e.til)) allNames.push(e.til);
  }); });
  ...
  for (var i = 0; i < allNames.length; i++) {
    ...
    if (!name || name.trim().length < 3 || INTERNAL_PATTERNS.test(name.trim())) {
      geocodeStatus.push({ name, ok: false, reason: "internt navn – hopper over" });
      continue;
    }
    const g = await geocode(name);
    if (g) {
      coordCache[name] = g;
      geocodeStatus.push({ name, ok: true, display: g.display, lat: g.lat, lon: g.lon });
    } else {
      geocodeStatus.push({ name, ok: false, reason: "ikke funnet i Skandinavia" });
    }
    if (i < allNames.length-1) await sleep(250);
  }

  // Hjelpefunksjon: avstand i km mellom to koordinater (Haversine)
  ...
  // Filtrer OSRM-segmenter som inneholder umulige koordinater:
  // Koordinater utenfor Skandinavia + buffer, eller enkeltpunkter som er >800km fra begge endepunktene
  function filterSegment(seg, fra, til) {
    const maxKm = Math.max(haversineKm(fra, til) * 3.5, 200); // maks 3,5x luftlinjeavstand
    return seg.filter(function(p) {
      // Innenfor Skandinavia + litt buffer
      if (p.lat < 53 || p.lat > 73 || p.lon < -3 || p.lon > 34) return false;
      // Ikke for langt fra enten fra- eller til-punkt
      if (haversineKm(p, fra) > maxKm && haversineKm(p, til) > maxKm) return false;
      return true;
    });
  }

  const allCoords = [];   // flat liste for bakoverkompatibilitet
  const dayCoords = [];   // [[{lat,lon},...], ...] – én liste per dag
  const segmentStatus = [];
  for (var di = 0; di < days.length; di++) {
    const etapper = days[di].etapper || [];
    const daySeg = [];
    for (var ei = 0; ei < etapper.length; ei++) {
      const e = etapper[ei];
      const fra = coordCache[e.fra], til = coordCache[e.til];
      if (!fra || !til) {
        segmentStatus.push({ fra: e.fra, til: e.til, ok: false, reason: !fra ? "Fra ikke geocodet" : "Til ikke geocodet" });
        continue;
      }
      const directKm = haversineKm(fra, til);
      if (directKm > 900) {
        segmentStatus.push({ fra: e.fra, til: e.til, ok: false, reason: "For langt ("+Math.round(directKm)+" km) – mulig feil geocoding" });
        geocodeStatus.push({ name: e.fra+"→"+e.til, ok: false, reason: "Avstand "+Math.round(directKm)+" km – sjekk stedsnavn" });
        continue;
      }
      const seg = await fetchSegment(fra, til, e.profile||"driving");
      const filtered = filterSegment(seg, fra, til);
      filtered.forEach(function(p) { allCoords.push(p); daySeg.push(p); });
      segmentStatus.push({ fra: e.fra, til: e.til, ok: true, points: filtered.length, rawPoints: seg.length });
      if (ei < etapper.length-1) await sleep(150);
    }
    if (daySeg.length) dayCoords.push(daySeg);
    if (di < days.length-1) await sleep(150);
  }

  const waypoints = [];
  days.forEach(function(d) {
    (d.etapper||[]).forEach(function(e) {
      if (e.type === "start"  && coordCache[e.fra]) waypoints.push(Object.assign({}, coordCache[e.fra], {type:"start", name:e.fra}));
      if (e.type === "slutt"  && coordCache[e.til]) waypoints.push(Object.assign({}, coordCache[e.til], {type:"end",   name:e.til}));
      if (e.type === "hotell" && coordCache[e.til]) waypoints.push(Object.assign({}, coordCache[e.til], {type:"hotel", name:e.til}));
    });
  });

  res.json({ coords: allCoords, dayCoords, waypoints, coordCache, geocodeStatus, segmentStatus });
});
// ── Last opp kartbilde fra browser (html2canvas) ─────────────────
app.post("/api/events/:id/routemap-upload", auth, managerOrAdmin,
  ...
  function(req, res) {
    ...
    const crypto = require("crypto");
    const dayIdx = req.body.dayIdx != null ? parseInt(req.body.dayIdx) : null;
    const hash = crypto.createHash("md5").update(req.file.buffer).digest("hex").slice(0, 12);
    const fname = dayIdx != null ? "routemap_day" + dayIdx + "_" + hash + ".png" : "routemap_" + hash + ".png";
    const dest  = path.join(UPLOADS, fname);
    fs.writeFileSync(dest, req.file.buffer);
    const relUrl = "/uploads/" + fname;

    if (dayIdx != null) {
      // Lagre per-dag kartbilde
      if (!events[i].dayMapImages) events[i].dayMapImages = [];
      events[i].dayMapImages[dayIdx] = relUrl;
    } else {
      events[i].routeMapImage = relUrl;
    }
    writeEventsJSON(events);
    res.json({ ok: true, url: relUrl, dayIdx });
  }
);

// ── Statisk kartbilde fra rute ────────────────────────────────────
app.get("/api/events/:id/routemap", auth, rateLimit(20, 60000), async function(req, res) {
  const events = readEventsJSON();
  const ev = events.find(function(e) { return e.id === req.params.id; });
  if (!ev) return res.status(404).json({ error: "Not found" });
  if (ev.eventType !== "tur") return res.status(400).json({ error: "Kun tur-events" });

  const crypto = require("crypto");
  const route  = ev.route || { days: [] };
  const days   = route.days || [];

  // Steg 1: bruk koordinater som er lagret direkte på etappene
  const points = [];
  days.forEach(function(d, di) {
    (d.etapper || []).forEach(function(e) {
      if (e._fra_lat && e._fra_lon && e.fra) points.push({ name: e.fra, lat: e._fra_lat, lon: e._fra_lon, di });
      if (e._lat    && e._lon    && e.til)  points.push({ name: e.til,  lat: e._lat,     lon: e._lon,     di });
    });
  });

  // Dedupliser koordinater
  const seen = new Set();
  let coords = points.filter(function(p) {
    const key = p.lat.toFixed(3) + "," + p.lon.toFixed(3);
    if (seen.has(key)) return false;
    seen.add(key); return true;
  });

  // Steg 2: hvis for få koordinater, geocode stedsnavn via Nominatim (samme som GPX)
  if (coords.length < 2) {
    const allNames = [];
    days.forEach(function(d) {
      (d.etapper || []).forEach(function(e) {
    ...
    if (!allNames.length) return res.status(400).json({ error: "Ingen steder i ruten – legg til etapper først." });

    if (!global._geocodeCache) global._geocodeCache = {};
    const sleep = function(ms) { return new Promise(function(r) { setTimeout(r, ms); }); };

    for (var gi = 0; gi < allNames.length; gi++) {
      const nm = allNames[gi];
      if (global._geocodeCache[nm]) continue;
      try {
        const gurl = "https://nominatim.openstreetmap.org/search?format=json&limit=1&q=" + encodeURIComponent(nm);
        const gr = await fetch(gurl, { headers: { "User-Agent": "EventsAdmin-GPX/1.0" } });
        const gd = await gr.json();
        if (gd && gd[0]) global._geocodeCache[nm] = { lat: parseFloat(gd[0].lat), lon: parseFloat(gd[0].lon) };
      } catch(ge) {}
      if (gi < allNames.length - 1) await sleep(300);
    }

    const geocoded = [];
    days.forEach(function(d, di) {
      (d.etapper || []).forEach(function(e) {
        if (e.fra && global._geocodeCache[e.fra]) geocoded.push({ name: e.fra, ...global._geocodeCache[e.fra], di });
        if (e.til && global._geocodeCache[e.til]) geocoded.push({ name: e.til, ...global._geocodeCache[e.til], di });
      });
    });
    const seen2 = new Set();
    coords = geocoded.filter(function(p) {
      const key = p.lat.toFixed(3) + "," + p.lon.toFixed(3);
  ...
  if (coords.length < 2) return res.status(400).json({ error: "For få koordinater i ruten – legg til Fra/Til på etappene." });
  ...
  const lonSpan = maxLon - minLon || 0.5;
  const padLat  = latSpan * 0.18;
  const padLon  = lonSpan * 0.18;
  ...
  const bLonSpan = bMaxLon - bMinLon;
  const W = 1200, H = 630, PAD = 48;
  const innerW = W - PAD * 2;
  const innerH = H - PAD * 2 - 40;

  function toX(lon) { return (PAD + (lon - bMinLon) / bLonSpan * innerW).toFixed(2); }
  function toY(lat) { return (PAD + (bMaxLat - lat)  / bLatSpan * innerH).toFixed(2); }

  const cacheKey = crypto.createHash("md5").update(
    ev.id + coords.map(function(c) { return c.lat.toFixed(4) + c.lon.toFixed(4); }).join("|")
  ).digest("hex").slice(0, 12);
  const dest = path.join(UPLOADS, "routemap_" + cacheKey + ".svg");

  if (req.query.refresh !== "1" && fs.existsSync(dest)) {
    const age = Date.now() - fs.statSync(dest).mtimeMs;
    if (age < 12 * 3600 * 1000) {
      return res.json({ ok: true, url: "/uploads/" + path.basename(dest), cached: true });
    }
  }

  // SVG-generering
  const polyPts = coords.map(function(c) { return toX(c.lon) + "," + toY(c.lat); }).join(" ");

  const markers = coords.map(function(c, i) {
    const x = toX(c.lon), y = toY(c.lat);
    const isFirst = i === 0, isLast = i === coords.length - 1;
    const fill = isFirst ? "#22c55e" : isLast ? "#ef4444" : "#818cf8";
    const r    = (isFirst || isLast) ? 11 : 4.5;
    const label = isFirst ? "S" : isLast ? "E" : "";
    return (
      '<circle cx="' + x + '" cy="' + y + '" r="' + r + '" fill="' + fill + '" stroke="white" stroke-width="2"/>'
      + (label ? '<text x="' + x + '" y="' + (parseFloat(y) + 4.5).toFixed(1) + '" text-anchor="middle" font-size="11" font-weight="700" fill="white" font-family="system-ui,sans-serif">' + label + '</text>' : "")
    );
  }).join("\n  ");

  const seenDays = new Set();
  const dagLabels = coords.map(function(c, i) {
    if (seenDays.has(c.di) || (c.di === 0 && i === 0)) { seenDays.add(c.di); return ""; }
    seenDays.add(c.di);
    const x = parseFloat(toX(c.lon)), y = parseFloat(toY(c.lat)) - 18;
    return '<rect x="' + (x-18) + '" y="' + (y-11) + '" width="36" height="13" rx="3" fill="#1e1b4b" stroke="#6366f1" stroke-width="0.8" opacity="0.9"/>'
         + '<text x="' + x + '" y="' + (y+1) + '" text-anchor="middle" font-size="9.5" font-weight="600" fill="#c7d2fe" font-family="system-ui,sans-serif">Dag ' + (c.di + 1) + '</text>';
  }).join("\n  ");

  const startName = escHtml((coords[0].name || "").split(",")[0].slice(0, 22));
  const endName   = escHtml((coords[coords.length-1].name || "").split(",")[0].slice(0, 22));
  const evTitle   = escHtml((ev.title || "Tur").slice(0, 60));
  const totalKm   = days.reduce(function(s, d) {
    return s + (d.etapper || []).reduce(function(s2, e) { return s2 + (parseFloat(e.km) || 0); }, 0);
  }, 0);
  const statsStr  = (days.length > 1 ? days.length + " dager · " : "") + (totalKm > 0 ? Math.round(totalKm) + " km" : "");

  const svg = '<?xml version="1.0" encoding="UTF-8"?>'
    + '<svg xmlns="http://www.w3.org/2000/svg" width="' + W + '" height="' + H + '" viewBox="0 0 ' + W + ' ' + H + '">'
    + '<defs>'
    + '<linearGradient id="bg" x1="0" y1="0" x2="0.3" y2="1"><stop offset="0%" stop-color="#0f172a"/><stop offset="100%" stop-color="#1e1b4b"/></linearGradient>'
    ...
    + '<g opacity="0.06" stroke="#818cf8" stroke-width="0.5">'
    + Array.from({length:13},function(_,i){return '<line x1="'+(i*100)+'" y1="0" x2="'+(i*100)+'" y2="'+H+'"/>';}).join("")
    + Array.from({length:7}, function(_,i){return '<line x1="0" y1="'+(i*90)+'" x2="'+W+'" y2="'+(i*90)+'"/>';}).join("")
    + '</g>'
    + '<polyline points="' + polyPts + '" fill="none" stroke="#6366f1" stroke-width="9" stroke-linecap="round" stroke-linejoin="round" opacity="0.2" filter="url(#glow)"/>'
    + '<polyline points="' + polyPts + '" fill="none" stroke="#818cf8" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/>'
    + dagLabels
    + markers
    + '<rect x="0" y="' + (H-44) + '" width="' + W + '" height="44" fill="rgba(0,0,0,0.6)"/>'
    + '<line x1="0" y1="' + (H-44) + '" x2="' + W + '" y2="' + (H-44) + '" stroke="#6366f1" stroke-width="1" opacity="0.6"/>'
    + '<circle cx="' + (PAD-8) + '" cy="' + (H-22) + '" r="5" fill="#22c55e"/>'
    + '<text x="' + (PAD+2) + '" y="' + (H-18) + '" font-size="13" fill="#86efac" font-family="system-ui,sans-serif">' + startName + '</text>'
    + '<text x="' + (W/2) + '" y="' + (H-26) + '" text-anchor="middle" font-size="15" font-weight="700" fill="white" font-family="system-ui,sans-serif">' + evTitle + '</text>'
    + (statsStr ? '<text x="' + (W/2) + '" y="' + (H-11) + '" text-anchor="middle" font-size="11" fill="#94a3b8" font-family="system-ui,sans-serif">' + statsStr + '</text>' : "")
    + '<circle cx="' + (W-PAD+6) + '" cy="' + (H-22) + '" r="5" fill="#ef4444"/>'
    + '<text x="' + (W-PAD-4) + '" y="' + (H-18) + '" text-anchor="end" font-size="13" fill="#fca5a5" font-family="system-ui,sans-serif">' + endName + '</text>'
    + '</svg>';

  fs.writeFileSync(dest, Buffer.from(svg, "utf8"));
  const relUrl = "/uploads/" + path.basename(dest);
  const evIdx = events.findIndex(function(e) { return e.id === req.params.id; });
  if (evIdx !== -1) { events[evIdx].routeMapImage = relUrl; writeEventsJSON(events); }
  res.json({ ok: true, url: relUrl, cached: false, format: "svg" });
});

// ── Stemming på vegalternativ ─────────────────────────────────────
app.post("/api/events/:id/route/vote", auth, rateLimit(30, 60000), function(req, res) {
  const events = readEventsJSON();
  const i = events.findIndex(e => e.id === req.params.id);
  if (i === -1) return res.status(404).json({ err: "Not found" });

  const { di, ei, altIdx, name } = req.body;
  if (typeof di !== "number" || typeof ei !== "number" || typeof altIdx !== "number" || !name)
    return res.status(400).json({ err: "Mangler felt" });

  const ev = events[i];
  if (!ev.route || !ev.route.days || !ev.route.days[di] || !ev.route.days[di].etapper[ei])
    return res.status(400).json({ err: "Ugyldig etappe" });

  const etappe = ev.route.days[di].etapper[ei];
  if (!etappe.votes) etappe.votes = {};

  // Fjern eksisterende stemme fra same navn
  Object.keys(etappe.votes).forEach(k => {
    etappe.votes[k] = (etappe.votes[k] || []).filter(n => n !== name);
  });

  // Legg til ny stemme
  const key = String(altIdx);
  if (!etappe.votes[key]) etappe.votes[key] = [];
  etappe.votes[key].push(String(name).slice(0, 100));

  // Sett vinnende alternativ som aktivt
  const winner = Object.keys(etappe.votes).reduce((best, k) =>
    (etappe.votes[k].length > (etappe.votes[best] || []).length ? k : best), Object.keys(etappe.votes)[0]);
  etappe.activeAlternative = parseInt(winner);
...
app.post("/api/ai/route-from-waypoints", auth, managerOrAdmin, rateLimit(10, 60000), async function(req, res) {
  ...
  if (!Array.isArray(waypoints) || waypoints.length < 2)
    return res.status(400).json({ error: "Minst 2 ankerpunkter kreves" });

  const wps = waypoints.slice(0, 12).map(w => String(w).trim().slice(0, 200)).filter(Boolean);
  const labels = Array.isArray(waypointLabels) ? waypointLabels.map(l => String(l||'').trim().slice(0,100)) : wps.map(()=>'');
  const usedProfile   = ["driving","scenic","gravel"].includes(profile) ? profile : "scenic";
  const numAlts       = Math.min(2, Math.max(0, parseInt(altCount) || 1));
  const profileMap    = {
    scenic:  { name: "LANDEVEI",     desc: "kun landeveier og riksveier – INGEN motorveier. Pittoreske ruter, kystveier, fjellveier." },
    driving: { name: "MOTORVEI",     desc: "primært motorveier og hovedveier." },
    gravel:  { name: "GRUS/OFFROAD", desc: "grusveier og skogsveier – unngå asfalt der mulig." },
  };
  const profileDesc = profileMap[usedProfile];

  const allWps = roundtrip ? [...wps, wps[0]] : wps;
  const allLabels = roundtrip ? [...labels, ''] : labels;
  const segments = [];
  for (let i = 0; i < allWps.length - 1; i++) {
    segments.push({ fra: allWps[i], til: allWps[i + 1], tilLabel: allLabels[i + 1] || '' });
  }

  // Estimer dager: grovt 300-400 km per dag på landevei, men split på ankerpunkter
  const numDays = Math.max(1, Math.ceil(wps.length / 3));

  const fmtDate = d => d ? new Date(d).toLocaleDateString("nb-NO", { weekday:"long", day:"numeric", month:"long" }) : "";
  const startDesc = startDate ? fmtDate(startDate) : "";
  const endDesc   = endDate   ? fmtDate(endDate)   : "";

  const altInstructions = numAlts > 0
    ? `For hvert segment: generer ETT hoved-alternativ og ${numAlts} vegalternativ. Alternativene skal bruke ANDRE veger mellom de samme by/tettstedene – gjerne via mer pittoreske ruter, kystveier, fjellveier eller historiske strekninger.`
    : "Generer kun én rute per segment (ingen alternativer).";

  // Bygg segmentliste til prompten
  const segList = segments.map((s, i) => {
    const labelNote = s.tilLabel ? ` [${s.tilLabel}]` : '';
    return `  Segment ${i+1}: ${s.fra} → ${s.til}${labelNote}`;
  }).join("\n");

  const prompt = `Du er en ekspert på MC-turer og bilutflukter i Norge og Skandinavia.

Planlegg en tur som MÅ passere gjennom følgende ankerpunkter i nøyaktig denne rekkefølgen:

${segList}
${roundtrip ? `  (returner deretter til ${wps[0]})` : ""}

VEIPROFIL: ${profileDesc.name} – ${profileDesc.desc}
${startDesc ? `AVREISE: ${startDesc}` : ""}${endDesc ? `\nHJEMKOMST: ${endDesc}` : ""}

ETIKETTER – tolkningsregler for [klammer] etter stedsnavn:
- [Overnatting] / [Hotell]: type="hotell" – dag slutter her
- [Lunsj] / [Husmanskost]: type="lunsj"
- [Middag]: type="middag"
- [Kafé] / [Kaffe]: type="stopp" med notat
- [Bensin] / [Bensinstopp]: type="bensin"
- [Museum]: type="opplevelse", opplevelseSubtype="museum"
- [Historisk] / [Kirke] / [Stavkirke]: type="opplevelse", opplevelseSubtype="historisk"
- [Utsikt] / [Panorama] / [Fjell]: type="opplevelse", opplevelseSubtype="utsikt"
- [Natur] / [Fjord] / [Foss]: type="opplevelse", opplevelseSubtype="natur"
- [Ferje]: type="stopp", notat nevner ferjeovergangen
- [Aktivitet]: type="opplevelse", opplevelseSubtype="aktivitet"
- Andre etiketter: bruk som notat-tekst

${altInstructions}

╔══════════════════════════════════════════╗
║  ABSOLUTTE REGLER – IKKE BRYT DISSE     ║
╠══════════════════════════════════════════╣
║ 1. LISTEN ER LISTEN. Legg IKKE til      ║
║    ekstra stopp, lunsjer, opplevelser   ║
║    eller overnattinger som IKKE er      ║
║    oppgitt i ankerpunktlisten.          ║
║                                          ║
║ 2. Rekkefølgen er LÅST. Ikke flytt,     ║
║    ikke swap, ikke push steder til      ║
║    en annen dag.                        ║
║                                          ║
║ 3. [Overnatting] plasseres på NØYAKTIG  ║
║    den dagen de er oppgitt. Ny dag      ║
║    starter ETTER hotell-etappen.        ║
║                                          ║
║ 4. Ikke legg til lunsj, middag eller    ║
║    andre stopp mellom ankerpunktene     ║
║    med mindre de er eksplisitt i        ║
║    listen.                              ║
║                                          ║
║ 5. profile = "${usedProfile}" på ALLE  ║
╚══════════════════════════════════════════╝
  ...
  "days": [
    {
      ...
      "etapper": [
        {
          "type": "start|stopp|opplevelse|lunsj|middag|hotell|slutt|bensin",
          "fra": "stedsnavn",
          "til": "stedsnavn",
          "km": tall,
          "profile": "${usedProfile}",
          "notat": "kort beskrivelse maks 80 tegn",
          "opplevelseSubtype": "museum|natur|utsikt|historisk|aktivitet|kultur|annet",
          "alternatives": []
        }
      ]
    }
...
Struktur:
- Én etappe per segment mellom ankerpunktene (fra → til) – ingen ekstra steder imellom
- Del i dager: ny dag starter etter hotell-etappe
- Første etappe type="start", siste type="slutt"
- Kun JSON, ingen annen tekst`;

  if (MOCK_AI) {
    const mockDays = [];
    for (let d = 0; d < numDays; d++) {
      const dayWps = wps.slice(d * 3, d * 3 + 4);
      const dato = startDate ? (function(){ const dt = new Date(startDate); dt.setDate(dt.getDate()+d); return dt.toISOString().slice(0,10); })() : "";
      mockDays.push({ dato, etapper: [
        { type: d===0?"start":"stopp", fra: dayWps[0]||wps[0], til: dayWps[1]||wps[1], km: 120, profile: usedProfile, notat: "Kjører til neste ankerpunkt", alternatives: numAlts>0 ? [{ fra: dayWps[0]||wps[0], til: dayWps[1]||wps[1], km: 150, profile: usedProfile, notat: "Via kystveien, 30 min lengre men flottere utsikt" }] : [] },
        { type: "opplevelse", fra: dayWps[1]||wps[1], til: "Lokalt utsiktspunkt", km: 15, profile: usedProfile, notat: "Spektakulær fjordutsikt", opplevelseSubtype: "utsikt", alternatives: [] },
        { type: "lunsj", fra: "Utsiktspunkt", til: "Lokal kafé", km: 8, profile: usedProfile, notat: "Anbefalt lunsjsted", alternatives: [] },
        { type: d===numDays-1?"slutt":"hotell", fra: "Kafé", til: d===numDays-1?(roundtrip?wps[0]:wps[wps.length-1]):dayWps[2]||wps[Math.min(d+1, wps.length-1)], km: 80, profile: usedProfile, notat: d===numDays-1?"Hjemkjøring":"Overnatting", alternatives: [] },
      ]});
    }
  ...
  try {
    ...
    try { parsed = JSON.parse(text); } catch(e) {
      return res.status(500).json({ error: "Kunne ikke tolke AI-svar", raw: text.slice(0, 300) });
    }
    const validTypes = ["start","stopp","opplevelse","lunsj","middag","hotell","slutt","bensin"];
    const resultDays = Array.isArray(parsed.days) ? parsed.days.slice(0, 14).map(function(d, di) {
      const dato = d.dato || (startDate ? (function(){ const dt = new Date(startDate); dt.setDate(dt.getDate()+di); return dt.toISOString().slice(0,10); })() : "");
      return {
        dato,
        etapper: Array.isArray(d.etapper) ? d.etapper.slice(0, 15).map(function(e) {
          const alts = Array.isArray(e.alternatives) ? e.alternatives.slice(0, 2).map(a => ({
            fra:     (a.fra   || "").trim().slice(0, 200),
            til:     (a.til   || "").trim().slice(0, 200),
            km:      a.km != null ? parseFloat(a.km) || null : null,
            tid:     null,
            profile: usedProfile,
            notat:   (a.notat || "").trim().slice(0, 200),
          })) : [];
          return {
...
app.get("/api/events/:id/display/stream", function(req, res) {
  ...
  const evForSse = readEventsJSON().find(function(e) { return e.id === req.params.id || e.slug === req.params.id; });
...
app.post("/api/events/:id/display/state", auth, managerOrAdmin, function(req, res) {
  const _evRaw = readEventsJSON().find(function(e){ return e.id === req.params.id || e.slug === req.params.id; });
...
app.post("/api/events/:id/display/slides", auth, managerOrAdmin,
  ...
  function(req, res) {
    if (!req.file) return res.status(400).json({ error: "Ingen fil" });
    const _evSl = readEventsJSON().find(function(e){ return e.id === req.params.id || e.slug === req.params.id; });
...
app.delete("/api/events/:id/display/slides/:idx", auth, managerOrAdmin, function(req, res) {
  const _evR = readEventsJSON().find(function(e){ return e.id === req.params.id || e.slug === req.params.id; });
...
app.delete("/api/events/:id/display/slides", auth, managerOrAdmin, function(req, res) {
  const _evR2 = readEventsJSON().find(function(e){ return e.id === req.params.id || e.slug === req.params.id; });
...
app.get("/api/tv/:deptId/stream", function(req, res) {
  ...
  const upcoming = events
    .filter(function(e) {
        ...
        // Include all future events (hele kalenderåret)
        return startMs > now - 3600000;
      })
    .sort(function(a, b) {
    ...
    .map(function(e) {
        try {
          ...
          if (route && Array.isArray(route.days) && route.days.length) {
            var totalKm = 0;
            var firstStop = null, lastStop = null;
            var dayLegs = []; // [{fra, til, km}] per dag
            route.days.forEach(function(d) {
              var etapper = (d && d.etapper) || [];
              var dayKm = 0;
              var dayFra = null, dayTil = null;
              etapper.forEach(function(et) {
                if (!et) return;
                if (et.km) totalKm += parseFloat(et.km) || 0;
                if (et.km) dayKm   += parseFloat(et.km) || 0;
                if (et.fra && !firstStop) firstStop = et.fra;
                if (et.fra && !dayFra)   dayFra    = et.fra;
                if (et.til) { lastStop = et.til; dayTil = et.til; }
              });
              if (dayFra || dayTil) dayLegs.push({ fra: dayFra, til: dayTil, km: dayKm > 0 ? Math.round(dayKm) : null });
            });
            routeSummary = {
          ...
          return {
            ...
            dayMapImages: Array.isArray(e.dayMapImages) ? e.dayMapImages : [],
...
app.get("/api/tv/:deptId/state", function(req, res) {
  ...
  const upcoming = events
    .filter(function(e) {
        ...
        return sMs > now - 3600000; // Vis alle fremtidige events
      })
    .sort(function(a, b) {
        var aS = new Date(a.date).getTime(); var bS = new Date(b.date).getTime();
        var aO = aS <= now; var bO = bS <= now;
...
app.post("/api/tv/:deptId/state", auth, managerOrAdmin, function(req, res) {
  ...
  if (req.body.colorIntensity !== undefined) next.colorIntensity = Math.max(0, Math.min(100, parseInt(req.body.colorIntensity) || 40));
...
#evDetail{flex:1;position:relative;display:flex;flex-direction:column;justify-content:center;padding:3vh 3vw 3vh 2vw;overflow:hidden}
#evDetailBg{position:absolute;inset:0;transition:background 1.2s ease}
#evDetailContent{position:relative;z-index:1;display:flex;flex-direction:column;gap:.8vh;font-family:inherit}
...
/* TV-bilder topp/bunn */
#evDetailTopImg{width:100%;max-height:26vh;object-fit:cover;border-radius:1vw;display:none;margin-bottom:0}
#evDetailBottomImg{width:100%;max-height:26vh;object-fit:cover;border-radius:1vw;display:none;margin-top:0}
#evDetailBottomPanel{width:100%;max-height:26vh;border-radius:1vw;overflow:hidden;display:none;margin-top:0}
...
<div id="screen">
  ...
  <div id="eventLayer">
    <!-- Left: original event cards -->\n    <div id=\"evList\"><div id=\"evCards\"></div></div>\n    <!-- Right: active event detail with color glow -->\n    <div id=\"evDetail\">\n      <div id=\"evDetailBg\"></div>\n      <div id=\"evDetailContent\">\n        <!-- TOP: totalkart -->\n        <img id=\"evDetailTopImg\" src=\"\" alt=\"\" style=\"width:100%;max-height:26vh;object-fit:cover;border-radius:1vw;display:none;margin-bottom:0\"/>\n        <!-- MITTEN: info + tracking 50/50 -->\n        <div id=\"evDetailMiddle\" style=\"display:flex;gap:.75vw;align-items:stretch\">\n          <div id=\"evDetailLeft\" style=\"flex:1;min-width:0\">\n            <div id=\"evDetailBadgeRow\">\n              <span id=\"evDetailBadge\"></span>\n              <span id=\"evDetailOngoing\">\n                <span class=\"pulse\" style=\"width:.5vw;height:.5vw;min-width:5px;min-height:5px;border-radius:50%;background:currentColor;display:inline-block\"></span>\n                Pågår nå\n              </span>\n            </div>\n            <div id=\"evDetailTitle\"></div>\n            <div id=\"evDetailMeta\"></div>\n          </div>\n          <div id=\"evDetailTrackImg\" style=\"display:none;flex:1;min-width:0;border-radius:1vw;overflow:hidden;position:relative;align-self:stretch\"></div>\n        </div>\n        <!-- BOTTOM: dagskart -->\n        <img id=\"evDetailBottomImg\" src=\"\" alt=\"\" style=\"width:100%;max-height:26vh;object-fit:cover;border-radius:1vw;display:none;margin-top:0\"/>\n        <div id=\"evDetailBottomPanel\" style=\"display:none;border-radius:1vw;overflow:hidden;margin-top:0;background:#0a0a1a;width:100%\"></div>\n      </div>\n    </div>\n  </div>
...
function rebuildCycle() {
  ...
  // Legg til live tracking-slides for events med aktiv tracking-URL
  var trackingSlots = [];
  for (var ti = 0; ti < evCount; ti++) {
    var tev = upcoming[ti];
    if (tev && tev.liveTrackingUrl) {
      trackingSlots.push({ type: 'tracking', url: tev.liveTrackingUrl, evTitle: tev.title });
    }
  }

  if (!idxList.length) {
    // No slides — each event slot gets its own position
    for (var si2 = 0; si2 < eventSlots.length; si2++) {
      cycle.push(eventSlots[si2]);
    }
    trackingSlots.forEach(function(ts) { cycle.push(ts); });
  } else {
    // Distribute slides evenly between event slots
    var slotCount = eventSlots.length;
    var slidesPerBlock = slotCount > 0 ? Math.ceil(idxList.length / slotCount) : idxList.length;
    var si = 0;
    for (var ei = 0; ei < Math.max(slotCount, 1); ei++) {
      if (wantEvents && eventSlots[ei]) cycle.push(eventSlots[ei]);
...
function step() {
  ...
  if (item.type === "event") {
    ...
    showTrackingSlide(item.url, item.evTitle);
...
function _showEvDetail(upcoming, idx, count, dayIdx) {
  ...
  if (bg) {
    var intensity = (state.colorIntensity != null ? state.colorIntensity : 40) / 100;
    var _bgBase  = IS_DARK ? '#080808' : '#e8e8e8';
    var _bgOuter = IS_DARK ? '#0d0d0d' : '#f0f0f0';
    // Konverter intensity til hex-opasitet (0-255 → 00-ff)
    var hexOp1 = Math.round(intensity * 200).toString(16).padStart(2,'0'); // glow sentrum
    var hexOp2 = Math.round(intensity * 80).toString(16).padStart(2,'0');  // glow kant
    bg.style.background =
      'radial-gradient(ellipse 80% 90% at 40% 50%, '
      + style.color + hexOp1 + ' 0%, '
      + style.color + hexOp2 + ' 50%, '
  ...
  var botPanel = document.getElementById('evDetailBottomPanel');

  // ── TOP: totalkart (alltid for tur-events) ──
  var topSrc = (ev.eventType === 'tur') ? (ev.routeMapImage || ev.tvImageTop || null) : (ev.tvImageTop || null);
  if (topImg) {
    ...
    if (!IS_PREVIEW && ev.eventType === 'tur' && ev.liveTrackingUrl) {
      ...
      trackDiv.innerHTML =
        '<div style="position:absolute;top:0;left:0;right:0;z-index:2;background:rgba(0,0,0,.7);padding:.35vh .7vw;display:flex;align-items:center;gap:.5vw">' +
          '<span style="font-size:1.2vh;color:#f59e0b;font-weight:800;letter-spacing:.04em">📡 LIVE SPORING</span>' +
          '<span style="width:.7vh;height:.7vh;border-radius:50%;background:#ef4444;animation:pulse 1.5s infinite;display:inline-block;margin-left:.3vw"></span>' +
        '</div>' +
        '<iframe src="' + ev.liveTrackingUrl + '" style="width:100%;height:100%;min-height:18vh;border:none;display:block" allow="geolocation" sandbox="allow-scripts allow-same-origin allow-popups"></iframe>';
      window._trackingRefresh = setInterval(function() {
        var ifr = trackDiv && trackDiv.querySelector('iframe');
        if (ifr) ifr.src = ev.liveTrackingUrl + (ev.liveTrackingUrl.includes('?') ? '&' : '?') + '_t=' + Date.now();
      }, 30000);
    } else {
  ...
  // ── BOTTOM: dagskart eller tvImageBottom ──
  if (botPanel) botPanel.style.display = 'none';
  var botSrc = null;
  if (ev.eventType === 'tur') {
    botSrc = (dayIdx != null && ev.dayMapImages && ev.dayMapImages[dayIdx]) ? ev.dayMapImages[dayIdx] : (ev.tvImageBottom || null);
  } else {
    botSrc = ev.tvImageBottom || null;
  }
  if (botImg) {
    if (botSrc) { botImg.src = botSrc; botImg.style.display = 'block'; }
    else        { botImg.src = ''; botImg.style.display = 'none'; }
  }
  if (badge) {
  ...
  if (metaEl) {
    ...
    if (ev.routeSummary) {
      ...
      // Fra → Til
      if (r.fra && r.til) addRow('', r.fra + ' → ' + r.til);
      // Vis dag X av Y hvis dagskart
      if (dayIdx != null && r.days > 1) {
        addRow('Dag:', (dayIdx + 1) + ' av ' + r.days);
        // Fra/til for denne dagen
        var leg = r.dayLegs && r.dayLegs[dayIdx];
        if (leg && leg.fra && leg.til) {
          addRow('', leg.fra + ' → ' + leg.til + (leg.km ? ' · ' + leg.km + ' km' : ''));
        }
      }
    }
...
// ── Live tracking slide ──────────────────────────────────────────
function showTrackingSlide(url, evTitle) {
  var sl      = document.getElementById('slideLayer');
  var evLayer = document.getElementById('eventLayer');
  var urlLay  = document.getElementById('urlLayer');
  if (!sl || !evLayer) return;
  clearInterval(_evCycleTimer);
  sl.style.opacity      = '0';
  evLayer.style.opacity = '0';
  if (urlLay) urlLay.style.opacity = '0';

  // Gjenbruk eller lag tracking-layer
  var trackLay = document.getElementById('trackingLayer');
  if (!trackLay) {
    trackLay = document.createElement('div');
    trackLay.id = 'trackingLayer';
    trackLay.style.cssText = 'position:absolute;inset:0;display:flex;flex-direction:column;opacity:0;transition:opacity .5s';
    document.getElementById('tvContainer') && document.getElementById('tvContainer').appendChild(trackLay);
  }

  // Header-stripe
  trackLay.innerHTML =
    '<div style="position:absolute;top:0;left:0;right:0;z-index:2;background:rgba(0,0,0,.65);padding:.5vh 1.5vw;display:flex;align-items:center;gap:1vw">' +
      '<span style="font-size:1.4vh;color:#f59e0b;font-weight:800;letter-spacing:.05em">📡 LIVE SPORING</span>' +
      '<span style="font-size:1.4vh;color:rgba(255,255,255,.7)">' + (evTitle || '') + '</span>' +
      '<span id="trackingPing" style="margin-left:auto;font-size:1.1vh;color:rgba(255,255,255,.4)">Oppdaterer hvert 30. sek</span>' +
    '</div>' +
    '<iframe id="trackingIframe" src="' + url + '" style="flex:1;border:none;width:100%;height:100%" allow="geolocation" sandbox="allow-scripts allow-same-origin allow-popups"></iframe>';

  trackLay.style.opacity = '1';

  // Oppdater iframe hvert 30. sekund
  clearInterval(window._trackingRefresh);
  window._trackingRefresh = setInterval(function() {
    var iframe = document.getElementById('trackingIframe');
    if (iframe) {
      iframe.src = url + (url.includes('?') ? '&' : '?') + '_t=' + Date.now();
      var ping = document.getElementById('trackingPing');
      if (ping) { ping.style.color = '#4ade80'; setTimeout(function(){ping.style.color='rgba(255,255,255,.4)';}, 1000); }
    }
  }, 30000);
}
...
app.get("/m", auth, function(req, res) {
  ...
  const siteName  = settings.siteName || "Events Admin";
  const accent    = (settings.colors && settings.colors.accent) || "#FFD100";
  const eventsAll = readEventsJSON();
  const user      = req.session.user;
  const now       = new Date();
...
app.get("*", function(req, res) {
  ...
  // Injiser server-kjent tema for å unngå blink ved første besøk
  var _s = getSettings();
  var _theme = (_s && _s.theme) || "dark";
  var _indexHtml = fs.readFileSync(path.join(__dirname, "index.html"), "utf8");
  _indexHtml = _indexHtml.replace(
