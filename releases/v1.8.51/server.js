...
// Join the group — reads live tracker position and opens Google Maps navigation
async function catchUpToTracker() {
  var btn = document.getElementById('catchUpBtn');
  if (btn) { btn.disabled = true; btn.textContent = '⏳ Finding group…'; }
  try {
    // Fetch the Sizzapp page and extract coordinates from the HTML
    var r = await fetch(LIVE_TRACKING_URL, { mode: 'cors' });
    var html = await r.text();
    var lat = null, lon = null;
    // Try common coordinate patterns in Sizzapp page HTML
    var m = html.match(/"lat(?:itude)?"\s*:\s*([-\d.]+).*?"l(?:ng|on)(?:gitude)?"\s*:\s*([-\d.]+)/s)
          || html.match(/lat=([-\d.]+)&(?:amp;)?lon=([-\d.]+)/)
          || html.match(/([-\d.]{4,}),\s*([-\d.]{4,})/);
  ...
  // Fallback: open tracker page in new tab so user can navigate from there manually
  window.open(LIVE_TRACKING_URL, '_blank');
