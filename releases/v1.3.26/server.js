...
function showSlide(idx) {
  ...
  // Fjern dynamiske video-overlays
  sl.querySelectorAll(".slide-video-overlay").forEach(function(v) { v.pause(); v.remove(); });
...
function _showEvDetail(upcoming, idx, count) {
  ...
  if (botImg) {
    // For tur-events: vis dagskart i bunnen hvis tilgjengelig
    var botSrc = null;
    if (ev.eventType === 'tur' && ev.dayMapImages && ev.dayMapImages.length) {
      // Vis dagens dagskart basert på dato
      var today = new Date().toISOString().slice(0,10);
      var dayIdx = 0;
      if (ev.date) {
        var startDay = new Date(ev.date);
        var nowDay = new Date();
        dayIdx = Math.max(0, Math.min(ev.dayMapImages.length - 1, Math.floor((nowDay - startDay) / 86400000)));
      }
      botSrc = ev.dayMapImages[dayIdx] || ev.dayMapImages[0];
    }
