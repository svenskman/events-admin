...
function generateRegToken(evId, regId) {
  ...
  const secret  = process.env.SESSION_SECRET || (() => { if(process.env.NODE_ENV!=="development") console.warn("[SECURITY] SESSION_SECRET not set — using insecure default!"); return "change-me-in-env"; })();
...
function _broadcastTvUpcoming(department) {
  ...
  deptIds.forEach(function(deptId) {
    ...
    var upcoming = events
      .filter(function(e) {
        return e.department === deptId && e.date && new Date(e.date).getTime() > now - 3600000;
      })
      .sort(function(a, b) { return new Date(a.date) - new Date(b.date); })
      .slice(0, state.eventCount || 5)
      .map(function(e) {
        return { id: e.id, title: e.title, date: e.date, endDate: e.endDate || null, endTime: e.endTime || null, location: e.location, eventType: e.eventType, image: e.image || null };
      });
...
function serveOverviewPage(req, res) {
  ...
  // Only show public events from last 7 days forward
...
app.get("/api/tv/:deptId/event-slides", auth, managerOrAdmin, function(req, res) {
  ...
  events.forEach(function(ev) {
    ...
    var slides = state.slides.filter(function(s) {
      // Only plain image slides (not video-slide objects)
...
app.get("/api/tv/:deptId/stream", function(req, res) {
  ...
  const upcoming = events
    .filter(function(e) { return e.department === deptId && e.date && new Date(e.date).getTime() > now - 3600000; })
    .sort(function(a, b) { return new Date(a.date) - new Date(b.date); })
    .slice(0, state.eventCount || 5)
    .map(function(e) { return { id: e.id, title: e.title, date: e.date, endDate: e.endDate || null, endTime: e.endTime || null, location: e.location, eventType: e.eventType, image: e.image || null }; });
...
app.get("/api/tv/:deptId/state", function(req, res) {
  ...
  const now = Date.now();
  const upcoming = events
    .filter(function(e) { return e.department === req.params.deptId && e.date && new Date(e.date).getTime() > now - 3600000; })
    .sort(function(a, b) { return new Date(a.date) - new Date(b.date); })
    .slice(0, state.eventCount || 5)
    .map(function(e) { return { id: e.id, title: e.title, date: e.date, endDate: e.endDate || null, endTime: e.endTime || null, location: e.location, eventType: e.eventType, image: e.image || null }; });
...
body{background:#0a0a0a;color:#fff;font-family:"Helvetica Neue",Arial,sans-serif;overflow:hidden;height:100dvh;width:100dvw}
...
/* EVENT CARD LAYER */
#eventLayer{position:absolute;inset:0;background:linear-gradient(135deg,#0d0d0d 0%,#111 100%);opacity:0;transition:opacity .8s ease;display:flex;flex-direction:column;justify-content:center;padding:5vh 8vw;overflow:hidden}
...
/* EVENT CARD */
.ev-card-tv{display:flex;gap:3vw;align-items:flex-start;margin-bottom:2.5vh;opacity:0;transform:translateY(30px);transition:opacity .5s,transform .5s}
.ev-card-tv.visible{opacity:1;transform:translateY(0)}
.ev-thumb{width:clamp(70px,11vw,150px);height:clamp(48px,7.5vw,100px);object-fit:cover;border-radius:10px;flex-shrink:0;border:2px solid rgba(255,255,255,.1)}
.ev-thumb-placeholder{width:clamp(70px,11vw,150px);height:clamp(48px,7.5vw,100px);border-radius:10px;flex-shrink:0;display:flex;align-items:center;justify-content:center;font-size:1.8rem;border:2px solid rgba(255,255,255,.08)}
.ev-info{}
.ev-type-badge{font-size:.7rem;font-weight:800;text-transform:uppercase;letter-spacing:.6px;padding:3px 10px;border-radius:12px;display:inline-block;margin-bottom:.5rem}
.ev-title-tv{font-size:clamp(.9rem,2vw,1.6rem);font-weight:900;line-height:1.2;margin-bottom:.3rem}
.ev-meta-tv{font-size:clamp(.68rem,1.1vw,.9rem);color:#888;display:flex;gap:.8rem;flex-wrap:wrap}
.ev-meta-tv span{display:flex;align-items:center;gap:.4rem}

/* SINGLE BIG EVENT (when only 1) */
.ev-single .ev-thumb,.ev-single .ev-thumb-placeholder{width:280px;height:190px}
.ev-single .ev-title-tv{font-size:clamp(2rem,4vw,3rem)}

/* HEADER FOR EVENT SCREEN */
#evScreenTitle{font-size:.8rem;font-weight:800;text-transform:uppercase;letter-spacing:.8px;color:${esc(accent)};margin-bottom:3vh;opacity:.7}
</style>
</head>
<body>
<div id="screen">
  ...
  <div id="slideLayer">
    ...
    <div id="slideEmpty">📺</div>
  </div>

  <!-- Event layer -->
  <div id="eventLayer">
    <div id="evScreenTitle">Kommende arrangementer – ${esc(deptName)}</div>
    <div id="evCards"></div>
  </div>
...
function _connectTvSSE() {
  ...
  es.onmessage = function(e) {
    try {
      var d = JSON.parse(e.data);
      if (d.type === "upcoming") {
        // Live event list update — rebuild cycle with new upcoming
...
function rebuildCycle() {
  ...
  var cycle = [];
  if (!idxList.length) {
    // No slides — just events on loop, or empty
    if (wantEvents) cycle.push({ type: "events" });
  } else {
    // Interleave: events first, then a block of slides, repeat
    var i = 0;
    while (i < idxList.length) {
      if (wantEvents) cycle.push({ type: "events" });
      var end = Math.min(i + SLIDES_PER_BLOCK, idxList.length);
      for (var j = i; j < end; j++) {
...
function step() {
  ...
  // Wrap around — reshuffle each full loop if random
...
function showEventCards(upcoming) {
  var sl = document.getElementById("slideLayer");
  var evLayer = document.getElementById("eventLayer");
  var cards = document.getElementById("evCards");
  var count = Math.min(upcoming.length, state.eventCount || 3);
  var isSingle = count === 1;

  sl.style.opacity = "0";

  setTimeout(function() {
    cards.innerHTML = upcoming.slice(0, count).map(function(ev) {
      ...
      var thumb = ev.image
        ...
        : '<div class="ev-thumb-placeholder" style="background:' + style.bg + ';display:flex;flex-direction:column;align-items:center;justify-content:center;gap:4px">'
        + '<svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="' + style.color + '" stroke-width="1.5" opacity="0.9">'
        + (ev.eventType==="stand"
        ...
        + '</svg>'
        + '<span style="font-size:.55rem;font-weight:700;color:' + style.color + ';text-transform:uppercase;letter-spacing:.5px;opacity:.8">' + evTypeLabel(ev.eventType) + '</span>'
        + '</div>';
      ...
      return '<div class="ev-card-tv' + (isSingle?" ev-single":"") + (ongoing?" ev-ongoing":"") + '" style="' + cardBorder + '">'
        + thumb
        + '<div class="ev-info">'
        + '<div style="display:flex;align-items:center;flex-wrap:wrap;gap:.25rem;margin-bottom:.4rem">'
        + '<span class="ev-type-badge" style="background:' + style.bg + ';color:' + style.color + ';border:1px solid ' + style.border + '">' + evTypeLabel(ev.eventType) + '</span>'
        + ongoingBadge
        + '</div>'
        + '<div class="ev-title-tv">' + esc(ev.title) + '</div>'
        + '<div class="ev-meta-tv">'
        + (ev.date ? '<span><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="vertical-align:middle;margin-right:3px"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>' + fmtDate(ev.date) + '</span>' : '')
        + (ev.location ? '<span><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="vertical-align:middle;margin-right:3px"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7z"/><circle cx="12" cy="9" r="2.5"/></svg>' + esc(ev.location) + '</span>' : '')
        + '</div></div></div>';
    }).join("");
