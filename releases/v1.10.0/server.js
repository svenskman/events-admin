...
function serveDepartmentPage(dept, req, res) {
  ...
  var deptThemeD = deptApp2.themeDept || deptApp2.theme || "dark";
  ...
  var css = '*{box-sizing:border-box;margin:0;padding:0}'
    ...
    + ':root{--y:#f5c500;--g:'+gClr+';--g2:'+g2Clr+';--g3:'+g3Clr+';--r:8px;--text2:'+txt2+';--text:'+bodyClr+'}'
    ...
    + '.header-label{font-size:.6rem;text-transform:uppercase;letter-spacing:.8px;color:'+txt2+';line-height:1;margin-top:.1rem}.header-title{font-size:.85rem;font-weight:800;white-space:nowrap;line-height:1.2}'
    ...
    + '.header-dept{font-size:.85rem;font-weight:800;color:'+bodyClr+';flex:1;text-align:center}'
    ...
    + '.banner-track{display:flex;align-items:center;white-space:nowrap;animation:marquee 30s linear infinite;gap:3rem;font-size:.78rem;color:'+txt2+';padding:0 1rem}'
    + '.banner-track:hover{animation-play-state:paused}'
    + '.banner-item-sep{color:var(--y);font-size:.9rem;flex-shrink:0}'
  ...
  var html = '<!DOCTYPE html><html lang="no"><head>'
    ...
    + '</div>'
    + '<div class="header-dept">' + escHtml(dept.name) + '</div>'
  ...
  if (bannerText) {
    var speed = Math.max(15, Math.round(bannerText.length * 0.3));
    var items = escHtml(bannerText) + '&nbsp;&nbsp;&nbsp;●&nbsp;&nbsp;&nbsp;' + escHtml(bannerText);
    html += '<div class="banner-marquee">'
      + '<div class="banner-track" style="animation-duration:' + speed + 's">' + items + '</div>'
      + '</div>';
  }
...
async function doRegister(){
  ...
  var r=await fetch('/api/events/'+EV_ID+'/register',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name,email,phone,attendance,slotId})});
  var d=await r.json();
