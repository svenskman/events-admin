...
<nav class="ev-tabs">
  <div class="ev-tab active" data-tab="info" onclick="showTab('info');_navFabToggle('info')">ℹ️ Om turen</div>
  <div class="ev-tab" data-tab="rute" onclick="showTab('rute');_navFabToggle('rute')">🗺️ Etappeplan</div>
  <div class="ev-tab" data-tab="reg" onclick="showTab('reg');_navFabToggle('reg')">📋 Påmelding</div>
  <div class="ev-tab" data-tab="forere" onclick="showTab('forere');_navFabToggle('forere')">🏍 Deltagere</div>
  <div class="ev-tab" data-tab="gb" onclick="showTab('gb');_navFabToggle('gb')">✍️ Gjestebok</div>
</nav>
...
<div class="ev-content">
  ...
  <!-- Navigeringsknapp – fast i bunn på etappeplan-fanen -->
  ${days.length ? `
  <div id="navFab" style="display:none;position:fixed;bottom:0;left:0;right:0;z-index:160;padding:.75rem 1rem calc(.75rem + env(safe-area-inset-bottom));background:linear-gradient(transparent,var(--b) 40%)">
    <a href="#" onclick="openGoogleMapsNav();return false;"
      style="display:flex;align-items:center;justify-content:center;gap:.6rem;width:100%;padding:16px;background:var(--y);color:#111;font-weight:900;font-size:1rem;border-radius:12px;text-decoration:none;box-shadow:0 4px 20px rgba(0,0,0,.5);min-height:56px">
...
// Navigering: bygg Google Maps URL fra etappe-data og åpne
function openGoogleMapsNav(){
  var etapper=${JSON.stringify(days.map(function(day){return (day.etapper||[]).map(function(e){return e.til||'';});}).flat().filter(Boolean))};
  if(!etapper.length){alert('Ingen etappeplan registrert.');return;}
  var origin=encodeURIComponent(etapper[0]);
  var dest=encodeURIComponent(etapper[etapper.length-1]);
  var wps=etapper.slice(1,-1).map(encodeURIComponent).join('|');
  var url='https://www.google.com/maps/dir/?api=1&origin='+origin+'&destination='+dest+(wps?'&waypoints='+wps:'')+'&travelmode=driving';
  window.open(url,'_blank');
}
function _navFabToggle(tabId){
  var fab=document.getElementById('navFab');
  if(fab)fab.style.display=(tabId==='rute')?'block':'none';
}
