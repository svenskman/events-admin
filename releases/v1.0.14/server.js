...
<body>
${tabletHeader(ev, settings, 'badge-stand', '🟢 Stand')}
${tabletHero(ev, 'badge-stand', '🟢 Stand')}
...
<body>
${tabletHeader(ev, settings, 'badge-mote', '🟡 Møte')}
${tabletHero(ev, 'badge-mote', '🟡 Møte')}
...
<body>
${tabletHeader(ev, settings, 'badge-kurs', '🔵 Kurs')}
${tabletHero(ev, 'badge-kurs', '🔵 Kurs')}
...
<body>
${tabletHeader(ev, settings, 'badge-tur', '🟣 Tur')}
${tabletHero(ev, 'badge-tur', '🟣 Tur')}
  ...
  <div class="ev-panel" id="tab-reg">
    <div class="ev-card">
      ...
      <div class="ev-form">
        ...
        <input id="rReg" type="text" placeholder="AB 12345" autocomplete="off"/>
...
async function doRegister(){
  ...
  var email=document.getElementById('rEmail').value.trim();
  var phone=document.getElementById('rPhone')?.value.trim()||'';
  var make=document.getElementById('rMake')?.value.trim()||'';
  var model=document.getElementById('rModel')?.value.trim()||'';
  var reg=document.getElementById('rReg')?.value.trim()||'';
  var ecName=document.getElementById('rEcName')?.value.trim()||'';
  var ecPhone=document.getElementById('rEcPhone')?.value.trim()||'';
  if(!name||!email){msg.innerHTML='<div class="msg msg-err">Navn og e-post er påkrevd</div>';return;}
  btn.disabled=true;btn.textContent='Sender…';
  var body={name,email,phone};
  if(make)body.vehicle={make,model,reg};
  ...
  if(r.ok){
    ...
    ['rName','rEmail','rPhone','rMake','rModel','rReg','rEcName','rEcPhone'].forEach(function(id){var el=document.getElementById(id);if(el)el.value='';});
  } else {
    msg.innerHTML='<div class="msg msg-err">'+esc(d.error||'Feil')+'</div>';
  }
}
