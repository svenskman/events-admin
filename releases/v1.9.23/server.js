...
function serveDepartmentPage(dept, req, res) {
  ...
  var css = '*{box-sizing:border-box;margin:0;padding:0}'
    ...
    + '/* Mobil tab-nav */'
    + '.mob-tabs{display:none;gap:.5rem;padding:.75rem 1rem;overflow-x:auto;background:var(--g);border-bottom:1px solid var(--g3);position:sticky;top:0;z-index:50;scrollbar-width:none}'
  ...
  if (filtered.length === 0) {
    ...
    // Build mob-tabs HTML separately — will be injected before </body>
    var mobTabsHtml = '';
    if (showFilterBar) {
      ...
      mobTabsHtml += '<div class="mob-tabs" id="mobTabs" style="'
        + 'display:none;'  // shown via CSS on mobile
        + 'position:fixed;bottom:0;left:0;right:0;z-index:1000;'
        + 'background:'+headerBg+';border-top:1px solid '+divBorder+';'
        + 'padding:.5rem .75rem;gap:.4rem;'
        + 'justify-content:space-around;align-items:center;'
        + 'box-shadow:0 -2px 12px rgba(0,0,0,.15)'
        + '">';
      mobTabsHtml += '<div style="display:flex;gap:.4rem;overflow-x:auto;scrollbar-width:none;flex:1">';
      // 'Alle' first when multiple columns exist
      if (columns.length > 1) {
        mobTabsHtml += '<button class="mob-tab active" id="mobTabAll" onclick="switchMobTabAll()" style="white-space:nowrap">Alle</button>';
      }
      columns.forEach(function(kol, ki) {
        mobTabsHtml += '<button class="mob-tab" data-ki="' + ki + '" onclick="switchMobTab(' + ki + ')">' + escHtml(kol.label) + '</button>';
      });
      mobTabsHtml += '</div>';
      if (allHistCount > 0) {
        mobTabsHtml += '<button id="histBtnAll" onclick="toggleAllHistory()" data-count="' + allHistBtnCount + '" style="'
