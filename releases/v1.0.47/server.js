...
function _textToEmailHtml(text, siteName, settings) {
  ...
  // Parse markdown-lignende syntaks linje for linje
  ...
  while (i < lines.length) {
    ...
    if (/^[-•*]\s/.test(line)) {
      ...
      while (i < lines.length && /^[-•*]\s/.test(lines[i])) {
        items.push("<li style=\"margin-bottom:.35rem\">" + parseLine(lines[i].replace(/^[-•*]\s/, "")) + "</li>");
        i++;
      }
      blocks.push('<ul style="margin:0 0 1em;padding-left:1.5em;color:#333;font-size:.95rem;line-height:1.7">' + items.join("") + "</ul>");
      continue;
    }
...
app.put("/api/departments/:deptId/subgroups/:sgId/appearance", auth, function(req, res) {
  ...
  if (deptIdx < 0) return res.status(404).json({ error: "Avdeling ikke funnet" });
  const sgIdx = (depts[deptIdx].subgroups || []).findIndex(function(sg) { return sg.id === req.params.sgId; });
  if (sgIdx < 0) return res.status(404).json({ error: "Undergruppe ikke funnet" });
...
function buildConsolationEmail(ev, reg, settings) {
  ...
  const customTpls = ev._resolvedEmailTemplates || (ev.emailTemplates ? ev.emailTemplates : null) || settings.emailTemplates || {};
  const tpl = customTpls["lottery_consolation"];
  if (tpl && tpl.subject && tpl.body) {
    const subject = applyEmailVars(tpl.subject, ev, reg, settings);
...
app.post("/api/events/:id/lottery/send-consolation-email", auth, managerOrAdmin, function(req, res) {
  ...
  // Send to all non-winners (those who registered but did not win)
  const nonWinners = regs.filter(function(r) { return !winners.has(r.id); });
  if (!nonWinners.length) return res.status(400).json({ error: "Ingen ikke-vinnere å sende til" });
  ...
  nonWinners.forEach(function(reg) {
    ...
    sendEmail({ to: reg.email, subject: mail.subject, html: mail.html, text: mail.text })
      .catch(function(e) { console.error("[consolation] Send feil:", e && e.message); });
    sent++;
  });
...
function applyEvLang() {
  ...
  if(document.getElementById('tab-info')) {
    ...
    if(infoTab) infoTab.innerHTML = tabMap.info || '';
    if(regTab)  regTab.innerHTML  = tabMap.reg  || '';
    if(gbTab)   gbTab.innerHTML   = tabMap.gb   || '';
  }
  // Labels in reg form
