...
app.get("/api/search", auth, function(req, res) {
  ...
  // Inventory
  var inventarResults = readJSON(INVENTAR_FILE).filter(function(item) {
    if (user.role === "admin") return true;
    if (!myAvds.length) return false;
    return !item.department || myAvds.includes(item.department);
  }).filter(function(item) {
    return (item.navn        && item.navn.toLowerCase().includes(q))
        || (item.beskrivelse && item.beskrivelse.toLowerCase().includes(q))
    ...
    return {
      ...
      id: item.id,
      navn: item.navn,
      kategori: item.kategori || "annet",
      antall: item.antall || 0,
      perPakke: item.perPakke || 1,
      enhet: item.enhet || "stk",
      bilde: item.bilde || null
    };
  });
    ...
    ${(function(){
      ...
      if (days.length > 1) {
        days.forEach(function(d, di) {
          var fra = (d.etapper && d.etapper[0] && d.etapper[0].fra) || '';
          var lastE = d.etapper && d.etapper[d.etapper.length - 1];
...
app.post("/api/events/:id/utstyr", auth, managerOrAdmin, function(req, res) {
  ...
  const antall = parseInt(req.body.antall) || 1;
  ...
  if (existing) {
    existing.antall = (existing.antall || 1) + antall;
  } else {
...
app.get("/api/public/events/:id/gpx", rateLimit(10, 60000), async function(req, res) {
  ...
  const allNames = [];
  days.forEach(function(d){ (d.etapper||[]).forEach(function(e){ if(e.fra&&!allNames.includes(e.fra))allNames.push(e.fra); if(e.til&&!allNames.includes(e.til))allNames.push(e.til); }); });
  ...
  const segCache = {};
  for (var di=0;di<days.length;di++) { var etapper=days[di].etapper||[]; for(var ei=0;ei<etapper.length;ei++){ var e=etapper[ei]; var fra=coordCache[e.fra],til=coordCache[e.til]; if(!fra||!til)continue; var key=e.fra+"||"+e.til; if(!segCache[key]){segCache[key]=await fetchSegment(fra,til);await sleep(200);} } }
  ...
  function buildGpx(title, dayList) {
    ...
    var seenWpt={};
    dayList.forEach(function(d,dIdx){ (d.etapper||[]).forEach(function(e,eIdx){ [[e.fra,eIdx===0&&dIdx===0?"start":"wpt"],[e.til,e.type==="slutt"?"end":e.type==="hotell"?"hotel":"wpt"]].forEach(function(pair){ var nm=pair[0],wt=pair[1]; if(!nm||seenWpt[nm])return; seenWpt[nm]=true; var c=coordCache[nm];if(!c)return; var sym=wt==="start"?"Flag, Green":wt==="end"?"Flag, Red":wt==="hotel"?"Lodging":"Waypoint"; gpx+='  <wpt lat="'+c.lat.toFixed(6)+'" lon="'+c.lon.toFixed(6)+'">\n    <name>'+xmlEsc(nm)+'</name><sym>'+sym+'</sym>\n  </wpt>\n'; }); }); });
    dayList.forEach(function(d){ (d.etapper||[]).forEach(function(e,eIdx){ var fra=coordCache[e.fra],til=coordCache[e.til];if(!fra||!til)return; var key=e.fra+"||"+e.til; var pts=segCache[key]||[{lat:fra.lat,lon:fra.lon},{lat:til.lat,lon:til.lon}]; gpx+='  <trk><name>'+xmlEsc(title+" – etappe "+(eIdx+1))+'</name><trkseg>\n'; pts.forEach(function(pt){gpx+='    <trkpt lat="'+pt.lat.toFixed(6)+'" lon="'+pt.lon.toFixed(6)+'"/>\n';}); gpx+='  </trkseg></trk>\n'; }); });
    gpx+='</gpx>';
    return gpx;
  }
