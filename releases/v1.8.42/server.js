...
app.get("/api/public/events/:id/gpx", rateLimit(10, 60000), async function(req, res) {
  ...
  function buildGpx(title, dayList) {
    ...
    var seenWpt={};
    dayList.forEach(function(d,dIdx){ (d.stages||[]).forEach(function(e,eIdx){ [[e.from,eIdx===0&&dIdx===0?"start":"wpt"],[e.to,e.type==="slutt"?"end":e.type==="hotell"?"hotel":"wpt"]].forEach(function(pair){ var nm=pair[0],wt=pair[1]; if(!nm||seenWpt[nm])return; seenWpt[nm]=true; var c=coordCache[nm];if(!c)return; var sym=wt==="start"?"Flag, Green":wt==="end"?"Flag, Red":wt==="hotel"?"Lodging":"Waypoint"; gpx+='  <wpt lat="'+c.lat.toFixed(6)+'" lon="'+c.lon.toFixed(6)+'">\n    <name>'+xmlEsc(nm)+'</name><sym>'+sym+'</sym>\n  </wpt>\n'; }); }); });
    dayList.forEach(function(d){ (d.stages||[]).forEach(function(e,eIdx){ var fra=coordCache[e.from],til=coordCache[e.to];if(!fra||!til)return; var key=e.from+"||"+e.to; var pts=segCache[key]||[{lat:fra.lat,lon:fra.lon},{lat:til.lat,lon:til.lon}]; gpx+='  <trk><name>'+xmlEsc(title+" – etappe "+(eIdx+1))+'</name><trkseg>\n'; pts.forEach(function(pt){gpx+='    <trkpt lat="'+pt.lat.toFixed(6)+'" lon="'+pt.lon.toFixed(6)+'"/>\n';}); gpx+='  </trkseg></trk>\n'; }); });
    gpx+='</gpx>';
    return gpx;
  }
  ...
  function buildGpx(title, dayList) {
    ...
    dayList.forEach(function(d, dIdx) {
      (d.stages || []).forEach(function(e, eIdx) {
        [[e.from, eIdx === 0 && dIdx === 0 ? "start" : "wpt"],
         [e.to, e.type === "slutt" ? "end" : e.type === "hotell" ? "hotel" : "wpt"]
