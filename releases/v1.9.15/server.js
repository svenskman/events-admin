...
app.put("/api/departments/:id/settings", auth, managerOrAdmin, function(req, res) {
  ...
  var allowed = ["historyDays", "showEventTypes", "showSubgroupFilter", "showCountdown"];
...
function serveDepartmentPage(dept, req, res) {
  ...
  if (filtered.length === 0) {
    ...
    if (showFilterBar) {
      ...
      html += '</div>';
      // History toggle is per-column (in col-head), not global
      html += '</div>';
    }
    html += '<div class="cols">';
    columns.forEach(function(kol, ki) {
  ...
  html += '</div><footer>' + escHtml(siteName) + ' · ' + escHtml(dept.name) + '</footer>'
    ...
    + '}\n'
    + 'function switchMobTab(ki){document.querySelectorAll(".col-section").forEach(function(s,i){s.classList.toggle("mob-active",i===ki);});document.querySelectorAll(".mob-tab").forEach(function(b,i){b.classList.toggle("active",i===ki);});var w=document.getElementById("colWrap"+ki);if(w){var aktiv=w.querySelector(".ev-card.active");var target=aktiv||w.querySelector(".ev-card");if(target){var top=target.getBoundingClientRect().top+window.scrollY-120;window.scrollTo({top:Math.max(0,top),behavior:"smooth"});}}}\n'
