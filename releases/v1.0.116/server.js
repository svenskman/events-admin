...
app.get("/api/events/:id/routecoords", auth, rateLimit(10, 60000), async function(req, res) {
  ...
  async function geocode(name) {
    ...
    if (!global._geocodeCache) global._geocodeCache = {};
    if (global._geocodeCache[name]) return global._geocodeCache[name];
    try {
      ...
      const d = await r.json();
      if (d && d[0]) {
        // Finn første treff innenfor Skandinavia
        const nordic = d.find(function(r2) {
          const la = parseFloat(r2.lat), lo = parseFloat(r2.lon);
          return la >= 54 && la <= 72 && lo >= -5 && lo <= 35;
        });
        if (nordic) {
          global._geocodeCache[name] = { lat: parseFloat(nordic.lat), lon: parseFloat(nordic.lon), display: nordic.display_name };
        }
      }
    } catch(e) {}
    return global._geocodeCache[name] || null;
  }
  ...
  for (var di = 0; di < days.length; di++) {
    ...
    for (var ei = 0; ei < etapper.length; ei++) {
      ...
      seg.forEach(function(p) { allCoords.push(p); });
      segmentStatus.push({ fra: e.fra, til: e.til, ok: true, points: seg.length });
  ...
  res.json({ coords: allCoords, waypoints, coordCache, geocodeStatus, segmentStatus });
});
...
app.post("/api/events/:id/routemap-upload", auth, managerOrAdmin,
  ...
  function(req, res) {
    ...
    const fname = "routemap_" + hash + ".png";
